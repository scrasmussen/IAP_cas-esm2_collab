...
!     Output Arrays
!     ------------------------------------------------------------------
      real(r4),dimension(:,:),allocatable::z0mon,himon,hdmon
!
!linpf091126      
      real(r4),dimension(:,:),allocatable::lthfmon,sshfmon,lwvmon,swvmon
      real(r4),dimension(:,:),allocatable::sumon,svmon
!linpf091126
!
      real(r4),dimension(:,:,:),allocatable::wsmon,tsmon,ssmon,usmon,vsmon
      real(r4),dimension(:,:,:),allocatable::visomon,uisomon,wisomon
      real(r4),dimension(:,:,:),allocatable:: icmon
      real(r4),dimension(:,:,:),allocatable::netmon
!
!lhl1204
      real(r4),dimension(:,:),allocatable::mldmon
      real(r4),dimension(:,:,:),allocatable::akmmon,aktmon,aksmon
!lhl1204
!
      real(r4),dimension(:,:,:),allocatable:: psi
      real(r4),dimension(:,:),allocatable:: bsf
      real(r4),dimension(:,:,:),allocatable:: mth,mth_adv,mth_dif,mth_adv_iso
!
      real(r4),dimension(:,:,:,:),allocatable::trendmon
      real(r4),dimension(:,:,:,:),allocatable::axmon,aymon,azmon
      real(r4),dimension(:,:,:,:),allocatable::dxmon,dymon,dzmon
      real(r4),dimension(:,:,:),allocatable::penmon
!
      real(r4),dimension(:,:,:,:),allocatable::ddymon

#ifdef ISO
      real(r4),dimension(:,:,:,:),allocatable::axmon_iso,aymon_iso,azmon_iso
      real(r4),dimension(:,:,:,:),allocatable::dxmon_iso,dymon_iso,dzmon_iso
      real(r4),dimension(:,:,:,:),allocatable::aaymon_iso,ddymon_iso
#endif

#ifdef omipout
     real(r4),dimension(:,:,:),allocatable::psurmon,ritmon,rictmon
#endif
...
!#endif
#if (defined SMAG_OUT)
      real(r4),dimension(:,:,:),allocatable::a3mon
#endif
