...
!FRESH1(I,J)*(ATB (I,J,1,2)*1000.0+35.0)*1.0e-3/DZP(1)*OD0   
   fresh11(i,j)=-vit(i,j,1)*fresh1(i,j)*(ATB(I,J,1,2)*1000.0+35.0)*1.0e-3*OD0 
   u3(I,J)=vit(i,j,1)*(1.0-ifrac(i,j))*(SQRT(duu10n(I,J)))**3  
 !  write(5,*) fresh11(56,:)
      end do
      end do
!JJB20140830   MAKE SURE GLOBAL AREA-MEAN FRESH FLUX IS ZERO
...
! mius(I,J) = -(0.002256*(SQRT(duu10n))**3.*(ATB (I,J,1,2)*1000.0+35.0)+0.11107)*(1.0-seaice(i,j))*0.01/365./86400/1000.0*VIT(I,J,1)
  ...
  mius(i,j)=-FRESH011*(SQRT(duu10n(i,j)))**3.*(ATB(I,J,1,2)*1000.0+35.0)*(1.0-ifrac(i,j))*VIT(I,J,1)*1.0e-3
...
#ELSE
    SSF(i,j) =-vit(i,j,1)*(prec(i,j)+evap(i,j)+ meltw(i,j)+roff(i,j))*34.7*1.0e-3/DZP(1)*OD0
#endif   
!  write(4,*) SSF(56,:)                      
   ! jjb 20140713  
        end do
...
!        psa=patm
        ...
        where(vit(:,:,1)<0.5) mius=spval
        where(vit(:,:,1)<0.5) psa=spval

!
#ifdef biochem
#ifdef USE_OCN_CARBON
!for instant purpose lyc        call exchange_2d(pco2)
#endif
#endif         
!
! surface stress
