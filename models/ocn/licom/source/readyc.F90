...
!      REAL(r8)  :: WKP (KMP1)
      ...
      REAL(r6)  :: xxx
#ifdef BACKD
 real(r8) ::      diff_back(imt,jmt),&
                  diff_back_sh(imt,jmt),&
                  diff_back_nh(imt,jmt)
#endif
...
#ifdef omipout
            ritsave(1,j,k)=ritsave(imm,j,k)
            ritsave(imt,j,k)=ritsave(2,j,k)
#endif
...
!lhl0711      call exch_boundary(s2t,kmm1)
      call exch_boundary(rit,kmm1) !jjr bug
#ifdef omipout
      call exch_boundary(ritsave,kmm1)
#endif
...
!         END DO
   ...
   DO J = 1, JMT
         DO I = 1, IMT
          if(lat(j)<0.0) then
           diff_back_sh(i,j)=diff_back_coef_max*exp(-(0.4d0*(lat(j)+28.9))**2)
          else
           diff_back_nh(i,j) =diff_back_coef_max*exp(-(0.4d0*(lat(j)-28.9))**2)
          endif
          diff_back(i,j)=diff_back_eq+diff_back_sh(i,j)+diff_back_nh(i,j)
        if ( lat(j) .lt. -10.0 ) then
          diff_back(i,j) = diff_back(i,j) + diff_back_coef
        elseif  ( abs(lat(j)) .le. 10.0 ) then
          diff_back(i,j) = diff_back(i,j,iblock) +diff_back_coef*(abs(lat(j))/10.0)**2
        else
          diff_back(i,j) = diff_back(i,j) + diff_back_coef
        endif
        ENDDO
...
!AKT_BACK,AKS_BACK?
#ifdef BACKD 
        AKMT(I,J,K)=WK1(K)*1.0d-4+diff_back(i,j)*10.0*1.d-4
        AKT(I,J,K,1)=AKT(I,J,K,1)+(WK2(K))/NCC*1.d-4+diff_back(i,j)/float(NCC)*1.d-4
        AKT(I,J,K,2)=AKT(I,J,K,2)+(WK3(K))/NCC*1.d-4+diff_back(i,j)/float(NCC)*1.d-4
#else
