...
!$OMP PARALLEL DO PRIVATE (J,I)
            ...
            STF (I,J) = SSF(I,J)/ODZP(1)&
                        +(GAMMA*50.*ODZP(1))*(SSS(I,J) &  
                        -ATB(I,J,1,2))*ifrac(i,j)/ODZP(1)&
                        +(GAMMA*30./365./4.*50.*ODZP(1))*(SSS(I,J)-ATB(I,J,1,2))/ODZP(1) 
#else
...
!
    ...
    DO j=jsm,jem
        DO i= 2,imm
        ERR0 = ERR0 + DYT(J)* SINT (J)* NET(I,J,2)*vit(I,J,1)
        END DO
     END DO
#ifdef SPMD 
      ...
      DO J = JSM,JEM
        DO I = 2,IMM
          TF (I,J,1)=TF (I,J,1)+NET01*vit(I,J,1)
        END DO
     END DO          
#endif
...
#ifdef SPMD
       call exch_boundary(vtl(1,1,1),km) !need to be changed !LPF20131015
#endif
!LPF20140125 !del for test
!wangty bug
         else
