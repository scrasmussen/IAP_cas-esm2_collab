...
#if (defined SMAG_OUT)
      real(r4),dimension(imt_global,jmt_global,km,1)::  t3_cdf !LPF 2012Jul27
#endif
...
!      write(fname1(11:12),'(i2.2)')mon0
      if ((mon0-1)==0) then 
        write(fname1(11:12),'(i2.2)')mon0+11
      else 
...
!
...
#ifdef omipout
         psur_dims (4) = time_dim
         psur_dims (3) = lev_dim
         psur_dims (2) = lat_dim
         psur_dims (1) = lon_dim
         iret = nf_def_var (ncid, 'psur', NF_REAL, psur_rank, psur_dims,psur_id)
         CALL check_err (iret)
         ...
         rit_dims (2) = lat_dim
         rit_dims (1) = lon_dim
         iret = nf_def_var (ncid, 'rit', NF_REAL, rit_rank, rit_dims,rit_id)
         CALL check_err (iret) 

         rict_dims (4) = time_dim
         rict_dims (3) = lev_dim
         rict_dims (2) = lat_dim
         rict_dims (1) = lon_dim
         iret = nf_def_var (ncid, 'rict', NF_REAL, rict_rank, rict_dims,rict_id)
         CALL check_err (iret) 
         ...
         !uiso
         uiso_dims (4) = time_dim
         uiso_dims (3) = lev_dim
         uiso_dims (2) = lat_dim
         uiso_dims (1) = lon_dim
         iret = nf_def_var (ncid, 'uiso', NF_REAL, uiso_rank, uiso_dims,uiso_id)
         CALL check_err (iret)

         !viso
         viso_dims (4) = time_dim
         viso_dims (3) = lev_dim
         viso_dims (2) = lat_dim
         viso_dims (1) = lon_dim
         iret = nf_def_var (ncid, 'viso', NF_REAL, viso_rank, viso_dims,viso_id)
         CALL check_err (iret)

         !wiso
         wiso_dims (4) = time_dim
         wiso_dims (3) = lev_dim
         wiso_dims (2) = lat_dim
         wiso_dims (1) = lon_dim
         iret = nf_def_var (ncid, 'wiso', NF_REAL, wiso_rank, wiso_dims,wiso_id)
         CALL check_err (iret)
...
!JJB 20181114
#ifdef NEWSSBC
        iret = nf_put_att_text (ncid, hd_id, 'long_name', 28, 'outward salt flux')
         CALL check_err (iret)
         iret = nf_put_att_text (ncid, hd_id, 'units', 8, 'psu*cm/s')
         CALL check_err (iret) 
#else
...
!
...
#ifdef omipout
         iret = nf_put_att_text (ncid, psur_id, 'long_name', 8, 'pressure')
         CALL check_err (iret)
         iret = nf_put_att_text (ncid, psur_id, 'units', 2, 'Pa')
         CALL check_err (iret)
         iret = nf_put_att_real (ncid, psur_id, '_FillValue', NF_REAL, 1, spval)
         CALL check_err (iret)

         iret = nf_put_att_text (ncid, rit_id, 'long_name', 8, 'vaisala')
         CALL check_err (iret)
         iret = nf_put_att_text (ncid, rit_id, 'units', 2, 'Pa/m2')
         CALL check_err (iret)
         iret = nf_put_att_real (ncid, rit_id, '_FillValue', NF_REAL, 1, spval)
         CALL check_err (iret)


         iret = nf_put_att_text (ncid, rict_id, 'long_name', 2, 'Ri')
         CALL check_err (iret)
         iret = nf_put_att_text (ncid, rict_id, 'units', 2, 'Pa/m2')
         CALL check_err (iret)
         iret = nf_put_att_real (ncid, rict_id, '_FillValue', NF_REAL, 1, spval)
         CALL check_err (iret)
#endif  
         ...
         !uiso
          iret = nf_put_att_text (ncid, uiso_id, 'long_name', 26, 'eddy induced zonal current')
         CALL check_err (iret)
         iret = nf_put_att_text (ncid, uiso_id, 'units', 3,  'm/s')
         CALL check_err (iret)
         iret = nf_put_att_real (ncid, uiso_id, '_FillValue', NF_REAL,1,spval)
         CALL check_err (iret)
      !viso
         iret = nf_put_att_text (ncid, viso_id, 'long_name', 31, 'eddy induced meridional current')
         CALL check_err (iret)
         iret = nf_put_att_text (ncid, viso_id, 'units', 3,  'm/s')
         CALL check_err (iret)
         iret = nf_put_att_real (ncid, viso_id, '_FillValue', NF_REAL,1,spval)
         CALL check_err (iret)
      !wiso
         iret = nf_put_att_text (ncid, wiso_id, 'long_name', 29, 'eddy induced vertical current')
         CALL check_err (iret)
         iret = nf_put_att_text (ncid, wiso_id, 'units', 3,  'm/s')
         CALL check_err (iret)
         iret = nf_put_att_real (ncid, wiso_id, '_FillValue', NF_REAL,1,spval)
         CALL check_err (iret)
...
!   define global attribute
         ...
         CALL check_err (iret)
         iret = NF_PUT_ATT_TEXT (NCID, NF_GLOBAL, 'source', 19, 'CAS-ESM Ocean Model')
         CALL check_err (iret)
! leave define mode
         iret = nf_enddef (ncid)
         CALL check_err (iret)
...
!pressure
#ifdef SPMD
         call local_to_global_4d(psurmon,buffer3_r4,klv,1,0)
         if(mytid==0) then
         iret = nf_put_vara_real (ncid,psur_id, start4, count4, buffer3_r4)
         CALL check_err (iret)
         endif !mytid==0

        call local_to_global_4d(ritmon,buffer3_r4,klv,1,0)
         if(mytid==0) then
         iret = nf_put_vara_real (ncid,rit_id, start4, count4, buffer3_r4)
         CALL check_err (iret)
         endif !mytid==0
        
        call local_to_global_4d(rictmon,buffer3_r4,klv,1,0)
         if(mytid==0) then
         iret = nf_put_vara_real (ncid,rict_id, start4, count4, buffer3_r4)
         CALL check_err (iret)
         endif !mytid==0 
...
!uiso
#ifdef SPMD
         call local_to_global_4d(uisomon(:,:,:),buffer3_r4,klv,1,1)
         if(mytid==0) then
         iret = nf_put_vara_real (ncid,uiso_id, start4, count4, buffer3_r4)
         CALL check_err (iret)
         endif !mytid==0
#endif
!viso
#ifdef SPMD
         call local_to_global_4d(visomon(:,:,:),buffer3_r4,klv,1,1)
         if(mytid==0) then
         iret = nf_put_vara_real (ncid,viso_id, start4, count4, buffer3_r4)
         CALL check_err (iret)
         endif !mytid==0
#endif
!wiso
#ifdef SPMD
         call local_to_global_4d(wisomon(:,:,:),buffer3_r4,klv,1,1)
         if(mytid==0) then
         iret = nf_put_vara_real (ncid,wiso_id, start4, count4, buffer3_r4)
         CALL check_err (iret)
         endif !mytid==0
#endif
