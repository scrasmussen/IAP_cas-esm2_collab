...
!     variable id
      ...
      integer::  lat_id,lon_id,lev1_id,lev_id,time_id,z0_id,hi_id,hd_id, &
             ...
             ,h0_id,u_id,v_id,t_id,s_id,h1_id,h2_id,h3_id,month_id, day_id,uiso_id,viso_id,wiso_id

#ifdef omipout
     integer::psur_id,rit_id,rict_id
#endif
...
!YU
      integer, parameter ::lat_rank=1,lon_rank=1,lev_rank=1,time_rank=1,z0_rank=3,hi_rank=3, &
             ...
             ,h0_rank=2,u_rank=3,v_rank=3,t_rank=3,s_rank=3,h1_rank=2,h2_rank=2,h3_rank=2,month_rank=1,day_rank=1,uiso_rank=4,viso_rank=4,wiso_rank=4

#ifdef omipout
 integer, parameter ::psur_rank=4,rit_rank=4,rict_rank=4
#endif
...
!     variable shapes
      integer::  lat_dims(lat_rank),lon_dims(lon_rank),lev_dims(lev_rank),time_dims(time_rank), &
             ...
             ,h1_dims(h1_rank),h2_dims(h2_rank),h3_dims(h3_rank),uiso_dims(uiso_rank),viso_dims(viso_rank),wiso_dims(wiso_rank) 
#ifdef omipout
    integer::psur_dims(psur_rank),rit_dims(rit_rank),rict_dims(rict_rank)
#endif
