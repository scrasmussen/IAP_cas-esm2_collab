...
!TianyiWang,JinrongJiang, 20160116
      namelist /namctl/ AFB1,AFC1,AFT1,IDTB,IDTC,IDTS,AMV,AHV,NUMBER, &
                        IO_HIST,IO_REST,klv,DLAM,AM_TRO,AM_EXT,&
                        diag_msf,diag_bsf,diag_budget,diag_mth,hist_freq,out_dir,&
                        nx_proc,ny_proc,imt_global,jmt_global,km
!TianyiWang,JinrongJiang, 20160116
!-------------------------------------------------------
...
!-------------------------------------------------------
      ...
      if (mytid==0) then
	      open(11,file='licom_in',form='formatted',status='OLD')
!Tianyiwang, 20150910
	      rewind 11
	      read(11,namctl)
	      close(11)
...
!     call mpi_barrier(mpi_comm_ocn,ierr)
      ...
      if (mytid == 0) write(*,*) "nstart1=",nstart
...
!TianyiWang,JinrongJiang,2016.01.16
      call mpi_bcast(diag_bsf,1,mpi_logical,0,mpi_comm_ocn,ierr) !LPF20131027
      call mpi_bcast(nx_proc,1,mpi_logical,0,mpi_comm_ocn,ierr) !jjr
      call mpi_bcast(ny_proc,1,mpi_logical,0,mpi_comm_ocn,ierr) !LPF20131027
      call mpi_bcast(imt_global,1,mpi_logical,0,mpi_comm_ocn,ierr) !LPF20131027
      call mpi_bcast(jmt_global,1,mpi_logical,0,mpi_comm_ocn,ierr) !LPF20131027
      call mpi_bcast(km,1,mpi_logical,0,mpi_comm_ocn,ierr) !LPF20131027

       nproc=nx_proc*ny_proc
      jet=(jmt_global-jst_global+1)/ny_proc+1+num_overlap
      jem=jet-1 ! Number of the End Grid for Momentum in
       jmt=jet
      imt=(imt_global-2)/nx_proc+num_overlap
...
!TianyiWang,20150910
      ...
      read(11,namctl)
!TianyiWang,JinrongJiang,2016.01.16
      jet=jmt_global   ! Number of the End Grid for Tracer
      jem=jet-1 ! Number of the End Grid for Momentum in
      jmt=jmt_global
      imt=imt_global
!TianyiWang,JinrongJiang,2016.01.16
...
#endif
!TianyiWang,JinrongJiang,2016.01.16                                                       
        jmm_global=jmt_global-1                              
        imm_global=imt_global-1                              
        imm=imt-1                                            
        jmm=jmt-1                                            
        kmp1=km+1                                            
        kmm1=km-1                                          
