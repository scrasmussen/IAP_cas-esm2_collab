...
!----------------------------------------------------------------------
      ...
      real,dimension(imt,jmt)::dpco2a
      real,dimension(imt,jmt)::nta_pt
#endif      
!
...
! add biological model variable 
      ...
      real*8::delldoc
      real::sigma_b
      real,dimension(km)::r_cap,delta_a
      real,dimension(km)::kappa_b
      real,dimension(imt,jmt)::nta_pt
! biological source, suffix 'b' mean biological variables       
      real,dimension(imt,jmt,km)::a0_b,a1_b,a2_b,b0_b,b1_b,b2_b,c_b
...
!---some constant for iron cycle------------------------------------
     ...
     real,dimension(km)::Fe_scav_prof,ReFe !ReFe the remineralization of POFe(including 0.6*Fe_scav)
     real,dimension(imt,jmt,km)::dust_in
     real,dimension(imt,jmt,km)::Fe_scav
     real,dimension(imt,jmt,kmp1)::P_Fe
     real,dimension(imt,jmt,km)::Fe_source
     real,dimension(imt,jmt,km)::TFe_scav !the column integrate
     real,dimension(imt,jmt)::r_fec,r_fep
!     real,dimension(imt,jmt,km)::Fe_free
#ifdef SPMD
     real,dimension(imt_global,jmt_global,km)::flux_Fe_io
     real,dimension(imt_global,jmt_global,km)::Fe_scav_io
...
!
      ...
      real,dimension(imt,jmt)::pco2a,pco2o,dpco2o
#endif
!
#ifdef carbonC14
      integer,parameter::nptra=1
...
! global variables for carbonC, carbonC14 and carbonBio
      real,dimension(imt,jmt,km,nptra)::pt
      real,dimension(imt,jmt,0:km,nptra)::ptb
      real,dimension(imt,jmt,km,nptra)::ptf
      real,dimension(imt,jmt)::sge,ssfc
      integer::yearR,monthR
      integer::NSTARTC
      integer::isp
!
#ifdef SPMD
      real,dimension(imt_global,jmt_global,km,nptra)::pt_io
      real,dimension(imt_global,jmt_global,km+1,nptra)::ptb_io
#endif      
 real,dimension(imt_global,jmt_global,km)::wstmon
 real,dimension(imt_global,jmt_global,kmp1)::wst_io
!
!add the output of a0 and caco3
real,dimension(imt,jmt,km)::tocaco3,toa0
#ifdef SPMD
      real,dimension(imt_global,jmt_global,km)::tocaco3_io,toa0_io
#endif   
...
! biological model forcing data
      real,dimension(imt,jmt,kmmix,12)::po4obs
      real,dimension(km,12)::taobs
      real,dimension(imt,jmt,kmmix)::po4force
      real,dimension(km)::taforce
!lyc 2013,07
      real,dimension(imt,jmt,12)::fe_flux,dust_flux !umol/cm2/s->10m*umol/l/s->10m*umol/kg/s/1.025 
      real,dimension(imt,jmt)::fe_f,dust_f  !iron flux in a day
#endif
!
#ifdef carbonC14
...
! global variables for carbonC, carbonC14 and carbonBio
      ...
      real,dimension(imt,jmt)::w22np
      real,dimension(imt,jmt,12)::winds
!cm090330---------------------------------
      real,dimension(imt,jmt,12)::pressure
      real,dimension(imt,jmt)::pressureday
      real,dimension(imt,jmt)::iceday
!cm090330---------------------------------
!      
#ifdef SPMD
      real,dimension(imt_global,jmt_global,12)::winds_global
!cm090330---------------------------------
      real,dimension(imt_global,jmt_global,12)::pressure_global
!cm090330---------------------------------
#ifdef carbonBio
      real,dimension(imt_global,jmt_global,kmmix,12)::po4obs_global
#endif
#endif      
!
...
!
#if (defined carbonC) || (defined carbonBio) || (defined carbonAbio)
      real,dimension(imt,jmt)::tpco2o,tdpco2o,totup
#endif
!
#ifdef carbonBio
      real,dimension(imt,jmt,km)::po4mon,ldocmon,tamon 
      real,dimension(imt,jmt,km)::prodmon,fpopmon,pldocmon,remimon
      real,dimension(imt,jmt,km)::jpopmon,caco3mon
      real,dimension(imt,jmt,km)::o2mon,femon  !lyc,2013.05
!for flux
      real,dimension(imt,jmt)::o2up,feup
#ifdef COUP
      real*8,dimension(imt,jmt)::uptake
#endif
#endif
!
#ifdef carbonC14
   real,dimension(imt,jmt)::ssfcmon
#ifdef SPMD
   real,dimension(imt_global,jmt_global)::ssfcmon_io
#endif
#endif
!lyc
#ifdef cfc
...
! global variables for carbonC, carbonC14 and carbonBio
      ...
      real,dimension(imt,jmt,km)::ccmon
!      
#ifdef SPMD
      real,dimension(imt_global,jmt_global,km)::ccmon_io
#if (defined carbonC) || (defined carbonBio) || (defined carbonAbio)
      real,dimension(imt_global,jmt_global)::tpco2o_io,tdpco2o_io,totup_io
#endif
#ifdef carbonBio
     real,dimension(imt_global,jmt_global,km)::po4mon_io,ldocmon_io,tamon_io
     real,dimension(imt_global,jmt_global,km)::prodmon_io,fpopmon_io,pldocmon_io,remimon_io
     real,dimension(imt_global,jmt_global,km)::jpopmon_io,caco3mon_io
     real,dimension(imt_global,jmt_global,km)::o2mon_io,femon_io !lyc 2013.06
!for flux,lyc 2013.06
     real,dimension(imt_global,jmt_global)::O2up_io,Feup_io
#endif
#endif      
!
...
!   integer::ttVarID,ssVarID,ccVarID
   ...
   real,dimension(imt,jmt_global)::cct
