...
!     =================
      subroutine SSAVEINS(rstwr,nlend)
#include <def-undef.h>
!     =================
...
!LPF 20120815
      logical, intent(in)           :: rstwr           ! true => write restart file before returning
...
!     if ( mod(iday-1,rest_freq) == 0 .or. iday == 1) then !wangty bug iday => iday-1; imd => 1
!wangty modify
      if ( rstwr .or. nlend) then
!
       if ( mytid == 0 ) then
         fname(1:8)='fort.22.'
         fname(13:13)='-'
...
!         if ( number_day ==1 .and. mon0 < 12) then !wangty bug line:43 to 50
...
!         write(6,*) "in SSAVEcdf, month=",mon0,"day=",number_day
          ...
          call exch_boundary(ws(1,1,1),km)
