...
#=========================================================================
# create pop2_in by echoing semi-resolved namelists into LICOM_IN 
#=========================================================================
...
if      ( ${OCN_GRID} =~ gx3* ) then
  ...
  if ($LICOM_COUPLING  =~ *partial*) then
    setenv DT_COUNT 500
  else
    setenv DT_COUNT 300
  endif
...
#--------------------------------------------------------------------------
#  ice_nml
#   LICOM_ICE_FORCING is a CCSM environment variable; see $case/env_conf
#--------------------------------------------------------------------------
...
#    LICOM_COUPLING is a CCSM environment variable; see $case/env_conf
#--------------------------------------------------------------------------
...
if ( ${OCN_GRID} =~ gx3* ) then
 set qsw_distrb_opt = cosz
  ...
  if ($LICOM_COUPLING  =~ *partial*) then
    set qsw_distrb_opt = cosz
  else if ($LICOM_COUPLING  =~ *full*) then
    set qsw_distrb_opt = cosz
  endif
else if ( ${OCN_GRID} == tx1v1 ) then
...
#--------------------------------------------------------------------------
#  sw_absorption_nml
#   LICOM_CHL_TYPE is a CCSM environment variable; see $case/env_conf
#--------------------------------------------------------------------------

if ($LICOM_CHL_TYPE == prognostic) then
 setenv CHL_OPTION model
else if ($LICOM_CHL_TYPE == diagnostic) then
