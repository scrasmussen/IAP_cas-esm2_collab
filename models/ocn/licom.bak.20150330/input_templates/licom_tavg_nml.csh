...
#=========================================================================
# Purpose: create tavg_nml for inclusion in the file licom_in
...
#----------------------------------------------------------------------------
# parse the base-model tavg_nml, located in $LICOM_TAVG_NML_BASE, and 
...
set ltavg_temp  = `grep "^ *ltavg_ignore_extra_streams"   $LICOM_TAVG_NML_BASE | awk -F= '{print $2}' `

if ($ltavg_temp == .true. || $ltavg_temp == .false.) then
set ltavg_ignore_extra_streams  = $ltavg_temp
endif

set ltavg_streams_index_present = `grep "^ *ltavg_streams_index_present" $LICOM_TAVG_NML_BASE | awk -F= '{print $2}' `

set ltavg_nino_diags_requested  = `grep "^ *ltavg_nino_diags_requested"  $LICOM_TAVG_NML_BASE | awk -F= '{print $2}' `

set tavg_contents_filename      = `grep "^ *tavg_contents"  $LICOM_TAVG_NML_BASE | awk -F= '{print $2}' `
set tavg_infile                 = `grep "^ *tavg_infile"    $LICOM_TAVG_NML_BASE | awk -F= '{print $2}' `
set tavg_outfile                = `grep "^ *tavg_outfile"   $LICOM_TAVG_NML_BASE | awk -F= '{print $2}' `
...
foreach VAR ($VARS)
  set opts = `grep "^ *$VAR " $LICOM_TAVG_NML_BASE | awk -F= '{print $2}' `
  if ($VAR == tavg_freq_opt) then
    ...
    echo licom_in_build.csh failure in $module loop 
...
foreach module ($ocn_tracers)
    if (-f ${MY_PATH}/ocn.${module}.setup.csh) then
       ...
       echo error in licom_in_build.csh unknown tracer: $module
       exit -3
    endif
...
#------------------------------------------------------------------------------------
# parse the information in the $LICOM_DOCDIR/$module.tavg file (created by the setup.csh scripts)
#------------------------------------------------------------------------------------
    foreach VAR ($VARS)
      set opts = `grep "^ *$VAR " $LICOM_DOCDIR/$module.tavg | awk -F= '{print $2}' `
      if ($VAR == tavg_freq_opt) then
        set tavg_freq_opt_values = ($tavg_freq_opt_values  $opts)
      else if ($VAR == tavg_freq) then
        set tavg_freq_values = ($tavg_freq_values  $opts)
    ...
    # see section below, after the complete specification of licom_in
    #------------------------------------------------------------------------------------
   set tracer_stream_numbers = ($tracer_stream_numbers $module_stream)
   #---------
   # clean up
   #---------
   ### NEVER rm $LICOM_DOCDIR/$module.tavg
end # loop over tracer setup scripts

#---------
# clean up
#---------
### NEVER rm $LICOM_TAVG_NML_BASE
cat >&! $LICOM_TAVG_NML << EOF2
