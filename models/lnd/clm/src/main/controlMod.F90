...
! !USES:
  ...
  use clm_varctl   , only : caseid, ctitle, nsrest, brnch_retain_casename, hostname, model_version=>version,    &
                            ...
                            username, fsnowaging, fsnowoptics, fglcmask, dust_emis
...
! Namelist variables only used locally
  ...
  character(len= 16) :: soil_erod = 'soil_erod'   ! full pathname for soil erodibility dataset
  integer :: seasalt_modes = 0
!EOP
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
    ...
    !!! added by fkc
    call mpi_bcast (dust_emis,len(dust_emis),MPI_CHARACTER, 0, mpicom, ier )
    !!!
#ifdef CN
...
!!! added by fkc
  subroutine dust_readnl()
     use shr_file_mod, only: shr_file_getUnit, shr_file_freeUnit

!     character(SHR_KIND_CL), intent(out) :: dust_emis_in
     character(SHR_KIND_CL) :: subname = 'dust_readnl'
     character(SHR_KIND_CL) :: atm_nlfile = 'atm_in'
     integer :: unitn, ierr, iu
     
     namelist /aerosol_nl/ dust_emis_fact, soil_erod, dust_emis, seasalt_modes

   if (masterproc) then
      unitn = shr_file_getUnit()
      open( unitn, file=trim(atm_nlfile), status='old' )
