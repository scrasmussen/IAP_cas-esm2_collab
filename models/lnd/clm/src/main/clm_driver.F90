...
! !USES:
  ...
  use clm_varctl          , only : iulog, dust_emis
...
#endif
  use DUSTMod             , only : DustDryDep, DustEmission_Zender03, DustEmission_Kok14
...
!$OMP PARALLEL DO PRIVATE (nc,l,c,begg,endg,begl,endl,begc,endc,begp,endp)
  do nc = 1,nclumps
     ...
     if (dust_emis == 'zender03') then
         call DustEmission_Zender03(begp, endp, begc, endc, begl, endl, &
                                    filter(nc)%num_nolakep, filter(nc)%nolakep)
     else if (dust_emis == 'kok14') then
         call DustEmission_Kok14(begp, endp, begc, endc, begl, endl, &
                                    filter(nc)%num_nolakep, filter(nc)%nolakep)
     else
         write(iulog,*) 'dust_emis type not supported, currently only zender03 and kok14 are available'
         call endrun
     end if
