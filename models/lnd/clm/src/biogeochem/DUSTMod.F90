...
!
  ...
  public DustEmission_Zender03   ! Dust mobilization Zender03
  public DustEmission_Kok14      ! Dust mobilization Kok14
...
!
! !INTERFACE:
  subroutine DustEmission_Zender03 (lbp, ubp, lbc,ubc,lbl,ubl,num_nolakep, filter_nolakep)
!
! !DESCRIPTION: 
...
! initialize variables which get passed to the atmosphere
  ...
  end subroutine DustEmission_Zender03

  subroutine DustEmission_Kok14 (lbp, ubp, lbc,ubc,lbl,ubl,num_nolakep, filter_nolakep)
!!!! modified by LiuXiaoHong's Group, using Kok14 dust emission scheme, added by
!    fkc
!
! !DESCRIPTION: 
! Dust mobilization. This code simulates dust mobilization due to wind
...
!    real(r8) :: flx_mss_hrz_slt_ttl  !variable no longer needed, -jfk
    real(r8) :: flx_mss_vrt_dst_ttl(lbp:ubp)
    real(r8),pointer :: frc_thr_wet_fct(:)  ! the impact of soil moisture on dust emission
    real(r8) :: frc_thr_rgh_fct
    real(r8) :: wnd_frc_thr_slt
    real(r8) :: wnd_rfr_thr_slt
    ...
    real(r8) :: wnd_frc_slt
    real(r8),pointer :: lnd_frc_mbl(:) ! bare land fraction 
    real(r8) :: bd
    real(r8) :: gwc_sfc
    real(r8) :: ttlai(lbp:ubp)
    real(r8) :: tlai_lu(lbl:ubl)
...
!    real(r8), parameter :: cst_slt = 2.61_r8           ! [frc] Saltation constant !variable no longer needed, -jfk
    ...
    logical  :: found                       ! temporary for error check
    integer  :: index
    real(r8), pointer :: land_frac(:)       ! land fraction
!------------------------------------------------------------------------
    ...
    mbl_bsn_fct     => clm3%g%l%c%cps%mbl_bsn_fct
    mss_frc_cly_vld => clm3%g%l%c%cps%mss_frc_cly_vld
!    mss_frc_cly_vld => clm3%g%l%c%p%pdf%mss_frc_cly_vld1
    h2osoi_vol      => clm3%g%l%c%cws%h2osoi_vol
    h2osoi_liq      => clm3%g%l%c%cws%h2osoi_liq
    h2osoi_ice      => clm3%g%l%c%cws%h2osoi_ice
    watsat          => clm3%g%l%c%cps%watsat
    ...
    flx_mss_vrt_dst => clm3%g%l%c%p%pdf%flx_mss_vrt_dst
    flx_mss_vrt_dst_tot => clm3%g%l%c%p%pdf%flx_mss_vrt_dst_tot
    Cd1             => clm3%g%l%c%p%pdf%Cd
    lnd_frc_mbl     => clm3%g%l%c%p%pdf%lnd_frc_mbl
    frc_thr_wet_fct => clm3%g%l%c%p%pdf%frc_thr_wet_fct
   !land fraction
    land_frac       => ldomain%frac
   !local pointers from subgridAveMod/p2l_1d
    wtlunit         => clm3%g%l%c%p%wtlunit
...
! initialize variables which get passed to the atmosphere
    ...
    do fp = 1,num_nolakep
       ...
       if ((ityplun(l) == istsoil .or. ityplun(l) == istcrop).and.(land_frac(g).gt.0.3_r8)) then
          if (tlai_lu(l) < vai_mbl_thr) then
             lnd_frc_mbl(p) = 1.0_r8 - (tlai_lu(l))/vai_mbl_thr
          else
             lnd_frc_mbl(p) = 0.0_r8
    ...
    do fp = 1,num_nolakep
       ...
       if (lnd_frc_mbl(p) > 0.0_r8) then
          ...
          gwc_sfc = h2osoi_vol(c,1)*SHR_CONST_RHOFW/bd    ![kg kg-1] Gravimetric H2O cont
          if (gwc_sfc > gwc_thr(c)) then
             frc_thr_wet_fct(p) = sqrt(1.0_r8 + 1.21_r8 * (100.0_r8*(gwc_sfc - gwc_thr(c)))**0.68_r8)
          else
             frc_thr_wet_fct(p) = 1.0_r8
          end if
          ...
          wnd_frc_thr_slt = tmp1 / sqrt(forc_rho(g)) * frc_thr_wet_fct(p) * frc_thr_rgh_fct
  ...
  end subroutine DustEmission_Kok14
