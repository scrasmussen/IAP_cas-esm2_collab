...
#endif
#if(defined SPINUP)
 use colm_varctl, only : forcing_begyr, forcing_endyr, last_nstep, curr_nstep
#endif
#ifdef AUTOMASK
...
! -----------------------------------------------------------------
  ...
  real(r8) :: Ksf(maxpft)
  real(r8) :: Kss(maxpft)
  real(r8), pointer :: Isf_ave(:)
  real(r8), pointer :: Iss_ave(:)
  real(r8), pointer :: Ksf_ave(:)
  real(r8), pointer :: Kss_ave(:)
  logical  :: pool_tuning = .false.
...
#if(defined SPINUP)
      if(idate(1).lt.idate_p(1)) then
         curr_nstep = get_nstep()
         pool_tuning = .true.
         if(p_master) write(6,*) 'pool tuning'
      end if
#endif
...
! Yiqi Luo's spinup
!
         Isf_ave   => Isf_pft(p1:p2)
         Iss_ave   => Iss_pft(p1:p2)
         Ksf_ave   => Ksf_pft(p1:p2)
         Kss_ave   => Kss_pft(p1:p2)
#endif
...
! ======================================================================
       ...
       ! additional variables required by coupling with regional model (WRF & RSM) 
         ...
         if(itypwat_c==0) then
            ...
            if(pool_tuning) then
               Isf_ave = Isf_ave/(curr_nstep-last_nstep+1)
               Iss_ave = Iss_ave/(curr_nstep-last_nstep+1)
               Ksf_ave = Ksf_ave/(curr_nstep-last_nstep+1)
               Kss_ave = Kss_ave/(curr_nstep-last_nstep+1)

             ! print *, 'Isf_ave:', sum(Isf_ave)
             ! print *, 'Iss_ave:', sum(Iss_ave)
             ! print *, 'Ksf_ave:', sum(Ksf_ave)
             ! print *, 'Kss_ave:', sum(Kss_ave)

             ! print *, 'cpool_fast', sum(cpool_fast)
             ! print *, 'cpool_slow', sum(cpool_slow)

               where(Ksf_ave.ne.0) cpool_fast = Isf_ave/Ksf_ave
               where(Kss_ave.ne.0) cpool_slow = Iss_ave/Kss_ave

             ! do p = 1, n_pft
             !    if(isnan(cpool_fast(p)) .or. isnan(cpool_slow(p))) then
             !       write(6,*) 'pool nan', cpool_fast(p), cpool_slow(p), & 
             !                  Isf_ave(p), Iss_ave(p), Ksf_ave(p), Kss_ave(p)
             !    end if
             ! end do
