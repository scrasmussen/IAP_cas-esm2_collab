...
#include <define.h>
 ...
 SUBROUTINE SVATTHERMAL (itypwat  ,lb       ,nl_soil  ,num_filterp,filterp,&
...
! 
! FLOW DIAGRAM FOR SVATTHERMAL.F90
! 
! THERMAL ===> qsadv
!              groundfluxes
!              eroot                      |dewfraction
...
!---------------------Argument------------------------------------------
  ...
  real(r8), INTENT(in) ::  &
        ...
        phi0(1:nl_soil)   ,&! soil water suction, negative potential [mm]
  ...
  real(r8), INTENT(out) :: &
        ...
        fevpg(n_pft),      &! evaporation from ground [mm/s]
...
!=======================================================================
      ...
      ! emissivity
    ! emg = 0.96                                ! - jidy@13-Jan-2012
    ! if(scv>0. .OR. itypwat==3) emg = 0.97     ! + jidy@13-Jan-2012
    ! if(scv>1.E-6 .OR. itypwat==3) emg = 0.97  ! + jidy@13-Jan-2012
      if(itypwat==3) then
         emg = 0.97                             ! + jidy@07-Dec-2014
      else
         emg = (1.-fsno)*0.96 + fsno*0.97       ! + jidy@07-Dec-2014
      end if 
    ...
    ! if(wliq(lb)<=0. .AND. wice(lb)>0.) htvp = hsub
      if(wliq(lb)<1.E-6 .AND. wice(lb)>1.E-6) htvp = hsub
    !<<jidy@2012/01/12
