...
#endif
!#undef FHNP!  use timemgr     , only : get_curr_date
!==============wangy=============1
!
! !PUBLIC TYPES:
  implicit none
...
!==============wangy=============0 
  ...
  real(r8), allocatable, public :: sltvolr(:,:)     ! RTM fluxes - solute storage in cell (kg)  
...
! ==============wangy=============0
  ...
  real(r8), allocatable :: totsoluin_r(:,:)  ! RTM input - for CNP transport
  real(r8), allocatable :: sltin(:,:)        ! RTM input - solute flux into cell (kg/s)
  real(r8), allocatable :: sltout(:,:)       ! RTM input/output - solute flux out of cell (kg/s)
  real(r8), allocatable :: frac(:,:)         ! Fraction denitrified
  real(r8), allocatable :: solute_r(:,:)     ! RTM output - CNP flow (kg/s) 
...
! ==============wangy=============0
    ...
    allocate (sltvolr(rtmloni:rtmlonf,rtmlati:rtmlatf),&
              sltout(rtmloni-1:rtmlonf+1,rtmlati-1:rtmlatf+1), stat=ier)
    if (ier /= 0) then
       write(6,*)'Rtmgridini: Allocation error for sltout'
       call endrun
    end if
#endif
...
! ==============wangy=============1
    ...
    allocate (totrunin_r(rtmloni:rtmlonf,rtmlati:rtmlatf), &
              ...
              sfluxin(rtmloni:rtmlonf,rtmlati:rtmlatf), &
              stat=ier)    
    if (ier /= 0) then
       write(6,*)'Rtmgridini: Allocation error for ',&
...
! ==============wangy=============0
     ...
     allocate (solute_r(rtmloni:rtmlonf,rtmlati:rtmlatf), &         
              totsoluin_r(rtmloni:rtmlonf,rtmlati:rtmlatf), &        
              sltin(rtmloni:rtmlonf,rtmlati:rtmlatf), &            
              frac(rtmloni:rtmlonf,rtmlati:rtmlatf),  stat=ier)    
    if (ier /= 0) then
       write(6,*)'Rtmgridini: Allocation error for ',&
            'solute_r, totsoluin_r, sltin, frac'      
       call endrun
    end if
#endif
...
! Open and read input data (N pollution file)
   curr_yr = get_curr_year()
   call solute_flux(curr_yr, totsoluin_r)
...
!==============wangy=============0
        ...
        call UpdateRunoff(rtmloni, rtmlonf, rtmlati, rtmlatf, flxocn_r,flxlnd_r, solute_r,sltvolr)
...
!==============wangy=============0
    ...
    real(r8) :: buffernn(rtmlon)            !temp buffer       
    real(r8) :: bufferss(rtmlon)            !temp buffer
    real(r8) :: sltrdt                      !change in storage in N FLUX units(kg/s)      
!==============wangy=============1
#endif
...
! Determine sltout at extended points and at southern and northern outer lats

    sltout(rtmlon+1,rtmlat+1) = sltout(1,1)
    sltout(rtmlon+1,0)        = sltout(1,rtmlat)
    sltout(0,0)               = sltout(rtmlon,rtmlat)
    sltout(0,rtmlat+1)        = sltout(rtmlon,1)

    do i=1,rtmlon
       sltout(i,0)         = sltout(i,rtmlat)
       sltout(i,rtmlat+1)  = sltout(i,1)
       buffernn(i)         = sltout(i,rtmlat)
       bufferss(i)         = sltout(i,1)
    enddo
    do j=1,rtmlat
       sltout(0,j)        = sltout(rtmlon,j)
       sltout(rtmlon+1,j) = sltout(1,j)
    enddo
    do i=0,rtmlon+1
       sltout(i,0)        = buffernn(mod(i+rtmlon/2-1,rtmlon)+1)
       sltout(i,rtmlat+1) = bufferss(mod(i+rtmlon/2-1,rtmlon)+1)
    enddo
#endif
...
! ============wangy============0
     ...
     do j=rtmlati,rtmlatf
       do i=rtmloni,rtmlonf
          sltin(i,j) = 0.         
          if (rdirc(i  ,j-1)==1) sltin(i,j) = sltin(i,j) + sltout(i  ,j-1)
          if (rdirc(i-1,j-1)==2) sltin(i,j) = sltin(i,j) + sltout(i-1,j-1)
          if (rdirc(i-1,j  )==3) sltin(i,j) = sltin(i,j) + sltout(i-1,j  ) 
          if (rdirc(i-1,j+1)==4) sltin(i,j) = sltin(i,j) + sltout(i-1,j+1)
          if (rdirc(i  ,j+1)==5) sltin(i,j) = sltin(i,j) + sltout(i  ,j+1)
          if (rdirc(i+1,j+1)==6) sltin(i,j) = sltin(i,j) + sltout(i+1,j+1) 
          if (rdirc(i+1,j  )==7) sltin(i,j) = sltin(i,j) + sltout(i+1,j  ) 
          if (rdirc(i+1,j-1)==8) sltin(i,j) = sltin(i,j) + sltout(i+1,j-1) 
         enddo
     enddo
#endif
...
! ============wangy============0
          ...
          sltrdt = sltin(i,j) - sltout(i,j) + 0.001*totsoluin_r(i,j)*rivarea(i,j)  
...
! ============wangy============0
           ...
           frac(i,j) = 1-exp(-1.*(35./(24*3600*365))*(sqrt(rivarea(i,j)))/effvel)/(0.27*fluxout(i,j)**0.29)
           if (isnan(frac(i,j))) then
                frac(i,j)=0.

            sltvolr(i,j) = sltvolr(i,j) + sltrdt*delt_rtm        
            sltout(i,j)  = sltvolr(i,j) * effvel/ddist(i,j)            
            sltout(i,j)  = min(sltout(i,j),sltvolr(i,j) / delt_rtm) 
            sltout(i,j)  = sltout(i,j)*(1-frac(i,j))
            solute_r(i,j) = sltout(i,j)   
           end if
...
! ===========wangy===========0
       ...
       call mpi_bcast(runoff%solute, size(runoff%solute), MPI_REAL8  , 0, mpicom, ier)    
       call mpi_bcast(sltout,  size(sltout),  MPI_REAL8  , 0, mpicom, ier)                
       call mpi_bcast(sltvolr, size(sltvolr), MPI_REAL8  , 0, mpicom, ier) 
