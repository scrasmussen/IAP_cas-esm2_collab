...
!
  use precision
  use phycon_module, only : tfrz, denh2o, cpliq, cpice, hfus
  implicit none
! ------------------------ Dummy Argument ------------------------------
...
! -----------------------------------------------------------------
  use precision
  use phycon_module, only : tfrz,hvap,hfus,hsub,tkwat,tkice,tkair,stefnc,&
...
! Evaluated stability-dependent variables using moz from prior iteration
         displax = 0.
         call moninobuk_lake(forc_hgt_u,forc_hgt_t,forc_hgt_q,displax,z0mg,z0hg,z0qg,obu,um,&
...
!------------------------------------------------------------
      ...
      call hConductivity_lakesoil (itypwat,lb,nl_soil,&
                          dkdry,dksatu,porsl,&
...
! 10 m wind
      u10m = forc_us/max(0.1,ur) * ustar/vonkar * fm10m
      v10m = forc_vs/max(0.1,ur) * ustar/vonkar * fm10m
...
!-----------------------------------------------------------------------------------------------
  ...
  use precision
  use phycon_module, only : denh2o, denice, hfus, tfrz, cpliq, cpice
...
! ------------- in/inout/out variables -----------------------------------------
  ...
  logical,  INTENT(in)    :: do_capsnow
  real(r8), INTENT(inout) :: qflx_snwcp_liq
  real(r8), INTENT(inout) :: qflx_snwcp_ice
...
!-----------------------------------------------------------------------
  ...
  subroutine roughness_lake (snl,t_grnd,t_lake,lake_icefrac,forc_psrf,&
                             cur,ustar,z0mg,z0hg,z0qg)
  use precision
  use phycon_module, only : tfrz,vonkar,grav
...
! -------------------------------------------------------------------------
  ...
  use precision
  use phycon_module, only : tfrz,tkwat,tkice,tkair,&
