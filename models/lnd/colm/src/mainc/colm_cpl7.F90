   ...
   subroutine build_clm_l2a(init)
      ...
      if (present(init)) then
         ...
         !  clm_l2a%flxdst(g,:)       = 0._r8
            clm_l2a%flxdst(g,1)       = lnd_dustemis_bin_1(i)
            clm_l2a%flxdst(g,2)       = lnd_dustemis_bin_2(i)
            clm_l2a%flxdst(g,3)       = lnd_dustemis_bin_3(i)
            clm_l2a%flxdst(g,4)       = lnd_dustemis_bin_4(i)   
#else
            clm_l2a%flxdst(g,:)       = 0._r8
#endif
...
! read ig data IAPDGVM
#ifdef IAPDGVM
      CALL readigdata
#endif


      CALL colm_subgridvar_free

    ! Initialize land use module
      CALL landuse_init
#if (defined FHNP) && (defined GWUSE)
!--------------------------------------------------------------
!Read Grounwater Exploitation
!Note that GWUSE is still in the testing phase, don't define it. 
!--------------------------------------------------------------
    !   wanglh 2018/11
    if(p_master)then
             write(6,*) ' CoLM read gw data '
    endif
...
!--------------------------------------------------------------
    !   wanglh 2018/11
    if(p_master)then
             write(6,*) ' CoLM read cgw data '

        !    write(6,*) 'Nextstep=', nextsw_cday 
    ...
    call get_cgwdata
       ...
       ! Surface states sent to the flux coupler states are not time averaged
         ...
         CALL writehistdata(rstwr,nlend,rdate)
...
! ======================================================================
   ...
   subroutine finalize()
      ...
      CALL colm_gridvar_free
      CALL colm_pcgvar_free
