...
!=======================================================================
  ...
  use spmd_decomp  !wanglh
  use colm_varMod, only :fldv,fldv_col,fldv_pft
...
!                   
  ...
  real(r8), parameter :: fff    = 5.      ! runoff decay factor (m-1) !niu
  real(r8), parameter :: aaa    = 3.      ! adjustable for fice !niu

  real(r8) :: & 
      icefrac            , &! ice fraction (-) !niu
      fice(1:nl_soil)       ! impermeable area due to ice (-) (see Niu&Yang (2006)) !niu
#if (defined FHNP) && (defined GWUSE)
...
!Note that GWUSE is still in the testing phase, don't define it.
      !qseva_col=qseva_col+0.7_r8*noirr_frac(g_index)*cgwintake(c_index)!wanglh 2018/11
      do fp = 1, num_filterp
         p = filterp(fp)
         fevpg_p(p)=fevpg_p(p)+0.7_r8*noirr_frac(g_index)*cgwintake(c_index)
      end do
...
#if (defined FHNP) && (defined GWUSE)
!Note that GWUSE is still in the testing phase, don't define it.
      gwat=gwat+(1._r8-noirr_frac(g_index))*cgwintake(c_index)!wanglh 2018/11
#endif
      if (lb>=1)then
         gwat = pg_rain_col + sm - qseva_col
      else
         call snowwater (lb,dtime,ssi,wimp,&
                         pg_rain_col,qseva_col,qsdew_col,qsubl_col,qfros_col,&
...
!=======================================================================
  ...
  if(itypwat<=1)then   ! soil ground only
      ...
      do i = 1, nl_soil
         ...
         if (porsl(i) > 0.) then                                   !jidy
            icefrac = min(1.,wice(i)/porsl(i))                     !niu
         else
            icefrac = 0.                                           !jidy
         end if
         fice(i)    = max(0.,exp(-aaa*(1.-icefrac))-exp(-aaa)/ &
                            (1.0                   -exp(-aaa)) )   !niu
      enddo
      ...
      call surfacerunoff (nl_soil,wtfact,wimp,bsw,porsl,phi0,hksati,&
           z(1:),dz(1:),zi(0:),vol_liq,vol_ice,eff_porosity,gwat,rsur,fff,fice) !niu
      else
           rsur = 0.
      endif
#if (defined FHNP) && (defined GWUSE)
      ...
      !Note that GWUSE is still in the testing phase, don't define it.
      rsur=rsur+0.3_r8*noirr_frac(g_index)*cgwintake(c_index) !wanglh 2018/11
...
!print*,'rootr_sum',sum(rootr_col)
      ...
      call soilwater (nl_soil,dtime,wimp,smpmin,porsl,phi0,bsw,hksati, &
                      ...
                      qinfl,etr_col,rootr_col,dwat,hk,dhkdw,fice) !niu
...
!=======================================================================
      ...
      if(lb<1 .and. do_capsnow) then
         rnof = rnof + qflx_snwcp_liq + qflx_snwcp_ice
         rsur = rsur + qflx_snwcp_liq + qflx_snwcp_ice
      end if
