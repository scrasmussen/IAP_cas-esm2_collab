...
subroutine cal_Tmomin(msec,tref,t_mo,t_mo_sum,t_mo_min,nday)  !added by zhq. 08.11
 ...
 use timemgr, only : dtime, is_newmonth, get_ndays_of_month
...
!if((msec.eq.0).and.(mod(nday,30.).eq.0)) then
 if(is_newmonth()) then
  ...
  ! t_mo = t_mo_sum/30./(86400/dtime)
    t_mo = t_mo_sum/get_ndays_of_month(laststep=.true.)/(86400/dtime)
    t_mo_sum = 0.
...
subroutine cal_wf(nl_soil,porsl,wliq,wice,z,dz,wf25,wf)
...
!intent inout variables:
 real(r8), INTENT(out) :: wf25        ! soil water as frac. of whc for top 0.5 m
 real(r8), INTENT(out) :: wf(nl_soil) ! soil water as frac. of whc
...
!fraction of liquid water/soil por. volume to a depth of 0.5 m.
 ...
 if(porslsum .gt. 0.)then
    wf25 = min(wliqsum/porslsum,1.0)
 else
    wf25 = 0.
 end if

 do j = 1,nl_soil
    porslsum = porsl(j)*dz(j)
    wliqsum  = wliq(j)/denh2o + wice(j)/denice  ! ??? only liquid ???

    if(porslsum .gt. 0.)then
       wf(j) = min(wliqsum/porslsum,1.0)
    else
       wf(j) = 0.
    end if
 end do
