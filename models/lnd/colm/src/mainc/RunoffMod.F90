...
!
  type runoff_flow
!===========wangy for CNP SOLUTE===========0
#if (defined FHNP) && (defined NP)
     real(r8), pointer :: solute(:)      ! CNP flow (kg/s) 
     real(r8), pointer :: sltvolr(:)     ! RTM solute storage (kg)
#endif
!===========wangy for CNP==================1
...
!-----------------------------------------------------------------------
    ...
    allocate(runoff%lnd(nsize), runoff%lnd_ixy(nsize), runoff%lnd_jxy(nsize), runoff%lnd_area(nsize), &
             stat=ier)
    if (ier /= 0) then
...
! =============wangy==============0
#if (defined FHNP) && (defined NP)
    allocate(runoff%solute(nsize), runoff%sltvolr(nsize), stat=ier)
    if (ier /= 0) then
        write(6,*)'set_roflnd: allocation error for runoff components :solute, sltvolr'
       call endrun
    end if
...
! ===============wangy=============0
#if (defined FHNP) && (defined NP)
 subroutine UpdateRunoff(nrloni, nrlonf, nrlati, nrlatf, flxocn_r, flxlnd_r,solute_r,sltvolr)
#else
...
! ==============wangy================0
#if (defined FHNP) && (defined NP)
    real(r8), intent(in) :: solute_r(nrloni:nrlonf, nrlati:nrlatf)     
    real(r8), intent(in) :: sltvolr(nrloni:nrlonf, nrlati:nrlatf)
#endif
! ==============wangy================1
! !REVISION HISTORY:
! Mariana Vertenstein: Created 10/2003
...
! ==============wangy================0
#if (defined FHNP) && (defined NP)
     do n = 1,size(runoff%solute)
       i = runoff%lnd_ixy(n)
       j = runoff%lnd_jxy(n)
       runoff%solute(n) = solute_r(i,j)
    end do

     do n = 1,size(runoff%sltvolr)
       i = runoff%lnd_ixy(n)
       j = runoff%lnd_jxy(n)
       runoff%sltvolr(n) = sltvolr(i,j)
    end do
#endif
! ==============wangy=================1
