 ...
 subroutine surfacerunoff (nl_soil,wtfact,wimp,bsw,porsl,phi0,hksati,&
                           z,dz,zi,vol_liq,vol_ice,eff_porosity,gwat,rsur,fff,fice)
...
! Saturation fraction
    ...
    ! if(eff_porosity(1)>=wimp) fcov = wtfact*min(1.,exp(-zwt))
      ...
      fcov =      fice(1)  + &                                        !niu
             (1.0-fice(1)) * wtfact*min(1.0,exp(-0.5*fff*(zwt-2.0)))  !niu
            !(1.0-fice(1)) * wtfact*min(1.0,exp(-0.5*fff*(zwt-zi(nl_soil)))) !!niu
...
! Surface runoff
      rsur = fcov*max(0.,gwat) + (1.-fcov)*max(0.,gwat-qinmax)
