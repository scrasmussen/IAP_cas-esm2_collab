#include <define.h>
  ...
  subroutine LPJ(nyr_r       ,npatch        ,npftpar       ,pftpar        ,&
...
! LOCAL VARIABLES:
   ...
   EXTERNAL Kill
    ...
    call Fire(npatch, afirefrac(1:), acfluxfire(1:),&
...
!=================================================================================
  ...
  subroutine Fire(npatch, afire_frac, acflux_fire, &
...
! INTENT IN VARIABLES: 
    ...
    real(r8), INTENT(in) :: firelength(npatch)    ! fire season in days
    real(r8), INTENT(in) :: vegclass(npatch)      ! 1.tree 2.shrub 3.grass 4.crop
    real(r8), INTENT(in) :: resist(npatch)        ! fire resistance index 
...
!       end if   ! if soil
    ...
    do p = 1, numpft
       if (ifpre(p) .gt. 0.) then
          if (vegclass(p) .le. 2.) then
             ...
             if (grid_estab(ivt(p))) nwoody = nwoody + 1
    ...
    do p = 1, npatch
       ...
       if (grid_prec365 >= prec_min_estab .and. nwoody > 0) then
          ...
          if (fpc_tree_total < 1.0) then    ! new test at Aug 5, 2008, 2:43pm, try to avoid negetive nind
            ...
            estab_rate(p) = estab_max * (1.0-exp(5.0*(fpc_tree_total-1.0)))/real(nwoody)
