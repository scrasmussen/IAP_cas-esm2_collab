...
module landuse
   ...
   use netcdf
   use spmd
   use colm_varctl, only: luc_emission
   ...
   character(len=255) :: fluc_emission = "null"
   ...
   public fluc_emission, carbon_emission
...
contains
   ...
   subroutine landuse_init
      ...
      if (.not. luc_emission) return

      if (p_master) then
         print *, 'open land use carbon emission file: '//trim(fluc_emission)

         call sanity(nf90_open(path=trim(fluc_emission),mode=nf90_nowrite,ncid=fid))
