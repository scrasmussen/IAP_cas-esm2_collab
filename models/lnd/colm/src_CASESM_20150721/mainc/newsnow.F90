 ...
 subroutine newsnow (itypwat,maxsnl,n_pft,num_filterp,filterp,dtime,&
                     wt_patch,wt_column,tm,tg,pg,rain,snow,tcrit,   &
                     zi,z,dz,tss,wliq,wice,fiold,snl,sag,scv,snowdp,&
                     pg_rain,pg_snow,do_capsnow,qflx_snwcp_ice)
...
!
  ...
  use spmd, only: p_iam, p_master
  use colm_varctl, only: lsnowfrac
...
! vary linearly with air temp, from 0% at 273.16 to 40% max at 275.16.
  ...
  ! set an special value to crash model quickly whilst error occurred
    dz_snowf = -9999.
    newnode = 0
...
! Progress Rep. 1, Alta Avalanche Study Center:Snow Layer Densification.
  ...
  ! if(tm<=tfrz+tcrit)then         ! 17/Jan/2015@jidy
       if(tm>tfrz+2.)then
          bifall=169.
       else if(tm>tfrz-15.)then
          bifall=50.+1.7*(tm-tfrz+15.)**1.5
