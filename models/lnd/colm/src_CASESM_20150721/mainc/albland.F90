  ...
  subroutine albland (itypwat,albsat,albdry,&
                      chil,refl,refs,tranl,trans,fveg,green,lai,sai,coszen,&
...
! ground cover index
 ...
 real(r8), INTENT(in) :: & 
      ...
      refl(2,2), &! leaf reflectance (iw=iband, il=life and dead)
      refs(2,2), &! stem reflectance (iw=iband, il=life and dead)
      tranl(2,2),&! leaf transmittance (iw=iband, il=life and dead)
      trans(2,2),&! stem transmittance (iw=iband, il=life and dead)
...
! set default soil and vegetation albedos and solar absorption
   ...
   !* Diffuse shortwave radiation (solsd+solld) could exist even during coszen<=0. jidy@13/Aug/2014
   !* What to do with this condition? For a coarse temporal resolution forcing,
   !* it's not easy by checking if (sols+soll+solsd+solld)>.0 satisfied?
   !* For a coarse temporal resolution forcing, it's better to just use coszen<=0. as condition.
   !* if(sols+soll+solsd+solld<=0. and. coszen<=0.) RETURN
...
! 3.1 bare soil albedos, depends on moisture
      if(itypwat<=1)then    ! not wetland, permanent ice and water
       ...
       ! Using NCAR soil color 20 categories, jidy@17/Jun/2013
       ! alwet = min(alwet,albsat)
       ! albg0 = albsat+alwet
         albg0 = min(albsat(1)+alwet,albdry(1))
         albg1 = min(albsat(2)+alwet,albdry(2))
         albg(1,1) = albg0
       ! albg(2,1) = 2.*albg0          !old soil color 8 categories
         albg(2,1) = albg1
...
! ----------------------------------------------------------------------
      if(fveg>0.001 .and. lsai>0.)then
         call twostream (chil,refl,refs,tranl,trans,green,lai,sai,&
