 ...
 subroutine twostream ( chil, refl, refs, tranl, trans, green, lai, sai, &
...
! parameters
  real(r8), intent(in) :: &
          ! static parameters associated with vegetation type
            ...
            trans(2,2),    &! step transmittance (iw=iband, il=life and dead)
...
!-----------------------------------------------------------------------        
      ...
      rho(1) = ( refl(iw,1)*lai +  refs(iw,1)*sai)/lsai
      rho(2) = ( refl(iw,2)*lai +  refs(iw,2)*sai)/lsai
      tau(1) = (tranl(iw,1)*lai + trans(iw,1)*sai)/lsai
      tau(2) = (tranl(iw,2)*lai + trans(iw,2)*sai)/lsai
                                                                               
    ! scat = green * ( tran(iw,1) + ref(iw,1) ) + &
    !        ( 1. - green ) * ( tran(iw,2) + ref(iw,2) ) 
      scat = green * ( tau(1) + rho(1) ) + &
             ( 1. - green ) * ( tau(2) + rho(2) ) 
    ...
    ! upscat = green * tran(iw,1) + ( 1. - green ) * tran(iw,2)                           
      upscat = green * tau(1) + ( 1. - green ) * tau(2)                           
