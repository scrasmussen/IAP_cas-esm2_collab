            ...
            do i=1,imt
             do j=1,jmt
              if(vit(i,j,k)<0.5) then 
               ...
               local(i,j,k,m)=local(i,j,k,m)/float(totalstep)
              endif
             enddo
            enddo
           end do
...
!$OMP PARALLEL DO PRIVATE (I,J,M,k)
           do m=1,mm
           do k=1,kk
!wangty bug 1,jmt => 2,jmt-1; 1,imt => 2,imt-1
           do j=1,jmt
           do i=1,imt
!wangty bug
              global(i_global(i),j_global(j),k,m)=local(i,j,k,m)
           end do
           end do
           end do
           end do
...
use param_mod
use pconst_mod
use msg_mod, only: tag_1d,tag_2d,tag_3d,tag_4d,status,mpi_comm_ocn
