...
use pconst_mod
use isopyc_mod
use dyn_mod,only:viso,uiso,wiso
#ifdef SPMD
use msg_mod
#endif
...
!$OMP PARALLEL DO PRIVATE (j,i,k)
      ...
      END DO
      if (nx_proc ==1) then
!jjr!$OMP PARALLEL DO PRIVATE (j)
      DO j = jstrt,jmm
         CALL setbcx (adv_vntiso (1,1,j), imt, km)
      END DO
      end if
...
!$OMP PARALLEL DO PRIVATE (j)
      ...
      !jjb 07/16/18
       DO j = 1,jmt
          DO k = 1,km
             DO i = 2,imt
                uiso(i,j,k) =p5*(adv_vetiso(i-1,k,j)+adv_vetiso(i,k,j))*vit(i,j,k) 
            END DO
          END DO
       END DO
      ...
      DO j = 2,jmt
          DO k = 1,km
             DO i = 1,imt
                viso(i,j,k)=p5*(adv_vntiso(i,k,j-1)+adv_vntiso(i,k,j))*vit(i,j,k)
            END DO
          END DO
       END DO   
       ...
       DO j = 1,jmt
          DO k = 1,km
             DO i = 1,imt
                wiso(i,j,k)=adv_vbtiso(i,k,j)*vit(i,j,k)
            END DO
          END DO
       END DO

#ifdef SPMD
       call exch_boundary(uiso(1,1,1),km)
       call exch_boundary(viso(1,1,1),km)
       call exch_boundary(wiso(1,1,1),km)
#endif
      !jjb end 07/16/18
