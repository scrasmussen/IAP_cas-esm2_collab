...
!---------------------------- test zhh ------------------------
!!    if (masterproc) print*, 'At beginning of ------sub. get_phys_decomp_mdnd--------'
!---------------------------- test zhh ------------------------
...
!---------------------------- test zhh ------------------------
!!    if (masterproc) print*, 'sub. get_phys_decomp_mdnd -- 1'
!---------------------------- test zhh ------------------------
    ...
    dimcnt=1
!---------------------------- test zhh ------------------------
!!    if (masterproc) print*, 'sub. get_phys_decomp_mdnd -- 2'
!---------------------------- test zhh ------------------------
...
!---------------------------- test zhh ------------------------
!!    if (masterproc) print*, 'sub. get_phys_decomp_mdnd -- 3'
!---------------------------- test zhh ------------------------
    if(ldim>1) then
       dimcnt=dimcnt+1
       dimlens(dimcnt)=ldim
...
!---------------------------- test zhh ------------------------
!!    if (masterproc) print*, 'sub. get_phys_decomp_mdnd -- 4, fileorder =', fileorder
!!    if (masterproc) print*, 'hdim1_d =', hdim1_d, ' hdim2_d =', hdim2_d, ' fdim =', fdim
!!    if (masterproc) print*, 'mdimprod =', mdimprod, ' ldim =', ldim
!---------------------------- test zhh ------------------------
    if(.not.found) then
       if(oldcolumns) then
...
!---------------------------- test zhh ------------------------
!!          print*, 'iam =', iam, ' ldof(1) =', ldof(1), ' ldof(10) =', ldof(10)
!---------------------------- test zhh ------------------------
       end if
!---------------------------- test zhh ------------------------
!!       if (masterproc) print*, 'sub. get_phys_decomp_mdnd -- 5: before: pio_initdecomp'
!!       if (masterproc) print*, 'dimcnt =', dimcnt, ' dimlens(dimcnt) =', dimlens(dimcnt)
!!       if (masterproc) then
!!          if (dimcnt == 4) then
!!             print*, 'dimlens(1) =', dimlens(1), ' dimlens(2) =', dimlens(2)   
!!             print*, 'dimlens(3) =', dimlens(3), ' dimlens(4) =', dimlens(4)   
!!          end if
!!       end if
!!       if (masterproc) print*, 'dtype =', dtype
!---------------------------- test zhh ------------------------
       call pio_initdecomp(pio_subsystem, dtype,dimlens(1:dimcnt), ldof, iodesc)
!---------------------------- test zhh ------------------------
!!       if (masterproc) print*, 'sub. get_phys_decomp_mdnd -- 6: after: pio_initdecomp'
!---------------------------- test zhh ------------------------
       deallocate(ldof)
    end if
...
!---------------------------- test zhh ------------------------
!!    if (masterproc) print*, '======================= At function get_phys_ldof ======================='
!!    if (masterproc) print*, 'hdim1_d =', hdim1_d, 'hdim2_d =', hdim2_d
!!    if (masterproc) print*, 'fdim =', fdim, 'mdim =', mdim, 'ldim =', ldim
!!    if (masterproc) print*, 'ldof_dim =', pcols*(endchunk-begchunk+1)*fdim*mdim*ldim
!!    if (masterproc) print*, 'fileorder =', fileorder
!---------------------------- test zhh ------------------------
    ldof_dim = pcols*(endchunk-begchunk+1)*fdim*mdim*ldim
...
!---------------------------------- zhh test -----------------------------
!         fdim = 1, mdim = 30, ldim = 1, hdim1_d = 256, hdim2_d = 128 
!         do k = 1, nlev
!            do i = 1, pcols
!               lcnt = lcnt + 1
!               if(localhmask(i,lchnk)) then  
!                 ldof(lcnt) = (ilon(i)-1) + (k-1)*nlon + (ilat(i)-1)*nlon*nlev + 1
!---------------------------------- zhh test -----------------------------
          do f=1,fdim
             do k=1,mdim
                do i=1,pcols
...
!---------------------------------- zhh test -----------------------------
!!    do i = 1, ldof_dim
!!       if (ldof(i) > ldof_dim*4) print*, 'iam =', iam, ' i =', i, ' ldof(i) =', ldof(i)
!!       if (ldof(i) > 122870) print*, 'iam =', iam, ' i =', i, ' ldof(i) =', ldof(i)
!!       if (ldof(i) == 0) print*, 'iam =', iam, ' i =', i, ' ldof(i) =', ldof(i)
!!    end do
!!    do i = 600, 700
!!       if (iam == 1) print*, 'ldof(',i,') =', ldof(i)
!!    end do
    if (masterproc) print*, '====================================================================='
!---------------------------------- zhh test -----------------------------
