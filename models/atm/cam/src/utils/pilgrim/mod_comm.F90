...
!    2013.02.20   Zhang He  Set No. of tracers max_nq = PCNST 
! !USES:
#if defined( STAND_ALONE )
# define iulog 6
#else
...
!zhh,2013-02-19      integer, parameter:: max_nq = 1             ! No. of tracers simultaneously
      integer, parameter:: max_nq = PCNST         ! No. of tracers simultaneously
...
!--------------zhh debug 2013-02-19 -----------------
      print*, 'gid =', gid, ' idimsize =', idimsize, ' max_nq =', max_nq, ' tracbmax =', tracbmax
      print*, 'gid =', gid, ' idimsizz =', idimsizz, ' nbuf =', nbuf, ' max_call =', max_call
!--------------zhh debug 2013-02-19 -----------------
...
!!      if (masterproc) print*, '----------- at mp_recv3d: before Ga_Get4d_r8 ------------'
!
! Recv from src
      if ( src >= 0 .and. src < numcomm ) then     ! is PE in valid range?
        t1_win%src  = src
...
!
!!      if (masterproc) print*, '----------- at mp_recv3d: after Ga_Get4d_r8 ------------'
...
!zhh debug
!!      if (masterproc) print*, '----------- at Ga_Get4d_r8: before MPI_WAIT ------------'
!!      if (masterproc) print*, 'win%nread =', win%nread
      call MPI_WAIT(win%rqest(win%nread), Status, ierror)
!!      if (masterproc) print*, '----------- at Ga_Get4d_r8: after MPI_WAIT ------------'
!zhh debug
