...
!           Zhang He, 2018-01-29, fixed bugs
!
! Modified  Zhang Minghua 2018-08-20: 1) combined backward and forward integrations
!                                     2) changed Coriolis force calc to make it stable 
!                                     3) changed sub. name to tend_lin2
...
!------------------------------------------------------------------------------------------------
   ...
   use flexib,    only:  b1, b21, b22, c10, kappa, kappa2, IBCFFT, Kstd, &
                         DTlin, zmh_nlitti1, zmh_Coriolis              !zmh 2018.8.20 added flags
...
!
   ...
   integer  :: I_leap1, I_leap2			!leap points CH 181116
   ! logical  :: leap_flag = .true.		!!!For test of leap-cofficient CH 181106
   ! logical  :: filt_flag = .true.		!!!For test of FILTER CH 181106
...
!---------------------------------------------------------
            IF ( abs(DPS1).GT.0.001 ) THEN
               PRINT*, 'At sub. tend_lin2, IN THE INNER AREA: DP=',DPS1, 'i=',I, 'j=', J
...
!  bug fixed by Zhang He, 2018-01-19
               O2P = O2P + (Pstar2(IB,NY)-Pstar2(I,JE))*VT(I,K,JE) / Pstar1(IB,NY)    !(4.17)-(3)
            ENDDO
            O2P    = RDLATI * O2P
...
! !
			   ...
			   ! CH 181116 modified
!!======================leap-cofficient modified by CH 181116===============
			   DYP0  = Pstar2(I,J+1) - Pstar2(I,J)
               OYAYP = RDLATQ  * ( PT(I,J) + PT(I,J+1) )
               OPCT0 = b0 * TT(I  ,K,J) * (1. - deltap(I  ,K,J)) / Pstar1(I  ,J)
               OPCT3 = b0 * TT(I,K,J+1) * (1. - deltap(I,K,J+1)) / Pstar1(I,J+1)
...
! zmh
       if( .not. zmh_Coriolis)then   !zmh Coriolis
         ...
         DO K = beglev ,endlev
            DO I = IB,IE
               ...
               W12 = FS0V*FS0V*DTlin*DTlin
               DV(I,K,J) = - (PY(I,K,J) - FSU + FS0V*FS0V*DTlin*VT(I, K,J))/(1.+W12)
...
! 
               ...
               W11 = FS0*FS0*DTlin*DTlin
               W12 = FS0V*FS0V*DTlin*DTlin
               DU(I,K,J) = - (PX(I,K,J) + FSV + FS0 *FS0 *DTlin*UT(I, K,J))/(1.+W11)
               DV(I,K,J) = - (PY(I,K,J) - FSU + FS0V*FS0V*DTlin*VT(I, K,J))/(1.+W12)
             ENDDO
             call period( DU(1,K,J) )
             call period( DV(1,K,J) )
...
!zmh 
   IF(zmh_nlitti1)then
      CALL nliter_uvtp(UL, VL, TL, PsaL, PstL, DTlin, 1)  !ISTAR = 1  
   ENDIF
