...
!----------------------------------Local workspace-----------------------------------------------
   ...
   integer  :: commyz,ierr     !zhh 2013-01-28
   integer  :: I_leap1, I_leap1_1, I_leap2	, I_leap3, I_leap4, I_leap5, I_leap6	!For difference leap points
...
! call t_stopf('tend_adv_barrcommy1') 
  ...
  ! if(iam.eq.127) then
    !print *,'tendadv_out',iam,beglatdyn,endlatdyn,beglev,endlev
  !  print*,iam,'tend_adv_TTUTVT',sum(Ustar(358:726,25,4)),sum(TT(358:726,25,4)),sum(UT(358:726,25,4)),sum(VT(358:726,25,4))
   !endif
   src = iam-1
   dest  = iam+1
            ...
            DO I = IB,IE
			  ...
			  if (leap_flag ) then
               ...
               ! TL1  = OUXH(J)*(alpha(J)*(Ustar(I+1,K,J)*TT(I+1,K,J)-Ustar(I  ,K,J)*TT(I-1,K,J)) &   
                         ! + betaw(J)/3.0*(Ustar(I+2,K,J)*TT(I+3,K,J)-Ustar(I-1,K,J)*TT(I-3,K,J)))   !(4.13) 
...
!           COMPUTE HORIZONTAL & VERTICAL ADVECTION OF TT
            ...
            DTTP   = -a32*TLP - a33*TL3P 
         !   if(K.eq.25) then
          !    print *,iam,'j=NYtend_adv_DTTP',DTTP,TLP,TL3P
          !  endif
            DO I = 1 ,NX
               DT(I,K,NY) = DTTP
            ENDDO
...
! for UT =================================================================================
               ! UL1  = OUXQ(J) * ( alpha(J) * ((Ustar(I  ,K,J)+Ustar(I+1,K,J))*UT(I+1,K,J)   &			! equation(2.117)
...
! for VT =================================================================================
			  ...
			  if (leap_flag ) then
               ...
               ! VL1  = OVXQ(J)*( alpha2(J)*((Ustar(I+1,K,J+1)+Ustar(I+1,K,J))*VT(I+1,K,J)    &			! equation(2.123)
                                        ! -  (Ustar(I  ,K,J+1)+Ustar(I  ,K,J))*VT(I-1,K,J))   &   
                    ! +      betaw2(J)/3.0 *((Ustar(I+2,K,J+1)+Ustar(I+2,K,J))*VT(I+3,K,J)    &
                      ! -  (Ustar(I-1,K,J+1)+Ustar(I-1,K,J))*VT(I-3,K,J))  )						! (4.1)  
          ...
          !if((iam.eq.127).and.(j.eq.4).and.(K.eq.25).and.(I.eq.361)) then
           ! print *,iam,'tend_adv_DUUL123',DU(I,K,J),UL1,UL2,UL3
           ! print *,iam,'tend_adv_DVVL123',DV(I,K,J),VL1,VL2,VL3
          !endif  
            ENDDO
            call period( DU(1,K,J) )
            call period( DV(1,K,J) )
