...
!zmh added DTlin, zmh_cfl_ variables 
   use flexib,    only:  b1, b21, b22, c10, kappa, kappa2, IBCFFT, Kstd, &
                         DTlin, zmh_cfl_u0r,zmh_cfl_drag !zmh
...
!!   allocate ( PX(NX,NL,NY) )
   ...
   allocate ( sbuf1(NX,3*(endlev-beglev+1)+5) )
   allocate ( rbuf(NX,2*(endlev-beglev+1)+4) )
   allocate ( rbuf1(NX,3*(endlev-beglev+1)+5) )
!
...
!     COMPUTE    DTT/DT
   ...
   !JJR BUGDO J = loc_JB,loc_JE
   DO J = loc_JB,endlatdyn
...
!jjr  TT0(IB:IE,beglatdyn:endlatdyn,beglev:endlev)=DIV(IB:IE,beglev:endlev,beglatdyn:endlatdyn)
      disps(1)=0
      do i=1,npr_z
         recs(i)=zdist(i)*(IE-IB+1)*(endlatdyn-beglatdyn+1)*2 !jjrNX
         if(i.ne.npr_z) disps(i+1)=disps(i)+recs(i)
      end do

      num1=zdist(myid_z+1)*(endlatdyn-beglatdyn+1)*(IE-IB+1)*2 !jjr NX
...
!---------------------------------------------------------
            ...
            ENDDO
!       if(i==170.and.j==8) print*,'test jjr in lin0',DPSA(i,j),rhos(i,j),&
!          ksa(i,j),DpDrY(i,j),DPS1
         ENDDO       ! end i = IB,IE
!!!!
...
!            DWK(K)    = (kappa*(DPsa1-DSK)/P00 + D1(K))*DSIG(K)/PT(IB,NY)
         ...
         DO I = 1 ,NX
            DPsa(I,NY)    = DPsa1
         ENDDO
!         print*,'test jjr in tend_lin DPSA',DPSA1,myid_z
         DO K = beglev ,endlev+1
            DO I = 1 ,NX
               WST(I,K,NY)  = W1(K)
...
!            do k=beglev,endlev
!            DO I = 1, NX
!        if(i==1..and.k==1.and.j==1) print*,'test jjr in lin11',&
!        DPSA(i,j),DT(i,k,j)
!        end do
!       end do
!   end do
...
!            do k=beglev,endlev
!            DO I = 1, NX
!        if(i==1.and.k==1.and.j==1) print*,'test jjr in lin12',&
!        DPSA(i,j),DT(i,k,j)
!       end do
!   end do
!  end do
...
!jjr            deltap(I,K,J) = PMTOP / PLY (I,K,J) !jjr test
         ENDDO
      ENDDO
      DO I = 1 ,NX
      ...
      DO K = beglev ,endlev
         DO I = 1 ,NX
            ...
            WPI  = ONE - WPJ
            KPI  = KPK + 1
!         if(i==36.and.k==20.and.j==47) then 
!               PRINT*,'************************THERE IS AN ERROR INCBB***************'
!               PRINT*,'PLY(I,K,J)=',PLY(I,K,J),'I,K,J=',I,K,J
!               PRINT*,'P(I,J)=',P(I,J),KPI,NA,DPALIB,PMTOP
!               PRINT*, 'stop: tend_lin'
!         endif
...
!            DO I = 1, nx
!
!        if(i==60.and.k==1.and.j==127) print*,'test jjr 2 in tend_lin',&
!         PY(I,K,J),pstar1(i,j),pstar2(i,j),pt(i,j),tt(i,k,j),deltap(i,k,j),ghi(i,k,j)
!       end do
!       end do
...
!          DO K = beglev, endlev
!            DO I = 1, NX
!        if(i==170.and.k==8.and.j==8) print*,'test jjr in lin',&
!         V(I,K,J),U(I,K,J),FF(J),CUR(J),RUPD(J),RUMD(J),DPSA(i,j)
!       end do
!       end do
!   end do
       ...
       DO K = beglev ,endlev
         DO I = 1 ,NX
               ...
               ZMH_TAU         = DTlin*max(10.0_r8, 1./(1._r8 - SQRT(1. - W_CFL*W_CFL))) 
                                         !n*DTlin is a minimum hard limit of tau

               W12       = zmh_cfl_coef*W11/ZMH_TAU !coef is either  0 or 1

          !if(J .ge. NY-4 )then
          !     if(K .eq. 3)then
          !         print*,'J,I,K',J,I,K
          !         print*,W_KE ,W_SPD,W11,DTlin, &
          !             W_CFL,ZMH_TAU, ZMH_DX(J),ZMH_CFL_U(J),W12
          !     endif
          ! endif
