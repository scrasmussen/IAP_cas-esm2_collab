...
!           ZhangHe, 2013-06-04, added subroutine smothq
!           ZhangHe, 2014-04-18, added smoothing of WS
!------------------------------------------------------------------------------------------------
   ...
   use pmgrid,     only: beglatdyn, endlatdyn, beglatdynex, endlatdynex, &
                         loc_JB, loc_JE,beglev,endlev,npr_y,endlevp
...
!================================================================================================
   SUBROUTINE SMOOTH
!------------------------------------------------------------------------------------------------
!  Purpose: SMOOTHING   T    BY 2-ORDER SHAPIRO SMOOTHER(LON & LAT)
!                     U & V  BY 2-ORDER SHAPIRO SMOOTHER(LON  ONLY)
!------------------------------------------------------------------------------------------------
      use IAP_prog,   only: U, V, T, Q, WS
      implicit none
!-------------------------------------Local workspace-------------------------------------------
!      integer, parameter :: IC = 1   ! index controlling smoothing intensity
      integer, parameter :: IC = 3   ! index controlling smoothing intensity
      real(r8) :: WW(NX,NY)
      real(r8) :: WG(NX,NL,NY)
      integer  :: I, J, K
!------------------------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------------------------
         WW(:,beglatdyn:endlatdyn)=WS(:,K,beglatdyn:endlatdyn)
         CALL SOSS2D( WW,0,1,IC )
         WS(:,K,beglatdyn:endlatdyn)=WW(:,beglatdyn:endlatdyn)
!------------------------------------------------------------------------------------------------
      END DO
!
!     SMOOTHING WIND
...
!================================================================================================
   SUBROUTINE SMOTHQ(qsmth)
!------------------------------------------------------------------------------------------------
! Purpose: SMOOTHING  Q  BY 2-ORDER SHAPIRO SMOOTHER(LON & LAT)
! Completed: Zhang He, 2013-06-04
!------------------------------------------------------------------------------------------------
    
      implicit none
!------------------------------------Arguments------------------------------------------------
      real(r8), intent(inout) :: qsmth(NX,beglev:endlev,beglatdyn:endlatdyn)
...
!     
      DO K = beglev , endlev
!------------------------------------------------------------------------------------------------
         WW(:,beglatdyn:endlatdyn)=qsmth(:,K,beglatdyn:endlatdyn)
         CALL SOSS2D( WW,0,1,IC )
         qsmth(:,K,beglatdyn:endlatdyn)=WW(:,beglatdyn:endlatdyn)
!------------------------------------------------------------------------------------------------
      END DO
!
      RETURN
...
!         
      ...
      JQ   = JF+ 4
      JJ   = 5 - IP
!jjr      DO J = beglatdynex ,min(JF,endlatdynex)        !zhh !jjr del ex
      DO J = beglatdyn ,min(JF,endlatdyn)        !zhh !jjr del ex
         J2          = J + 2
         DO I = 1 ,IM
            I2       = I + 2
            B(I2,J2) = A(I,J)
...
!     LONGITUDINAL B.C. ( Boundary Condition )
      DO I = 1 ,2
         ...
         IX         = I + NEX           !! zhh
         I2         = I + 2
!jjr bug         DO J = beglatdyn ,min(JQ,endlatdyn+2)
         DO J = beglatdyn ,min(JQ,endlatdyn+4) !jjr bug
            B(I ,J) = B(II,J)
!jjr            B(IX,J) = B(I2,J)
!jjr            B(I ,J) = B(II,J)
            B(IX,J) = B(I2,J)
...
!!jjr      call mpi_move_left(B(1,endlatdyn-1), B(1,beglatdyn-2), 4*IG)
      ...
      call mp_recv3d( commyz, src, IG,  JG,1,                             &
                      1, IG, 1, JG,1,1,       &
                      1, IG, beglatdyn, beglatdyn+1,1,1, B )
      call mp_send3d( commyz, dest, src, IG, JG,1,                &
                      1, IG, 1,JG ,1,1,       &
                      1, IG, endlatdyn-1, endlatdyn, 1, 1, B )
      call mp_recv3d( commyz, src, IG,  JG,1,                             &
                      1, IG, 1, JG,1,1,       &
...
!                               <10 11   12 13>14 15 16 17 18(iam=0,B) !wjp 2007.05
      ...
      if(endlatdyn.eq.NY) iend = JF2 !jjr bug
!
      DO J0 = ibeg, iend
#else
      DO J0 = 3 ,JF2
