...
!           Zhang He, 2013-06-04, added smoothing q in sub. trans_af_mass
!           Chai Zhaoyang, 2017-11-28, fixed the bug related to negtive specific humidity
!------------------------------------------------------------------------------------------------
...
!-------------------------------------Local workspace--------------------------------------------
      ...
      real(r8) :: qtmp1(NX,beglev:endlev,beglatdyn:endlatdyn)
      real(r8) :: qtmp2(NX,beglev:endlev,beglatdyn:endlatdyn)
      real(r8) :: qtmp3(NX,beglev:endlev,beglatdyn:endlatdyn)
      real(r8), allocatable :: q3tmp(:,:,:)!(plon,plev,pcnst) czy 20171128
!------------------------------------------------------------------------------------------------
!
! for ps
      ...
      DO Jd = beglatdyn, endlatdyn
         ...
         DO K = beglev, endlev
            DO I = 1, plon
               ...
               qtmp1(I+EX,K,Jd) = q3(I,K,1,Jp,n3) 
!!               qtmp2(I+EX,K,Jd) = q3(I,K,2,Jp,n3) 
!!               qtmp3(I+EX,K,Jd) = q3(I,K,3,Jp,n3) 
            END DO
            call period( T   (1,K,Jd) )
            call period( qtmp1(1,K,Jd) )
!!            call period( qtmp2(1,K,Jd) )
!!            call period( qtmp3(1,K,Jd) )
         END DO
      END DO 
!
! Smoothing qtmp
...
!----------------------------------------------------------
      ...
      DO Jd = beglatdyn, endlatdyn
         ...
         DO K = beglev, endlev
            DO I = 1, plon
               q3(I,K,1,Jp,n3) = qtmp1(I+EX,K,Jd) 
!!               q3(I,K,2,Jp,n3) = qtmp2(I+EX,K,Jd) 
!!               q3(I,K,3,Jp,n3) = qtmp3(I+EX,K,Jd) 
            END DO
         END DO
      END DO 
!---------------      czy20171128      ---------------
!$OMP PARALLEL DO PRIVATE (j)
      do j = beglat, endlat
        allocate(q3tmp(1:plon,1:plev,1:pcnst))
#if (defined SPMD)
        if (npr_z.gt.1) then
                q3tmp(:,:,:)=0.
        end if
#endif
        q3tmp(1:plon,beglev:endlev,1:pcnst)=q3(1:plon,beglev:endlev,1:pcnst,j,n3)
#if (defined SPMD)
        if (npr_z.gt.1) then
                call parcollective3d( comm_z, sumop,plon ,plev,pcnst,q3tmp)
        endif
#endif
        call qneg3 ('Trans_af_mass', j, nlon(j), plon, plev, 1, pcnst, qmin, q3tmp(1,1,1))
        q3(1:plon,beglev:endlev,1:pcnst,j,n3)=q3tmp(1:plon,beglev:endlev,1:pcnst)
        deallocate(q3tmp)
      enddo
!---------------      czy20171128     ---------------
