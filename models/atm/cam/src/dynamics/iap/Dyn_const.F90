...
!=======================================================================
   ...
   integer  :: k_leap(NY)  ! coefficient for flexible leaping-point zonal difference (at J+1/2)
   integer  :: k_leap2(NY)  ! coefficient for flexible leaping-point zonal difference (at J+1/2)
   integer  :: IP(NX) 
...
!-----------------------------------------------------------------------
	  ...
	  real(r8) :: sine_devide	!CH 180913
	  real(r8) :: cos_deglat
	  real(r8) :: cos_70
	  real(r8) :: cos_45
	  real(r8) :: devide_lon
	  real(r8) :: k_temp
	  ! integer	:: k_leap   !number of leap points
	  ! integer	:: k_leap2   !number of leap points
	  integer, parameter :: N = 1   !number of leap points
...
! set coefficient for flexible leaping-point zonal difference (zhh 2005.8. 26, modify: 2006.3.20)
      ...
      es  = 180.0 - en  ! 122.0
      inc = 1.0 / (en - bn)
	  devide_lon = 360.0E0/PLON   !LAT & LON devide (degrees) CH 190115 modified
	  cos_45 = COS(45.0E0*PI/180.0E0)			!CH 180913 181105 45 -> 30


!jjr      do J = beglatdyn, endlatdyn
      do j = 1, NY
		 Y1 = DBLE(J-1) * DEGLAT		! co-latitude
		 ! IF (JB .le. J .and. J .le. JE) then
		 degree = 90.0E0 - Y1			! Latitude  !CH 180913
		 if( (ABS(degree) .ge. setlat) .and. (JB .le. J) .and. (J .le. JE)) then	! high latitude

			cos_deglat = COS(degree*PI/180.0E0)		!CH 180913
			! cos_70 = COS(70.0E0*PI/180.0E0)			!CH 180913
			sine_devide = SIN(devide_lon*PI/180.0E0)	!CH 180913		 
			k_temp = ASIN(cos_45*sine_devide)/(N*ASIN(cos_deglat*sine_devide))	!CH 180829
			k_leap(J) = CEILING(k_temp)	!CH 180913
			! k_leap(J) = 1 	!For test 180918
			! print*, 'J = ', J, 'k_leap = ', k_leap
		 ELSE
			k_leap(J) = 1
		 ENDIF
...
!jjr      do J = beglatdyn, endlatdyn
      do J = 1, NY
		 ...
		 degree = 90.0E0 - Y1			! Latitude  !CH 180913
		 if( (ABS(degree).ge.setlat) .and. (1 .le. J) .and. (J .le. JE)) then	! high latitude

			cos_deglat = COS(degree*PI/180.0E0)		!CH 180913
			! cos_70 = COS(70.0E0*PI/180.0E0)			!CH 180913
			sine_devide = SIN(devide_lon*PI/180.0E0)	!CH 180913		 
			k_temp = ASIN(cos_45*sine_devide)/(N*ASIN(cos_deglat*sine_devide))	!CH 180829
			k_leap2(J) = CEILING(k_temp)	!CH 180913
			! k_leap2(J) = 1		!CH  For test
			! print*, 'J = ', J, 'k_leap = ', k_leap
		 ELSE
			k_leap2(J) = 1
		 ENDIF
...
!jjr      do J  = beglatdyn ,loc_JE
       ...
       DXYV(NY)   = HALF*(RAD*DLON*(COSL(JE)+COSL(NY)))*(RAD*DLAT)
!
#if (defined SPMD)
! Latitude distribution for plat=16 on 3 processors.             !wjp 2007.05
!  procId:      iam=0     |      iam=1     |      iam=2          !wjp 2007.05
