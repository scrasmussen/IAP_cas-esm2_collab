...
!   write(iulog,*)"*******before allocate ptxy, h2o, pexy, pmid, pdel*******"
   !zmh allocate (ptxy(beglonxy:endlonxy, beglatxy:endlatxy, plev))!(i,j,k)
   !zmh allocate (h2o(beglonxy:endlonxy, beglatxy:endlatxy, plev))!(i,j,k)
   allocate (ptxy(beglonxy:endlonxy, plev, beglatxy:endlatxy))!(i,k,j) !zmh  
   allocate (h2o (beglonxy:endlonxy, plev, beglatxy:endlatxy))!(i,k,j) !zmh
...
!zmh
            ptxy(i,k,j)=t3(i,k,j)*(1.0_r8+zvir*q3(i,k,1,j))
            h2o (i,k,j)=q3(i,k,1,j)
         end do
      end do
   end do
