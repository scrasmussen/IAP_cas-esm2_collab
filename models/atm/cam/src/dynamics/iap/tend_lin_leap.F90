...
!------------------------------------------------------------------------------------------------
   ...
   use IAP_grid,  only: NX, NY, NM, NL, NZ, JB, JE, IB, IE, period, NLON
   use Dyn_const 
   ...
   use physconst, only: b0, CAPA, cpair, rair !zmh
   use mathconst, only: ZERO, HALF, ONE, TWO, FOURTH
   !zmh added DTlin, zmh_cfl_ variables 
   use flexib,    only:  b1, b21, b22, c10, kappa, kappa2, IBCFFT, Kstd, &
                         DTlin, zmh_cfl_u0r, zmh_cfl_drag
   ...
   use infnan,       only: inf      !CH 180118

#if ( defined SPMD )
   use mod_comm, only: mp_send3d, mp_recv3d,mp_send3d_2, mp_recv3d_2
   use spmd_dyn          , only:  comm_y, comm_z,zdist,lreq,rreq
...
!
   ...
   integer  :: I_leap1, I_leap2
   logical  :: leap_flag = .true.		!!!For test of leap-cofficient CH 180919
   logical  :: filt_flag = .true.		!!!For test of FILTER CH 181026
!jjr
...
!zmh
   real(r8) :: zmh_tau,zmh_cfl_coef
   real(r8) :: ZMH_CFL_U(NY)
   real(r8) :: w10,w12,w_ke,w_spd,w_cfl, utmp,vtmp
...
!zmh
   if(zmh_cfl_drag) then
      zmh_cfl_coef = 1.0_r8
   else
      zmh_cfl_coef = 0.0_r8
   endif
   DO J = 1,NY
      ZMH_CFL_U(J)    = ZMH_DX(J)/DTlin 
                    ! TO Baodong and Cao Hang, THIS ZMH_DX SHOULD BE CHANGED TO LEAP GRID SPACE 
                    ! WHEN zmh_cfl_drag=.true. IT IS SET IN Dynconst.F90
   ENDDO

#if (defined SPMD)
!jjr src dest,right
! call MPI_BARRIER(comm_y,ierr)
! call t_startf('tend_lin_barrcommy1')
! call MPI_BARRIER(comm_y,ierr)
! call t_stopf('tend_lin_barrcommy1')
call t_startf('tend_lin_moveleft1')
...
! call MPI_BARRIER(comm_y,ierr)
call t_stopf('tend_lin_moveleft1')
#endif
call t_startf('tend_lin_comput1')
   DO J = loc_Jb,loc_JE 
      DO I = 1 ,NX
       ...
       !if(Psa(i,j) .eq. inf) then   !CH 181022   no inf found
      !  print *, 'Psa:', Psa(i,j), i, j, 'beglatdyn:', beglatdyn, 'endlatdyn:', endlatdyn
      !else
      !  if(iam .lt. 2) print *, 'Psa no inf'
      !endif   
      ENDDO
      DO I = IB,IE
...
call t_stopf('tend_lin_comput1')
#if (defined SPMD)
! call MPI_BARRIER(commyz,ierr)
! call t_startf('tend_lin_barrcommy2')
! call MPI_BARRIER(comm_y,ierr)
! call t_stopf('tend_lin_barrcommy2')
call t_startf('tend_lin_3d_right1')
...
! call MPI_BARRIER(commyz,ierr)
call t_stopf('tend_lin_3d_right1')
#endif
call t_startf('tend_lin_comput2')
   DpDrY=0.
   DO J = loc_JB, loc_JE
      DO I = 1, NX
      ...
      END DO
   END DO
call t_stopf('tend_lin_comput2')
!     COMPUTE    DTT/DT

#if (defined SPMD)
! call MPI_BARRIER(commyz,ierr)
! call t_startf('tend_lin_barrcommy3')
! call MPI_BARRIER(comm_y,ierr)
! call t_stopf('tend_lin_barrcommy3')
call t_startf('tend_lin_3d_2')
...
! call MPI_BARRIER(commyz,ierr)
call t_stopf('tend_lin_3d_2')
#endif
! call MPI_BARRIER(mpicom,ierr)
call t_startf('tend_lin_comput3')
   DO J = loc_JB,loc_JE
! -------------------------------- COMPUTE DIVERGENCES & THEIR SUMS --------------------------
      DO K = beglev ,endlev
         DO I = IB, IE
			...
			!CH 180917 modified
			if (leap_flag ) then
				I_leap1 = mod(I+1*k_leap(J),NX) + (I+1*k_leap(J))/NX * (IB+2)
				I_leap2 = (k_leap(J)-1+NLON)/(I+NLON) * NLON + (I-(k_leap(J)-1))
			else
				I_leap1 = I+1
				I_leap2 = I
				k_leap(J) = 1
            ...
            DIV(I,K,J) = (PXW(I_leap1,J) * UT(I_leap1,K,J) - PXW(I_leap2,J) * UT(I_leap2  ,K,J))/abs(2*k_leap(J)-1)   &
                          + (PYP(I,J)*VT(I,K,J)-PYM(I,J)*VT(I,K,J-1))                          !(4.22)
         ENDDO
      ENDDO
	  ! print*, 'J=', J, k_leap(J), abs(2*k_leap(J)-1)
   end do !jjr j
   if (myid_y.eq.(npr_y-1)) then   ! at north polar 
      ...
      if(npr_z.gt.1) then
#if (defined SPMD)
! call MPI_BARRIER(comm_z,ierr)
! call t_startf('tend_lin_barrcommz1')
! call MPI_BARRIER(comm_z,ierr)
! call t_stopf('tend_lin_barrcommz1')
call t_startf('tend_lin_parcollective1')
         call parcollective(comm_z,sumop,NL,D2)
! call MPI_BARRIER(comm_z,ierr)
call t_stopf('tend_lin_parcollective1')
#endif
      end if
   endif
   if (myid_y.eq.0) then   ! at north polar
      ...
      if (npr_z.gt.1) then
#if (defined SPMD)
! call t_startf('tend_lin_barrcommz2')
! call MPI_BARRIER(comm_z,ierr)
! call t_stopf('tend_lin_barrcommz2')
call t_startf('tend_lin_parcollective2')
         call parcollective(comm_z,sumop,NL,D3)
! call MPI_BARRIER(comm_z,ierr)
call t_stopf('tend_lin_parcollective2')
#endif
      end if
   ENDIF
...
! call MPI_BARRIER(mpicom,ierr)
! call t_startf('tend_lin_barrcommz3')
! call MPI_BARRIER(comm_z,ierr)
! call t_stopf('tend_lin_barrcommz3')
call t_startf('tend_lin_allgatherv')
      call mpi_allgatherv(TT0,num1,mpir8,TT1,recs,disps,mpir8,comm_z,ierr)
! call MPI_BARRIER(mpicom,ierr)
call t_stopf('tend_lin_allgatherv')
#endif
   endif
   call t_stopf('tend_lin_comput3')
   ! call MPI_BARRIER(mpicom,ierr)

   call t_startf('tend_lin_comput4')
     DO J = loc_JB,loc_JE 
         DO I = IB, IE
! -------------------------------- COMPUTE DIVERGENCES & THEIR SUMS --------------------------
            DO K = 1 ,NL
...
!------------------ COMPUTE DTT/DT FOR J [JB,JE]  &  I [IB,IE]  &  K [1,NL] ---------------------
         DO K = beglev ,endlev
            DO I = IB,IE
			   if (leap_flag ) then
				I_leap1 = mod(I+2*k_leap(J)-1,NX) + (I+2*k_leap(J)-1)/NX * (IB+2)
				I_leap2 = (2*k_leap(J)-1+NLON)/(I+NLON) * NLON + (I-(2*k_leap(J)-1))
				! I_leap1 = mod(I+k_leap(J)-1,NX) + (I+k_leap(J)-1)/NX * (IB+2)
				! I_leap2 = (k_leap(J)+NLON)/(I+NLON) * NLON + (I-k_leap(J))
			   else
				I_leap1 = I+1
				I_leap2 = I-1
				k_leap(J) = 1
               ...
               DXP0 = Pstar2(I  ,J) - Pstar2(I_leap2,J)
			   
			   ! ! I_leap1 = mod(I+1*k_leap(J),NX) + (I+1*k_leap(J))/NX * (IB+2)
			   ! ! I_leap2 = (k_leap(J)-1+NLON)/(I+NLON) * NLON + (I-(k_leap(J)-1))

               ! DXP1 = Pstar2(I+1,J) - Pstar2(I,J)
               DXP1 = Pstar2(I_leap1,J) - Pstar2(I,J)
...
!
               ...
               ! OO2X = OUXH(J)/Pstar1(I,J)*(alpha(J)*(DXP1*UT(I+1,K,J)+DXP0*UT(I,K,J))  &
                                      ! +betaw(J)/3.0*(DXP3*UT(I+2,K,J)+DXP2*UT(I-1,K,J)))  !(4.18)  !!Formula 2.135
				! CH 180917 modified
			   if (leap_flag ) then
				I_leap1 = mod(I+k_leap(J),NX) + (I+k_leap(J))/NX * (IB+2)
				I_leap2 = (k_leap(J)-1+NLON)/(I+NLON) * NLON + (I-(k_leap(J)-1))
			   else
				I_leap1 = I+1
				I_leap2 = I
               ...
               OO2X = OUXH(J)/Pstar1(I,J) * (DXP1 * UT(I_leap1 ,K,J) + DXP0 * UT(I_leap2,K,J))/abs(2*k_leap(J)-1)  !(4.18)
...
!           COMPUTE HORIZONTAL & VERTICAL ADVECTION OF TT
            O2P    = ZERO
            DO I   = IB,IE
               O2P = O2P + (Pstar2(I,NY)-Pstar2(IB,JE))*VT(I,K,JE) / Pstar1(IB,NY)    !(4.17)-(3)
            ENDDO
            O2P    = RDLATI * O2P
...
!! i,iam,testnum
      ...
      ENDIF
      call t_stopf('tend_lin_comput4')
      ! call MPI_BARRIER(mpicom,ierr)
!!

   call t_startf('FILT2Dnew2_DPDT')
   WW(:,beglatdyn:endlatdyn)=DPsa(:,beglatdyn:endlatdyn)   !zhh 2008.6.16
   
   ! if(iam.eq.0) then
	 ! DO i = IB, IE
		! print *, 'BF tend_lin_bfDPsa', i, DPsa(I, 360)
	 ! ENDDO	 
     ! ! print *,'j=360', 'AF LEAP tend_lin_DPsa',sum(DPsa(IB:IE,360))
   ! endif 
   if(filt_flag) then	  
	CALL FILT2Dadaptive( WW,0,1,IBCFFT,3 )       !zhh 2008.6.16
   endif
   DPsa(:,beglatdyn:endlatdyn)=WW(:,beglatdyn:endlatdyn)
   ! if(iam.eq.0) then
	 ! DO i = IB, IE
		! print *, 'AF tend_lin_afDPsa', i, DPsa(I, 360)
	 ! ENDDO	 
	 ! ! DO i = IB, IE
		! ! print *, 'AF FFT tend_lin_afDPsa', i, WW3(I, 360)
	 ! ! ENDDO
     ! stop 'Finish test CH'
   ! endif   
   DO KK = beglev,endlev
     ...
     if(filt_flag) then	  
      CALL FILT2Dadaptive( WW,0,1,IBCFFT,3 )   !OK\
	 endif
      DT(:,KK,beglatdyn:endlatdyn)=WW(:,beglatdyn:endlatdyn)
   ENDDO
   call t_stopf('FILT2Dnew2_DPDT')
   ! call MPI_BARRIER(mpicom,ierr)
   call t_startf('tend_lin_comput5')
!
!     *******************************************************************
!     *******************************************************************
!     ****************         COMPUTE DU & DV        *******************
      ...
      DO K = beglev ,endlev
         DO I = 1 ,NX
            ...
            IF (KPI.GT.NA) THEN   ! NA = PEALIB/DPALIB = 2320
               ...
               PRINT*,'PLY(I,K,J)=',PLY(I,K,J),'I,K,J,iam=',I,K,J,iam
               PRINT*,'P(I,J)=',P(I,J)
               PRINT*,'KPI(I,J)=',KPI
               PRINT*, 'stop: tend_lin'
               stop 'tend_lin'
...
#if (defined SPMD)
! call MPI_BARRIER(commyz,ierr)
call t_startf('tend_lin_barrcommy4')
call MPI_BARRIER(comm_y,ierr)
call t_stopf('tend_lin_barrcommy4')
call t_startf('tend_lin_3d_GHI')
...
!!======================origin in loop======================================
               ...
               ! ! DXP0  = Pstar2(I_leap1 ,J) - Pstar2(I_leap2 ,J)
               ! DXP1  = Pstar2(I+1,J) - Pstar2(I-2,J)
               ! OUXAXP = OUXQ(J) * ( PT(I  ,J) + PT(I-1,J) )
               ! ! OUXAXP = OUXQ(J) * ( PT(I_leap1  ,J) + PT(I_leap2 ,J) )
...
! !
			   ...
			   ! !CH 180917 modified
               ! ! OPCT0 = b0 * TT(I_leap1  ,K,J) * (1. - deltap(I_leap1  ,K,J)) / Pstar1(I_leap1  ,J)
               ! ! OPCT2 = b0 * TT(I_leap2  ,K,J) * (1. - deltap(I_leap2  ,K,J)) / Pstar1(I_leap2  ,J)
			   ...
			   ! !CH 180917 modified			   
               ! ! GH0   = GHI(I_leap1  ,K+1,J) + GHI(I_leap1  ,K,J)	!CH 180917 modified
               ! GH1   = GHI(I-1,K+1,J) + GHI(I-1,K,J)
               ! ! GH1   = GHI(I_leap2, K+1,J) + GHI(I_leap2 ,K,J)
...
!
               ...
               ! PX1   = alpha(J) * OUXAXP  * ( GH0 - GH1 )                      &
                     ! + betaw(J) * OUXAXP2 * ( GH2 - GH3 ) / 3.0                     ! (4.10)
               PX1   = OUXAXP  * ( GH0 - GH1 ) /abs(2*k_leap(J)-1)        
               ! PX2   = alpha(J) * OUXH(J) * ( OPCT0 + OPCT2 ) * DXP0           &
                     ! + betaw(J) * OUXH(J) * ( OPCT4 + OPCT5 ) * DXP1 / 3.0          ! (4.11)
               PX2   = OUXH(J) * ( OPCT0 + OPCT2 ) * DXP0 /abs(2*k_leap(J)-1)   
               ! PY1   = OYAYP  * ( GH4 - GH0)                                        ! (4.4 )
               ! PY2   = RDLATH * ( OPCT0 + OPCT3 ) * DYP0                            ! (4.5 )
               PX(I,K,J)  = b1*PX1 + b22*PX2
               ! PY(I,K,J)  = b1*PY1 + b21*PY2
...
! 
               ...
               if((J.eq.360).and.(I.eq.363).and.(K.eq.25).and.(iam.eq.96)) then
                  !print *,'tli_FSU:',iam,DV(I,K,J),PY(I,K,J),FSU
               endif
            ENDDO
            call period( DU(1,K,J) )
            call period( DV(1,K,J) )
...
!=============================== zhh =====================================
               ...
               !IF (abs(DU(I,K,J)) > 5E0 .OR. abs(DV(I,K,J)) > 5E0 .or.      &
                !   abs(SU(I,K,J)) > 5E0 .OR. abs(SV(I,K,J)) > 5E0) THEN
...
!zmh added drag
               ...
               W_KE       = UT(I,K,J)*UT(I,K,J) + VT(I,K,J)*VT(I,K,J) 
               W_SPD      = SQRT(W_KE)

               W11          = (W_SPD - ZMH_CFL_U(J))/ (zmh_cfl_u0r*W_SPD)
               W11          = min(max(W11, -1.0_r8), 1.0_r8) + 1.0_r8  !range from 0 to 2

               W_CFL         = min(W_SPD*DTlin/ZMH_DX(J),0.99_r8)

               ZMH_TAU         = DTlin/(1._r8 - SQRT(1. - W_CFL*W_CFL))

               W12       = zmh_cfl_coef/ZMH_TAU
               DU(I,K,J) = DU(I,K,J) - UT(I,K,J) * W12
               DV(I,K,J) = DV(I,K,J) - VT(I,K,J) * W12
               DT(I,K,J) = DT(I,K,J) + W_KE/2.0/cpair * W12
!zmh ------
   ...
   call t_stopf('tend_lin_comput5')
!
!     HIGH-MID LAT FILTER  FOR DU/DT & DV/DT

! call MPI_BARRIER(mpicom,ierr)
call t_startf('FILT2Dnew2_DUDV')
   ! if(iam.eq.0) then
	 ! DO i = IB, IE
		! print *, 'BF Filter_DU', i, DU(I, 0, 360)
	 ! ENDDO	 
   ! endif   
   DO KK = beglev,endlev
...
!         
   ...
   ! if(iam.eq.0) then
	 ! DO i = IB, IE
		! print *, 'AF Filter_DU', i, DU(I, 0, 360)
	 ! ENDDO	 
	 ! stop 'Finish test in tend_lin CH'

   ! endif
   
   call t_stopf('FILT2Dnew2_DUDV')

!zmh added drag
     DO J  = beglatdyn, endlatdyn
       DO K = beglev ,endlev
         DO I = 1 ,NX
               Utmp       = UT(I,K,J)+DTlin*DU(I,K,J) 
               Vtmp       = VT(I,K,J)+DTlin*DV(I,K,J) 
               W_KE       = Utmp*Utmp + Vtmp*Vtmp
               W_SPD      = SQRT(W_KE)
               ...
               W_CFL         = min(W_SPD*DTlin/max(ZMH_DX(J),1.0),0.99_r8)

               ZMH_TAU         = DTlin*max(10.0_r8, 1./(1._r8 - SQRT(1. - W_CFL*W_CFL))) 
                                         !2.0*DTlin is a minimum hard limit of tau

               W12       = zmh_cfl_coef/ZMH_TAU

               DU(I,K,J) = DU(I,K,J) - UT(I,K,J) * W12
               DV(I,K,J) = DV(I,K,J) - VT(I,K,J) * W12
               DT(I,K,J) = DT(I,K,J) + W_KE/cpair * W12 *(rair/PT(I,J)/b0) 
               !commented out
            ENDDO
         ENDDO
     ENDDO
