...
!lke--
  ...
  ! for the cotags kludge..
    ...
    plat = get_dyn_grid_parm('plat')
    if(dycore_is('UNSTRUCTURED') ) then
!       write(iulog,*)'Before call get_landuse_and_soilw '
       call get_landuse_and_soilw_from_file(do_soilw)
!       write(iulog,*)'After call get_landuse_and_soilw '
...
!       write(iulog,*)'Before infld fraction_landuse'
       call infld('fraction_landuse', piofile, 'ncol','class',' ',1,pcols,1,n_land_type, & 
                   begchunk,endchunk, fraction_landuse, readvar, 'PHYS')
!       write(iulog,*)'After infld fraction_landuse'
       if(do_soilw) then
