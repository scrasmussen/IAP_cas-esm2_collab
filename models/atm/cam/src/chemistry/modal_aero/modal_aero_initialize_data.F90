...
#elif (( defined MODAL_AERO_4MODE ) || (defined MODAL_AERO_3MODE))
...
#if ( defined MODAL_AERO_3MODE ) && ( defined MODAL_AERO_7MODE ) && ( defined MODAL_AERO_4MODE )
       call endrun( 'Error - when modal_aero defined, just 1 of modal_aero_3/7mode must be defined'
#elif ( ! ( defined MODAL_AERO_3MODE ) ) && ( ! ( defined MODAL_AERO_7MODE ) ) && ( ! ( defined MODAL_AERO_4MODE ) )
...
9213   format( 4x, a, 5(1pe14.5) )
       ...
       do m = 1, ntot_amode
          ...
          if      (m == modeptr_accum) then
             ...
             qneg3_worst_thresh_amode(l) = 1.0e3_r8
          else if (m == modeptr_newseas) then
             qneg3_worst_thresh_amode(l) = 1.0e3_r8
     ...
     subroutine initaermodes_setspecptrs
       ...
       do m = 1, ntot_amode
          if (modename_amode(m) .eq. 'accum') then
             ...
             modeptr_coardust = m
          else if (modename_amode(m) .eq. 'new seasalt') then
             modeptr_newseas = m
          end if
       end do
       ...
       write(iulog,*) 'modeptr_newseas =', modeptr_newseas
