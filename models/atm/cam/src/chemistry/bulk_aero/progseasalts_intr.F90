...
!  real(r8)::  soil_erodibility(plon,plat)            ! soil erodibility factor
...
#elif (defined MODAL_AERO_4MODE)
              (/0.08e-6_r8, 0.02e-6_r8, 1.0e-6_r8, 10.0e-6_r8/)  ! accu, aitken, coarse, newseasalt
#elif (defined MODAL_AERO_3MODE)
              (/0.08e-6_r8, 0.02e-6_r8, 1.0e-6_r8/)  ! accu, aitken, coarse, newseasalt
#endif
  real(r8) :: sst_sz_range_hi(nsst) = &
#if (defined MODAL_AERO_7MODE)
              (/0.3e-6_r8, 0.08e-6_r8, 1.0e-6_r8, 10.0e-6_r8, 300.0e-6_r8/)
#elif (defined MODAL_AERO_4MODE)
  ...
  real(r8) :: sst_sz_range(nsst+1) = &
              (/0.2e-6_r8,1.0e-6_r8,3.0e-6_r8,10.0e-6_r8,300.0e-6_r8/)  ! bottom and tops of size bins for sea salts in m 
#endif
...
#if (defined MODAL_AERO_7MODE)
  integer, pointer  :: ncl_a1_ndx, ncl_a2_ndx, ncl_a4_ndx, ncl_a6_ndx, ncl_a8_idx
  integer, pointer  :: num_a1_ndx, num_a2_ndx, num_a4_ndx, num_a6_ndx, num_a8_idx
#elif (defined MODAL_AERO_4MODE)
...
#if (defined MODAL_AERO)
#if (defined MODAL_AERO_7MODE)
              =(/0.051e-6_r8, 0.15e-6_r8, 0.56e-6_r8, 3.6e-6_r8, 25.0e-6_r8/) ![m] Mass-weighted mean diameter resolved
#elif (defined MODAL_AERO_4MODE)
              =(/0.051e-6_r8, 0.256e-6_r8, 3.6e-6_r8, 25.0e-6_r8/) ![m] Mass-weighted mean diameter resolved
#elif (defined MODAL_AERO_3MODE)
              =(/0.051e-6_r8, 0.256e-6_r8, 3.6e-6_r8/)
#endif
...
contains
  ...
  !===============================================================================
  subroutine progseasalts_initialize(seasalt_modes_in) 
    !----------------------------------------------------------------------- 
    ! 
...
#endif
    implicit none
    integer, intent(in) :: seasalt_modes_in
    !---------------------------Local workspace-----------------------------
    integer :: m,n
    ...
    !-----------------------------------------------------------------------
    seasalt_modes = seasalt_modes_in
...
! calculate constants form emission polynomials
    do m=1,sections
       ...
       elseif (m.ge.22.and.m.lt.32)then
! use monahan
...
#if (defined MODAL_AERO_4MODE) 
! use Andreas 1998
       elseif (m.ge.32.and.m.lt.34)then
...
! use Ekman's ss
    ...
    ! A new whitecap area function by Adrian Callaghan,2008 is used.
    do i=1,ncol
       if ((u10cubed(i).gt.3.70_r8).and.(u10cubed(i).lt.9.25_r8)) then
          u10cubed(i)=8.28_r8*(u10cubed(i)-3.70_r8)**3.0_r8
       else if (u10cubed(i).ge.9.25_r8) then
          u10cubed(i)=1.474_r8*(u10cubed(i)+0.2_r8)**3.0_r8
       else if (u10cubed(i).le.3.70_r8) then
          u10cubed(i)=u10cubed(i)**3.41_r8
       endif
    enddo
...
!        cflx(:ncol,mm)=0.0_r8   ! we need to include dst_a1, dst_a3 to num_a1 and num_a3
       ...
       do i=1, sections
         if (Dg(i).ge.sst_sz_range_lo(m) .and. Dg(i).lt.sst_sz_range_hi(m)) then
           cflx(:ncol,mm)=cflx(:ncol,mm)+fi(:ncol,i)*ocnfrc(:ncol) & !*1.35_r8  &  !++ ag: scale 
