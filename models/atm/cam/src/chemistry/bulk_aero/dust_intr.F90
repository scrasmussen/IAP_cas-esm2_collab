...
character(cl) :: dust_emis = 'dust_emis' 
...
subroutine dust_initialize(dust_emis_fact_in, soil_erod_in, dust_emis_in)
   ...
   character(cl), intent(in) :: dust_emis_in
...
! number and mass species
    ...
    do mm=1,ndst
       ...
       ! multiply by soil erodibility factor
       ...
       else if (dust_emis == 'kok14') then
          if(spc_num_ndx(mm) > 0) then
             m=dust_idx1()+spc_num_ndx(mm)-spc_ndx(1)
             x_mton = 6.0_r8 / (pi*dns_aer*(dmt_vwr(mm)**3.0_r8))
             cflx(:ncol,m)=cflx(:ncol,m)*x_mton
          endif
          m=dust_idx1()+spc_ndx(mm)-spc_ndx(1)
          cflx(:ncol,m)=cflx(:ncol,m)
       else if (dust_emis == 'shao04') then
          if(spc_num_ndx(mm) > 0) then
             m=dust_idx1()+spc_num_ndx(mm)-spc_ndx(1)
             x_mton = 6.0_r8 / (pi*dns_aer*(dmt_vwr(mm)**3.0_r8))
             cflx(:ncol,m)=cflx(:ncol,m)*x_mton
          ...
          call endrun('dust_emis type not supported, currently only zender03, kok14 and shao04 are available')  
       endif
