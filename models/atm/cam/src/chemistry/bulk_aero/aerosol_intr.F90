...
subroutine aerosol_readnl(nlfile)
   ...
   call mpibcast(dust_emis    ,   len(dust_emis), mpichar, 0, mpicom)
   call mpibcast(seasalt_modes,                1, mpiint,  0, mpicom)
#endif
...
#if ( defined MODAL_AERO )
    call dust_initialize(dust_emis_fact, soil_erod, dust_emis)
    call progseasalts_initialize(seasalt_modes)
#else
    if ( dust ) then
       call dust_initialize(dust_emis_fact, soil_erod, dust_emis)
    endif
