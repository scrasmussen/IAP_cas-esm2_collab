...
!         if( k == lwaterptr_amode(m) ) go to 20
    ...
    if( eddy_scheme .eq. 'diag_TKE' .or. eddy_scheme .eq. 'MTKE' ) then    
  ...
  subroutine vertical_diffusion_tend( &
    ...
    use time_manager,       only : is_first_step, get_curr_calday !zmh
...
!
!zmh
    use cloud_fraction,   only : zmh_eddy_perts,zmh_eddy_pertf
...
!zmh
    real(r8),            intent(out)    :: tpert2(pcols)
    real(r8),            intent(out)    :: qpert2(pcols)
...
qpert2(:ncol) = 0.0
    ...
    tpert2(:ncol) =  min(3.0_r8   , state%frontgf(:ncol,1)*30.e3)*cos(state%lat(:ncol))   !30 km
    qpert2(:ncol) =  min(3.0e-3_r8, state%frontgf(:ncol,2)*30.e3)*cos(state%lat(:ncol))   !30 km
    
    calday = get_curr_calday()
    call zenith (calday, state%lat, state%lon, coszrs, ncol)

    !tpert2(:ncol) = tpert2(:ncol)* max(coszrs(:ncol),0.0)*landfrac(:ncol) &
    !               * (200.0/max(sgh30(:ncol,lchnk),200.))*zmh_eddy_perts

    !qpert2(:ncol) = qpert2(:ncol)* max(coszrs(:ncol),0.0)*landfrac(:ncol) & 
    !               * (200.0/max(sgh30(:ncol,lchnk),200.))*zmh_eddy_pertf


   do i = 1, ncol
    if(landfrac(i) .gt. 0.0 )then !land grids
     tpert2(i) = tpert2(i)* max(coszrs(i),0.0)*zmh_eddy_perts* (200.0/max(sgh30(i,lchnk),200.))
     qpert2(i) = qpert2(i)* max(coszrs(i),0.0)*zmh_eddy_pertf* (200.0/max(sgh30(i,lchnk),200.))
    else  !ocean grids
     tpert2(i) = tpert2(i)* zmh_eddy_perts*cos(state%lat(i))/2.0
     qpert2(i) = qpert2(i)* zmh_eddy_pertf*cos(state%lat(i))/2.0
    endif
   enddo
!==========================
#endif
...
!wxc
    ...
    if( (eddy_scheme .eq. 'diag_TKE' .or. eddy_scheme .eq. 'MTKE') .and. do_pseudocon_diff ) then
