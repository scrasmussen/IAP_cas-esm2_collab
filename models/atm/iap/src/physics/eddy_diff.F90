  ...
  use ppgrid,           only : pver , pcols !zmh 
  use cloud_fraction,   only : zmh_eddy_evhc,zmh_eddy_radf,zmh_eddy_perts,&
            zmh_eddy_pertf,zmh_eddy_cleng,zmh_eddy_tunl, zmh_eddy_b1, zmh_eddy_pertt
...
!zmh
  ...
  ! -------------------------------------- !
! =========================================
  character        :: sftype         = 'l'           ! Method for calculating saturation fraction
...
!  character(len=4), parameter :: choice_evhc    = 'maxi'        ! 'orig',   'ramp',   'maxi'   : recommended to be used with choice_radf 
  character(len=6) :: choice_evhc    = 'maxi'        ! 'orig',   'ramp',   'maxi'   : recommended to be used with choice_radf added 'mzhang'
...
!  character(len=6), parameter :: choice_radf    = 'maxi'        ! 'orig',   'ramp',   'maxi'   : recommended to be used with choice_evhc  
  character(len=6):: choice_radf    = 'maxi'        ! 'orig',   'ramp',   'maxi'   : recommended to be used with choice_evhc  'mzhang' added
  ...
  !zmh real(r8),         parameter :: cleng          =  3._r8        !  Order of 'leng' when choice_leng = 'origin' [ no unit ]
  real(r8)         :: cleng          =  3._r8        !  Order of 'leng' when choice_leng = 'origin' [ no unit ]
  character(len=6), parameter :: choice_tkes    = 'ibprod'      ! 'ibprod' (include tkes in computing bprod), 'ebprod'(exclude)
!X  character(len=6), parameter :: choice_tkes    = 'ebprod'      !zmh
...
!not default opt  real(r8),         parameter :: a1l            =   0.05_r8     ! zmh
  ...
  real(r8),         parameter :: a2l            =   15._r8      ! zmh
...
!  real(r8),         parameter :: ricrit         =   0.30_r8     ! zmh

  real(r8),         parameter :: ae             =   1._r8       ! TKE transport efficiency [no unit]
!X  real(r8),         parameter :: ae             =   0.5_r8  !zmh
!zmh
...
!   =======================================================
    !zmh_b1(:ncol) = b1
    zmh_b1(:ncol) = zmh_eddy_b1*(1. - min(tpert2(:ncol), 2.)/3.0 )  
! eddy dissipation time reduced
!   =======================================================
 ...
 ! ----------------------   
  if(zmh_eddy_pertt)then
    do i=1,ncol
     !do k=pver,2,-1
     do k=1,pver 
      if(zi(i,k+1) .gt. pblh(i)) cycle
       tfd(i,k)       = tfd(i,k)  + 0.0*tpert(i) + tpert2(i)
       qvfd(i,k)      = qvfd(i,k) + 0.0*qpert(i) + qpert2(i)
      enddo
    enddo
  endif
!zmh end
  ...
  subroutine sfdiag( pcols   , pver    , ncol    , qt      , ql      , sl      , &
    ...
    ! ----------------------- !
     ...
     if(zmh_eddy_evhc)choice_evhc = 'mzhang'
     if(zmh_eddy_radf)choice_radf = 'mzhang'
...
!              rdzs2 = 0.0   ! currently turned off
!              rdzs2 = 0.0   ! currently turned off
!          endif
!          dsldz    = dsldz - tpert2(i)*rdzs2 * cpair !zmh
!          dqtdz    = dqtdz - qpert2(i)*rdzs2         !zmh
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!    real(r8) :: alph4exs                             ! For extended stability function in the stable regime
    real(r8) :: alph4exs(pcols)    ! For extended stability function in the stable regime
    real(r8) :: ghmin                                 !   
...
!zmh made alph4exs an array
   do i = 1, ncol 
    if( ricrit .eq. 0.19_r8 ) then
        alph4exs(i) = alph4
        ghmin    = -3.5334_r8
    elseif( ricrit .gt. 0.19_r8 ) then
        alph4exs(i) = -2._r8 * zmh_b1(i) * alph2 / ( alph3 - 2._r8 * zmh_b1(i) * alph5 ) / ricrit
        ghmin    = -1.e10_r8
    else
...
!endif
    ...
    do i = 1, ncol
     ...
     ! tkes(i) = (zmh_b1(i)*vk*z(i,pver)*sprod(i,pver+1))**(2._r8/3._r8)
       tkes(i) = max(zmh_b1(i)*vk*z(i,pver)*(bprod(i,pver+1)+sprod(i,pver+1)), 1.e-7_r8)**(2._r8/3._r8)
       tkes(i) = min(tkes(i), tkemax)
       tke(i,pver+1)  = tkes(i)
       wcap(i,pver+1) = tkes(i)/zmh_b1(i)
                  ...
                  if( .not. in_CL ) then ! SRCL is not within the pre-existing CLs.
                     ...
                     if( k .lt. pver ) then
                       ...
                       ! gh = min(max(gh,-3.5334_r8),0.0233_r8)
                         ...
                         smcl(i,ncvnew) =  max(0._r8,(alph1 + alph2*gh)/(1._r8+alph3*gh)/(1._r8+alph4exs(i)*gh))
...
!zmh end
          ...
          do k = kt + 1, kb - 1 ! If 'kt = kb - 1', this loop will not be performed. 
             ...
             ! wstar3 =  wstar3 - leng(i,k)*sqrt(zmh_b1(i)*wcap(i,k))*shcl(i,ncv)*n2(i,k)* &
             !                    (z(i,k-1) - z(i,k))
          end do      
...
!zmh              elseif( choice_evhc .eq. 'maxi' ) then
          ...
          if( wstarent ) then
              ...
              trmq = 0.5_r8 * zmh_b1(i) * ( leng(i,kt)  / lbulk ) * ( radf * dzht + a1i * fact * wstar3 )
              ...
              if( noroot ) then ! Solve cubic for r
                  trma = 1._r8 - zmh_b1(i) * ( leng(i,kt) / lbulk ) * a1i * fact / ccrit**3
                  trma = max( trma, 0.5_r8 )  ! Limit entrainment enhancement of ebrk
                  trmp = trmp / trma 
                  trmq = 0.5_r8 * zmh_b1(i) * ( leng(i,kt) / lbulk ) * radf * dzht / trma
              end if   ! noroot
          ...
          wbrk(i,ncv) = ebrk(i,ncv)/zmh_b1(i)  
          ...
          do k = kb - 1, kt + 1, -1
             rcap = ( zmh_b1(i) * ae + wcap(i,k) / wbrk(i,ncv) ) / ( zmh_b1(i) * ae + 1._r8 )
          ...
          trmp = -zmh_b1(i) * ae / ( 1._r8 + zmh_b1(i) * ae )
          trmq = -(bprod(i,kt)+sprod(i,kt))*zmh_b1(i)*leng(i,kt)/(1._r8+zmh_b1(i)*ae)/(ebrk(i,ncv)**(3._r8/2._r8))
          ...
          if( kb .lt. pver + 1 ) then 
              ...
              if( kb .ne. ktblw ) then
                  ...
                  trmp = -zmh_b1(i)*ae/(1._r8+zmh_b1(i)*ae)
                  trmq = -(bprod(i,kb)+sprod(i,kb))*zmh_b1(i)*leng(i,kb)/(1._r8+zmh_b1(i)*ae)/(ebrk(i,ncv)**(3._r8/2._r8))
                  rcap = compute_cubic(0._r8,trmp,trmq)**2._r8
                  rcap = min( max(rcap,rcapmin), rcapmax )
                ...
                ! dzht5 : Half thickness of the highest layer of adjacent CL regime just below current CL. 
                  ...
                  trmp = -zmh_b1(i)*ae/(1._r8+zmh_b1(i)*ae)
                  trmq = -kentr*(s2(i,kb)-n2hb)*zmh_b1(i)*leng(i,kb)/(1._r8+zmh_b1(i)*ae)/(ebrk(i,ncv)**(3._r8/2._r8))
                  rcap = compute_cubic(0._r8,trmp,trmq)**2._r8
                  rcap = min( max(rcap,rcapmin), rcapmax )
             ...
             rcap = (zmh_b1(i)*ae + wcap(i,kb)/wbrk(i,ncv))/(zmh_b1(i)*ae + 1._r8)
             rcap = min( max(rcap,rcapmin), rcapmax )
             tke(i,kb) = ebrk(i,ncv) * rcap
             tke(i,kb) = min( tke(i,kb),tkemax )
...
!endif
       ...
       do k = 2, pver
          ...
          if( belongst(i,k) ) then
              ...
              trma = alph3*alph4exs(i)*ri(i,k) + 2._r8*zmh_b1(i)*(alph2-alph4exs(i)*alph5*ri(i,k))
              trmb = (alph3+alph4exs(i))*ri(i,k) + 2._r8*zmh_b1(i)*(-alph5*ri(i,k)+alph1)
              trmc = ri(i,k)
            ...
            ! gh = min(max(gh,-3.5334_r8),0.0233_r8)
              ...
              sm = max(0._r8,(alph1 + alph2*gh)/(1._r8+alph3*gh)/(1._r8+alph4exs(i)*gh))

              tke(i,k)   = zmh_b1(i)*(leng(i,k)**2)*(-sh*n2(i,k)+sm*s2(i,k))
              tke(i,k)   = min(tke(i,k),tkemax)
              wcap(i,k)  = tke(i,k)/zmh_b1(i)
       ...
       ! goto 888
         ...
         if( ( turbtype(i,k) .eq. 3 ) .or. ( turbtype(i,k) .eq. 4 ) .or. &
           ...
           ! gh   = min(max(gh,-3.5334_r8),0.0233_r8)
             ...
             tke_imsi = zmh_b1(i)*(leng_imsi**2)*(-sh*n2(i,k)+sm*s2(i,k))
 ...
 ! 888   continue 
     ...
     ! gh = min(max(gh,-0.28_r8),0.0233_r8)
       ...
       if( bprod(i,pver+1) .gt. 0._r8 ) then       
           ...
           sm_a(i,pver+1) = max(0._r8,(alph1+alph2*gh)/(1._r8+alph3*gh)/(1._r8+alph4exs(i)*gh))
       endif
       ...
       do k = 1, pver
          if( ri(i,k) .lt. 0._r8 ) then
              trma = alph3*alph4*ri(i,k) + 2._r8*zmh_b1(i)*(alph2-alph4*alph5*ri(i,k))
              trmb = (alph3+alph4)*ri(i,k) + 2._r8*zmh_b1(i)*(-alph5*ri(i,k)+alph1)
              trmc = ri(i,k)
              det  = max(trmb*trmb-4._r8*trma*trmc,0._r8)
              gh   = (-trmb + sqrt(det))/(2._r8*trma)
              gh   = min(max(gh,-3.5334_r8),0.0233_r8)
              ...
              if( ri(i,k) .gt. ricrit ) then
                  ...
                  gh_a(i,k) = gh
                  sh_a(i,k) = max(0._r8,alph5/(1._r8+alph3*gh))
                  sm_a(i,k) = max(0._r8,(alph1+alph2*gh)/(1._r8+alph3*gh)/(1._r8+alph4exs(i)*gh))
                  ri_a(i,k) = ri(i,k)
              endif
          endif
    ...
    do while( ncv .le. ncvfin(i) )
       ...
       if( kb .eq. pver+1 ) then
           ...
           if( bflxs(i) .gt. 0._r8 ) then
             ...
             ! gh    = min(max(gh,-0.28_r8),0.0233_r8)
               ...
               dw_surf    = (tkes(i)/zmh_b1(i))*z(i,pver) 
       ...
       if( kt .lt. kb - 1 ) then ! The case of non-SBCL.
         ...
         !                May need future generalization.
           ...
           trma  = alph3*alph4*ricll+2._r8*zmh_b1(i)*(alph2-alph4*alph5*ricll)
           trmb  = ricll*(alph3+alph4)+2._r8*zmh_b1(i)*(-alph5*ricll+alph1)
           trmc  = ricll
           det   = max(trmb*trmb-4._r8*trma*trmc,0._r8)
           gh    = (-trmb + sqrt(det))/2._r8/trma
         ! gh    = min(max(gh,-0.28_r8),0.0233_r8)
       ...
       l2n2 = -min(-l2n2, tkemax*lint/(zmh_b1(i)*sh))
       l2s2 =  min( l2s2, tkemax*lint/(zmh_b1(i)*sm))
     ...
     ! do while ( -dl2n2 .gt. (-rinc*dzinc*l2n2/(lint+(1._r8-rinc)*dzinc)) )  ! Merging test based on l2n2 
       ...
       ebrk(i,ncv) = zmh_b1(i)*wbrk(i,ncv)
