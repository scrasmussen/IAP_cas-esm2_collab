...
!xiex
   ...
   if (.not. (eddy_scheme .eq. 'HB' .or. eddy_scheme .eq. 'HBR' .or. eddy_scheme .eq. 'diag_TKE' .or. &
              eddy_scheme .eq. 'CLUBB_SGS' .or. eddy_scheme .eq. 'MTKE') ) then
      write(iulog,*)'phys_setopts: illegal value of eddy_scheme:', eddy_scheme
      call endrun('phys_setopts: illegal value of eddy_scheme')
   endif
   if ((microp_scheme /= 'MG' .and. microp_scheme /= 'M2M' .and. microp_scheme /= 'RK')) then
...
!xiex
   ...
   if (( eddy_scheme .eq. 'diag_TKE' .or. eddy_scheme .eq. 'MTKE' ) .and. ( microp_scheme .eq. 'RK' )) then
      write(iulog,*)'UW and MTKE PBL are not compatible with RK microphysics.  Quiting'
      call endrun('PBL and Microphysics schemes incompatible')
   endif
   
   ! Add a check to make sure CLUBB and MG are used together
   if ( do_clubb_sgs .and. ( microp_scheme .ne. 'MG' .and. microp_scheme .ne. 'M2M')) then
