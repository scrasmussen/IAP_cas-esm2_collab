...
!===========
        ...
        vtgw=0._r8
        ttgw=0._r8
!print*,"shape(state%pint)",shape(state%pint)
!print*,"pverp",pverp
!print*,"pver",pver
!call endrun()
        call gwdo_gsd(&
        u3d=state%u(:,pver:1:-1),v3d=state%v(:,pver:1:-1),t3d=state%t(:,pver:1:-1),&
        qv3d=state%q(:,pver:1:-1,1),p3d=state%pmid(:,pver:1:-1),p3di=state%pint(:,pverp:1:-1),&
...
!
         if (k .ge. kbl(i)) then
           ...
           !Jinbo Xie changed the rigorous ri number
...
if (k.ge.klowtop(i).and.zl(i,k).le.zl_hint(i)) then
l1=(bnv2(i,kp1)/velco(i,kp1)**2)!-(shr2_xjb(i,kp1)/velco(i,kp1))
l2=(bnv2(i,k)/velco(i,k)**2)!-(shr2_xjb(i,k)/velco(i,k))
