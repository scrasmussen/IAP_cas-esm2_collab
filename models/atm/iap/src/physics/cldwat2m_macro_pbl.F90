...
! zmh 
   use cloud_fraction, only: cldfrc_getparams, zmh_cldfrc_fice,&
                          cldfrc_implicit_PDF, macrop_scheme, microp_scheme,&
                          zmh_cloud_alst, zmh_cloud_aist, zmh_ice_dep, zmh_rhmaxi,&
                          zmh_cloud_u00l,zmh_cloud_u00h, a2min   
...
!  ZMH OPTIONS 
   ...
   integer                      :: iceopt       = 5        ! 5 is cam default
                                !the 2 parameters will be overwritten when namelist
                                !     macrop_scheme = 'ZMH_WXC'
!   logical                      :: zmh_ice_dep = .false. !.true.
                                !     overwritten by microp_scheme
...
!zmh ----------- for new cld scheme
 if(macrop_scheme .eq. 'ZMH_WXC' .and. zmh_cloud_alst)then
     CAMstfrac = 'iap' 
 endif
 if(macrop_scheme .eq. 'ZMH_WXC' .and. zmh_cloud_aist)then
     iceopt    = 6
 endif
! write(*,*)'macrop_scheme,CAMstfrac,iceopt'
! write(*,*)macrop_scheme,CAMstfrac,iceopt
...
!zmh
   if(microp_scheme .eq. 'M2M' .and. zmh_ice_dep)then
...
!      if( CAMstfrac ) then
  ...
  !    al_st(:ncol,k)  =  (1._r8-a_cu(:ncol,k))*al_st(:ncol,k)
      al_st(:ncol,k)  =  (1._r8-a_cu(:ncol,k))*al_st_nc(:ncol,k)  ! xwang
      a_st(:ncol,k)   =  max(al_st(:ncol,k),ai_st(:ncol,k))  

!---------------------------------------
!if(macrop_scheme .eq. 'ZMH_WXC')then
!     do i = 1,ncol
!      dum1 = zmh_ramp(T(i,k),193.16_r8, 80._r8)
!      a_st(i,k)   =  dum1*al_st(i,k)+(1.-dum1)*ai_st(i,k))  
!     enddo
!endif
...
!print*,'ql0     ',ql_0(:,k)
!print*,'a_ql    ',a_ql(:,k)
!print*,'a_ql_adj',a_ql_adj(:,k)
!print*,'c_ql    ',c_ql(:,k)
!print*,'qqw_prog',qqw_prog(:,k)
!endif
...
!   endif
   ...
   do i = 1, ncol
      ...
      ! ------------------------------------------------------------ !
!if(k .eq. 1)then
!    write(*,*)'mylchnk1',lchnk,k,i ,ql0
!    write(*,*)ql0,a_dc,ql_dc,a_sc,ql_sc
!endif
...
!!!MZMZ 
!if(k .eq. 1)then
!    write(*,*)'mylchnk',lchnk,k,i ,ql0
!    write(*,*)ql0_nc*1.0,qi0_nc*1.0,a_sc*1.0,a_dc*1.0, al0_st*1.0
!    write(*,*)ql0,a_dc,ql_dc,a_sc,ql_sc
!endif
!!
      ! ------------------------------------------------------------- !
...
!zmh
   ...
   rhmin = (U00-drh00)   !drh00 added here 
...
!           ------------
   ...
   rhmin = (u00(i)-drh00(i))   !drh00 added
...
!            a2 = max(a2, a2min)
            a2 = max(a2, a2min) + (1.0 - zmh_ramp(T,180.0_r8, 20._r8))*0.2
            a2 = (1.-a2)*esl/esi+ a2
            U = (qv+qi)/qsat * a2

            !a2 = zmh_cloud_u00h
            rhmini2 = (u00-drh00)    !drh00 added here
            rhmini2 = min(rhmini2,1.0_r8)

!temperature dependence of supersaturation
            a2 = zmh_ramp(T,243.16_r8, 30._r8)

             rhmaxi2 = a2 + (1.-a2)*zmh_rhmaxi 

            U = min(U ,rhmaxi2) !vertical displacement
            U =  max(U,0.01_r8)
!scheme
...
!            a2 = max(a2, a2min)
            ...
            !a2 = zmh_cloud_u00h
            rhmini2 = (U00(i) - drh00(i))  !drh00 here
            rhmini2 = min(rhmini2,1.0_r8)
