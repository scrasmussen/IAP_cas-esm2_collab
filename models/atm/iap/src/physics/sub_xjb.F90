...
#if 0
subroutine
OAgrid(terr,lon_terr,lat_terr,wt,n,lon_cen,lat_cen,theta_in,OAout)!,terr_out)        
use transform, only:CubedSphereABPFromRLL,CubedSphereXYZFromABP                                 
IMPLICIT NONE                                                                                   
integer,intent(in)  :: n                                                                        
real*8,intent(in) :: lon_terr(n),lat_terr(n)                                                    
real*8,intent(in) :: lon_cen,lat_cen                                                            
real*8,intent(in) :: wt(n),theta_in                                                             
real*8,intent(inout) :: terr(n)                                                                 
real*8,intent(out) :: OAout(2)                                                                  
                                                                                                
real*8 :: xhds,yhds,zhds,hds                                                                    
real*8 :: xbar,ybar,zbar                                                                        
real*8 :: xxterr,yyterr,zzterr                                                                  
real*8 :: OA_var,OAx_var,OAy_var,OAz_var                                                        
real*8 :: OAx1,OAy1,OAz1                                                                        
real*8 :: OAlat,OAlon                                                                           
real*8 :: ix2,iy2                                                                               
real*8 :: ix3,iy3                                                                               
integer :: ip2,ip3                                                                              
real*8 :: xx3,yy3,zz3                                                                           
real*8 :: area_target,rad                                                                       
!local
integer  :: i
        xhds=0.0
        yhds=0.0
        ...
        area_target=sum(wt)
        !
        call CubedSphereABPFromRLL(lon_cen*rad,lat_cen*rad,&
                                   ix3,iy3,ip3,.true.)
        call CubedSphereXYZFromABP(ix3,iy3,ip3,xx3,yy3,zz3)
        !
        do i=1,n
        rad=4.0*atan(1.0)/180.0
        call CubedSphereABPFromRLL(lon_terr(i)*rad,lat_terr(i)*rad,ix2,iy2,ip2,.true.)
        call CubedSphereXYZFromABP(ix2,iy2,ip2,xxterr,yyterr,zzterr)
        xhds=xhds+xxterr*terr(i)*wt(i)
        yhds=yhds+yyterr*terr(i)*wt(i)
        zhds=zhds+zzterr*terr(i)*wt(i)
        hds =hds+terr(i)*wt(i)
        !
        OA_var=OA_var+wt(i)/area_target&
                *((xxterr-xx3)**2+(yyterr-yy3)**2+(zzterr-zz3)**2)
        OAx_var=OAx_var+(wt(i)/area_target)*(xxterr-xx3)**2
        OAy_var=OAy_var+(wt(i)/area_target)*(yyterr-yy3)**2
        OAz_var=OAz_var+(wt(i)/area_target)*(zzterr-zz3)**2
        enddo
        !
        xbar=xhds/hds
        ybar=yhds/hds
        zbar=zhds/hds
        !
        OAx1=(xx3-xbar)/sqrt(OAx_var)
        OAy1=(yy3-ybar)/sqrt(OAy_var)
        OAz1=(zz3-zbar)/sqrt(OAz_var)
        !
        OAlat=-(OAx1*sin(lat_cen*rad)*cos(lon_cen*rad)&
               +OAy1*sin(lat_cen*rad)*sin(lon_cen*rad)&
               -OAz1*cos(lat_cen*rad))
        OAlon= -OAx1*sin(lon_cen*rad)&
               +OAy1*cos(lon_cen*rad)
        !
        OAout(1) = OAlon
        OAout(2) = OAlat

        where(abs(OAout)<.001.or.&
              abs(OAout).gt.1e+7) OAout=0.0
        !where(abs(OAout).gt.1)  OAout=1.0
        where(OAout.ne.OAout)   OAout=0.0
end subroutine OAgrid
#endif
...
!#if 0
        ...
        !print*,"zzterr,zz3,OAz1",zzterr,zz3,OAz1
...
!#endif
!===================================
subroutine OAorig_old(terr,ntarget,ncube,n,jall,weights_lgr_index_all,weights_eul_index_all1,weights_eul_index_all2,weights_eul_index_all3,weights_all,landfrac_target,target_center_lon,target_center_lat,lon_terr,lat_terr,area_target,oa_target)
!use shr_kind_mod, only: r8 => shr_kind_r8
IMPLICIT NONE
integer ,intent(in)  :: ncube,ntarget,n,jall,weights_lgr_index_all(jall),weights_eul_index_all1(jall),weights_eul_index_all2(jall),weights_eul_index_all3(jall)
real(r8),intent(in)  :: weights_all(jall,1),terr(n)
real(r8),intent(in)  :: landfrac_target(ntarget),lon_terr(n),lat_terr(n),area_target(ntarget)
real(r8),intent(in)  :: target_center_lat(ntarget),target_center_lon(ntarget)
real(r8),intent(out) :: oa_target(ntarget,4)
!local
integer(r8) :: count,i,ix,iy,ip,ii
real(r8)    :: wt,Nu(4,ntarget),Nd(4,ntarget),terr_if,terr_avg(ntarget)
        ...
        terr_if=0._r8
        if (terr(ii).GT.terr_avg(i)) terr_if=1.
        !OA1,W
        if (lon_terr(ii).LT.target_center_lon(i)) then
        Nu(1,i)=Nu(1,i)+wt*terr_if
        else
        Nd(1,i)=Nd(1,i)+wt*terr_if
        endif
        !OA2,S
        if (lat_terr(ii).LT.target_center_lat(i)) then
        Nu(2,i)=Nu(2,i)+wt*terr_if
        else
        Nd(2,i)=Nd(2,i)+wt*terr_if
        endif
        !OA3,SW
        if (lon_terr(ii).LT.target_center_lon(i).and.&
            lat_terr(ii).LT.target_center_lat(i)) then
        Nu(3,i)=Nu(3,i)+wt*terr_if
        elseif (lon_terr(ii).LT.target_center_lon(i).and.&
                lat_terr(ii).GT.target_center_lat(i).or.&
                lon_terr(ii).LT.target_center_lon(i).and.&
                lat_terr(ii).GT.target_center_lat(i)) then
        Nu(3,i)=Nu(3,i)+0.5_r8*wt*terr_if
        Nd(3,i)=Nd(3,i)+0.5_r8*wt*terr_if
        else
        Nd(3,i)=Nd(3,i)+wt*terr_if
        endif
        !OA4,NW
        if (lon_terr(ii).LT.target_center_lon(i).and.&
            lat_terr(ii).GT.target_center_lat(i)) then
        Nu(4,i)=Nu(4,i)+wt*terr_if
        elseif (lon_terr(ii).GT.target_center_lon(i).and.&
                lat_terr(ii).GT.target_center_lat(i).or.&
                lon_terr(ii).LT.target_center_lon(i).and.&
                lat_terr(ii).LT.target_center_lat(i)) then
        Nu(4,i)=Nu(4,i)+0.5_r8*wt*terr_if
        Nd(4,i)=Nd(4,i)+0.5_r8*wt*terr_if
        else
        Nd(4,i)=Nd(4,i)+wt*terr_if
        endif
        enddo
        
        !output
        do i=1,ntarget
        oa_target(i,:)=(Nu(:,i)-Nd(:,i))/(Nu(:,i)+Nd(:,i))
        enddo

        !reverse to follow turning from 0 to 135
        !we do not follow the original order 
        !in the journal
        do i=1,ntarget
        oa_target(i,1)=(Nu(1,i)-Nd(1,i))/(Nu(1,i)+Nd(1,i))
        oa_target(i,2)=(Nu(3,i)-Nd(3,i))/(Nu(3,i)+Nd(3,i))
        oa_target(i,3)=(Nu(2,i)-Nd(2,i))/(Nu(2,i)+Nd(2,i))
        oa_target(i,4)=0._r8-(Nu(4,i)-Nd(4,i))/(Nu(4,i)+Nd(4,i))
        enddo
...
!real(r8),intent(out) :: terrout(6,ntarget,indexb)
...
!dx=rearth*cos(rlat)*(2._r8*pi/256._r8),dy=rearth*(pi/(128._r8-1._r8))
        !wo need degrees input rather than radians
        dx=(2._r8*180/real(nlon,kind=r8))*cos(lat_cen*rad)
        dy=(      180/real(nlat-1,kind=r8))
        ...
        do j=1,nvar_dir
        theta1(j)=(180._r8/nvar_dir)*real(j-1,kind=r8)
                do i=1,ntarget
...
!terrout(5,i,:index_b(3,i))=lon_terr(indexii_b(:index_b(3,i),i))
!terrout(6,i,:index_b(3,i))=lat_terr(indexii_b(:index_b(3,i),i))
        enddo
