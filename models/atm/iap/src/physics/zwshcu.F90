...
!   real(r8) :: uwshcu_rpen =  unset_r8!  For penetrative entrainment efficiency
   ...
   namelist /zwshcu_nl/       zmh_shallow_lf     , zmh_shallow_ent        ,zmh_ent0 , &
         zmh_shallow_pertt  , zmh_shallow_pertw  , zmh_shallow_stochastic           , &
         zmh_ufrcd          , zmh_ufrc0          , zmh_ufrcz                        , &
         shallow_closure_scheme                  , zmh_shallow_tau1                 , &
         zmh_shallow_tau2   , zmh_rkm            , uwshcu_rpen                      , &
         zmh_shallow_qpertf , zmh_shallow_qperth , zmh_shallow_tpertf               , &
         zmh_shallow_tpertb , zmh_shallow_xyt    , zmh_shallow_xyq                  , &
         zmh_shallow_tpertmax,zmh_shallow_qpertmax, zmh_shallow_fwcrit
   !-----------------------------------------------------------------------------
...
!   if (uwshcu_rpen == unset_r8) then
!     uwshcu_rpen = 10.0  !!!!!!!!!!!!! hard set
!   endif

    print*, ''
    print*, '/zwshcu_nl/'
    print*,     'zmh_shallow_lf     , zmh_shallow_ent        ,zmh_ent0' 
    print*,      zmh_shallow_lf     , zmh_shallow_ent        ,zmh_ent0 
    print*,     'zmh_shallow_pertt  , zmh_shallow_pertw  , zmh_shallow_stochastic           '
    print*,      zmh_shallow_pertt  , zmh_shallow_pertw  , zmh_shallow_stochastic           
    print*,     'zmh_ufrcd          , zmh_ufrc0          , zmh_ufrcz                        '
    print*,      zmh_ufrcd          , zmh_ufrc0          , zmh_ufrcz                        
    print*,     'shallow_closure_scheme                  , zmh_shallow_tau1                 '
    print*,      shallow_closure_scheme                  , zmh_shallow_tau1                 
    print*,     'zmh_shallow_tau2   , zmh_rkm            , uwshcu_rpen                      '
    print*,      zmh_shallow_tau2   , zmh_rkm            , uwshcu_rpen                      
    print*,     'zmh_shallow_qpertf , zmh_shallow_qperth , zmh_shallow_tpertf               '
    print*,      zmh_shallow_qpertf , zmh_shallow_qperth , zmh_shallow_tpertf               
    print*,     'zmh_shallow_tpertb , zmh_shallow_xyt    , zmh_shallow_xyq                  '
    print*,      zmh_shallow_tpertb , zmh_shallow_xyt    , zmh_shallow_xyq                  
    print*,     'zmh_shallow_tpertmax,zmh_shallow_qpertmax, zmh_shallow_fwcrit'
    print*,      zmh_shallow_tpertmax,zmh_shallow_qpertmax,zmh_shallow_fwcrit
   ...
   call mpibcast(zmh_shallow_lf,             1, mpilog,  0, mpicom)
   call mpibcast(zmh_shallow_ent,            1, mpilog,  0, mpicom)
   call mpibcast(zmh_ent0,                   1, mpir8,  0, mpicom)
   call mpibcast(zmh_shallow_pertt,          1, mpilog,  0, mpicom)
   call mpibcast(zmh_shallow_pertw,          1, mpilog,  0, mpicom)
   call mpibcast(zmh_shallow_stochastic,     1, mpilog,  0, mpicom)
   call mpibcast(zmh_ufrcd,                  1, mpir8,  0, mpicom)
   call mpibcast(zmh_ufrc0,                  1, mpir8,  0, mpicom)
   call mpibcast(zmh_ufrcz,                  1, mpir8,  0, mpicom)
   call mpibcast(shallow_closure_scheme,     1, mpichar,  0, mpicom)
   call mpibcast(zmh_shallow_tau1,           1, mpir8,  0, mpicom)
   call mpibcast(zmh_shallow_tau2,           1, mpir8,  0, mpicom)
   call mpibcast(zmh_rkm,                    1, mpir8,  0, mpicom)
   call mpibcast(uwshcu_rpen,                1, mpir8,  0, mpicom)

   call mpibcast(zmh_shallow_qpertf,         1, mpilog,  0, mpicom)
   call mpibcast(zmh_shallow_qperth,         1, mpilog,  0, mpicom)
   call mpibcast(zmh_shallow_tpertf,         1, mpilog,  0, mpicom)
   call mpibcast(zmh_shallow_tpertb,         1, mpilog,  0, mpicom)
   call mpibcast(zmh_shallow_xyt,            1, mpir8,  0, mpicom)
   call mpibcast(zmh_shallow_xyq,            1, mpir8,  0, mpicom)
   call mpibcast(zmh_shallow_tpertmax,       1, mpir8,  0, mpicom)
   call mpibcast(zmh_shallow_qpertmax,       1, mpir8,  0, mpicom)

   call mpibcast(zmh_shallow_fwcrit,         1, mpir8,  0, mpicom)
...
!zmh
                                 omega_inv    , tpert2   , qpert2   , gradt, gradq, &
                                 landfrac     , lat      , lon      , massflx_sh, &
...
!zmh
    ...
    real(r8), intent(in)    :: lat(mix)
    real(r8), intent(in)    :: lon(mix)
    real(r8), intent(in)    :: landfrac(mix)
    real(r8)                :: tpert(mix)
    real(r8)                :: qpert(mix)
    real(r8), intent(in)    :: gradt(mix)
    real(r8), intent(in)    :: gradq(mix)
...
!wxc zmh
    ...
    integer                 :: i                        !  Vertical index for local fields [ no ] 
...
! surface perturbations
  ...
  tpert(:)   = tpert2(:)
    ...
    call zenith (calday, lat, lon, coszrs, mix)
  ! q front
