...
!zmh
  ...
  subroutine macrop_driver_tend(                             &
             ...
             sst, shflx, lhflx, zdu,tpert2,qpert2,gradt, gradq,drhlims,       &                   !wxc &!zmh
  ...
  use cloud_fraction,   only: cldfrc, cldfrc_implicit_PDF, macrop_scheme, &
                              zmh_cloud_dp,zmh_cloud_sh, zmh_cloud_pbl, zmh_cloud_alst,&
                              zmh_cloud_u00l,zmh_cloud_u00h, zmh_cloud_rh00, &
                              sh1, sh2, dp1, dp2, wxc_u00tab, &  !wxc
                              wxc_u00, wxc_u00_v2, wxc_u00_v3, zmh_u00, zmh_u00_v2 !zmh
...
!     call zmh_u00(ncol, pver,state1%lat, state1%pmid, u00)

!     call wxc_u00(ncol, state1%lat,state1%pmid, u00)
     call wxc_u00_v2(ncol,pver, state1%lat(1:ncol),state1%pmid(1:ncol,1:pver), u00(1:ncol,1:pver))
!     call wxc_u00_v3(ncol,pver, state1%lat(1:ncol),state1%pmid(1:ncol,1:pver), u00(1:ncol,1:pver))

!     call zmh_u00_v2(ncol, pver,state1%lat(1:ncol), state1%pmid(1:ncol,:), u00(1:ncol,:))
     drh00(:,:) = 0.0_r8

! do i=1,ncol
!   print*,'u00--'
!   print*,i,state1%lat(i)*180./3.1416
!   print*,i,u00(i,:)
! enddo


  do i=1,ncol
     dum1 = state1%lat(i)*180./3.1416
     j0 = int((90._r8 + dum1)/1.+0.5) + 1
     j0 = min(j0,181)
    do k=1,pver
        dum2 = state1%pmid(i,k)/state1%pint(i,pverp)*1.0e5
        k0 = int( dum2/2000.+0.5)+1
        k0 = min(k0,51)
...
! enddo
    ...
    dum1 = zmh_ramp(state1%pmid(i,k)/state1%pint(i,pverp)*1.0e5,4.5e4_r8,1.0e4_r8)    
             ! gradual ramping 0 above 450mb, 1 below 550 mb
    u00(i,k) = u00(i,k)*((1.-dum1)+dum1*zmh_cloud_u00l)

    dum2 = 1.- dum1
             ! gradual ramping, 0 below 550, 1 above 450
    u00(i,k) = u00(i,k)*((1.-dum2)+dum2*zmh_cloud_u00h)

    u00(i,k) = min(u00(i,k),0.98_r8)

      dumt =  min(3.0_r8   , gradt(i)*30.e3)*cos(state1%lat(i))   !30 km
      dumq =  min(3.0e-3_r8, gradq(i)*30.e3)*cos(state1%lat(i))   !30 km

      dum1 = relhum(i,k)/max(state1%q(i,k,1),1.0e-3)
      dum2 = 2.5e6/4.61e2*relhum(i,k)/state1%t(i,k)/state1%t(i,k)
         
      drh00(i,k) = 0.5*(dum1*dumq + dum2*dumt) * zmh_cloud_rh00 !perturbation in RH
      drh00(i,k) = min(drh00(i,k), 0.3)
    enddo 
    enddo 

! print*,'myrelhum',relhum
! print*,'gradt',gradt*100.e3
! print*,'gradq',gradq*100.e6
! print*,'zmh_cloud_rh00',zmh_cloud_rh00
! print*,'drh00',drh00
...
!write(*,*)'pblht',pblht
 ...
 if(macrop_scheme .eq. 'ZMH_WXC' .and. zmh_cloud_pbl)then !pbl clouds
! ==============================
      call cldfrc_implicit_PDF( lchnk, ncol, pbuf, state1%zm, shflx, lhflx,&
          pblht, state1%zi, state1%pdel, state1%lat, state1%lon, &    
                   state1%pmid, t_inout, qv_inout, ql_inout, qi_inout, &
