...
!
      ...
      !LiuJP's team
      real (kind=dbl_kind) :: &
         alphaw          ! temporary variable for compute direct albedo of open water
      ...
      !-----------------------------------------------------------------
      ! Ocean  albedo
      ! For now, evaluate open water albedos according to Schramm et al, 1997.
      !-----------------------------------------------------------------
      do ij = 1, icells
         i = indxi(ij)
         j = indxj(ij)
         
         alphaw = p026/(coszen(i,j,iblk)**c17 + p065) + &
                 p015*(coszen(i,j,iblk)-p1)*(coszen(i,j,iblk)-p5)*(coszen(i,j,iblk)-c1)
         alvdr_ocn(i,j,iblk) = alphaw + p0082
         alidr_ocn(i,j,iblk) = alphaw - p007
         alvdf_ocn(i,j,iblk) = albocn
         alidf_ocn(i,j,iblk) = albocn
