...
!
      ...
      !LiuJP's team
      ! visible (0.25-0.69 miu-m), IR1 (0.69-1.19 miu-m), IR2 (1.19-2.38 miu-m), IR3 (2.38-4.00 miu-m)
      real (kind=dbl_kind), parameter :: &
         frcvdr_clr  = 0.26_dbl_kind,  & ! frac of incoming sw in vis direct band,clear sky
         frcvdf_clr  = 0.24_dbl_kind,  & ! frac of incoming sw in vis diffuse band,clear sky
         frci1dr_clr = 0.2_dbl_kind,   & ! frac of incoming sw in near IR1 direct band,clear sky
         frci1df_clr = 0.146_dbl_kind, & ! frac of incoming sw in near IR1 diffuse band,clear sky
         frci2dr_clr = 0.08_dbl_kind,  & ! frac of incoming sw in near IR2 direct band,clear sky
         frci2df_clr = 0.064_dbl_kind, & ! frac of incoming sw in near IR2 diffuse band,clear sky
         frci3dr_clr = 0.006_dbl_kind, & ! frac of incoming sw in near IR3 direct band,clear sky
         frci3df_clr = 0.004_dbl_kind, & ! frac of incoming sw in near IR3 diffuse band,clear sky

         frcvdr_cld  = 0.32_dbl_kind,  & ! frac of incoming sw in vis direct band,cloudy sky
         frcvdf_cld  = 0.262_dbl_kind, & ! frac of incoming sw in vis diffuse band,cloudy sky
         frci1dr_cld = 0.2_dbl_kind,   & ! frac of incoming sw in near IR1 direct band,cloudy sky
         frci1df_cld = 0.143_dbl_kind, & ! frac of incoming sw in near IR1 diffuse band,cloudy sky
         frci2dr_cld = 0.044_dbl_kind, & ! frac of incoming sw in near IR2 direct band,cloudy sky
         frci2df_cld = 0.03_dbl_kind,  & ! frac of incoming sw in near IR2 diffuse band,cloudy sky
         frci3dr_cld = 0.0006_dbl_kind,& ! frac of incoming sw in near IR3 direct band,cloudy sky
         frci3df_cld = 0.0004_dbl_kind    ! frac of incoming sw in near IR3 diffuse band,cloudy sky
...
!
    ...
    !-------------------------------------------------------------------
      ...
      call ice_HaloUpdate (swidf,             halo_info, &
                           field_loc_center,  field_type_scalar)
      !LiuJP's team
      call ice_HaloUpdate (cldf,             halo_info, &
                           field_loc_center,  field_type_scalar)
      call ice_HaloUpdate (swi1dr,             halo_info, &
                           field_loc_center,  field_type_scalar)
      call ice_HaloUpdate (swi1df,             halo_info, &
                           field_loc_center,  field_type_scalar)
      call ice_HaloUpdate (swi2dr,             halo_info, &
                           field_loc_center,  field_type_scalar)
      call ice_HaloUpdate (swi2df,             halo_info, &
                           field_loc_center,  field_type_scalar)
      call ice_HaloUpdate (swi3dr,             halo_info, &
                           field_loc_center,  field_type_scalar)
      call ice_HaloUpdate (swi3df,             halo_info, &
                           field_loc_center,  field_type_scalar)
...
!
      ...
      do i = ilo, ihi
        ...
        ! divide shortwave into spectral bands
         ...
         if( cldf(i,j) > p1 ) then ! cloudy sky

            swvdr(i,j)  = fsw(i,j)*frcvdr_cld        ! visible direct
            swvdf(i,j)  = fsw(i,j)*frcvdf_cld        ! visible diffuse
            swi1dr(i,j) = fsw(i,j)*frci1dr_cld        ! near IR1 direct
            swi1df(i,j) = fsw(i,j)*frci1df_cld        ! near IR1 diffuse
            swi2dr(i,j) = fsw(i,j)*frci2dr_cld        ! near IR2 direct
            swi2df(i,j) = fsw(i,j)*frci2df_cld        ! near IR2 diffuse
            swi3dr(i,j) = fsw(i,j)*frci3dr_cld        ! near IR3 direct
            swi3df(i,j) = fsw(i,j)*frci3df_cld        ! near IR3 diffuse
            swidr(i,j)  = swi1dr(i,j) + swi2dr(i,j) + swi3dr(i,j)
            swidf(i,j)  = swi1df(i,j) + swi2df(i,j) + swi3df(i,j)

         else                       ! clear sky

            swvdr(i,j) = fsw(i,j)*frcvdr_clr        ! visible direct
            swvdf(i,j) = fsw(i,j)*frcvdf_clr        ! visible diffuse
            swi1dr(i,j) = fsw(i,j)*frci1dr_clr        ! near IR1 direct
            swi1df(i,j) = fsw(i,j)*frci1df_clr        ! near IR1 diffuse
            swi2dr(i,j) = fsw(i,j)*frci2dr_clr        ! near IR2 direct
            swi2df(i,j) = fsw(i,j)*frci2df_clr        ! near IR2 diffuse
            swi3dr(i,j) = fsw(i,j)*frci3dr_clr        ! near IR3 direct
            swi3df(i,j) = fsw(i,j)*frci3df_clr        ! near IR3 diffuse
            swidr(i,j)  = swi1dr(i,j) + swi2dr(i,j) + swi3dr(i,j)
            swidf(i,j)  = swi1df(i,j) + swi2df(i,j) + swi3df(i,j)
