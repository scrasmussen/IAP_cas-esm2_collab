         ...
         do n = 1, ncat
            ...
            if (trim(shortwave) == 'dEdd') then ! delta Eddington
               ...
               call shortwave_ccsm3(nx_block,     ny_block,           &
                              ...
                              trcrn(:,:,nt_Tsfc,n,iblk),              &
                              swvdr(:,:,  iblk), swvdf(:,:,  iblk),   &
                              swi1dr(:,:,  iblk),swi1df(:,:,  iblk),  &
                              swi2dr(:,:,  iblk),swi2df(:,:,  iblk),  &
                              swi3dr(:,:,  iblk),swi3df(:,:,  iblk),  &
                              alvdrn(:,:,n,iblk),alidrn(:,:,n,iblk),  &
                              alvdfn(:,:,n,iblk),alidfn(:,:,n,iblk),  &
                              fswsfcn(:,:,n,iblk),fswintn(:,:,n,iblk),&
                              fswthrun(:,:,n,iblk),                   &
                              ...
                              coszen(:,:,  iblk), cldf(:,:,iblk),     &
                              apondn(:,:,n,iblk), hpondn(:,:,n,iblk), &
                              fsn(:,:,n,iblk))

            endif ! trim(shortwave)
...
!
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block), &
         ...
         swi1dr   , & ! sw down, near IR1, direct  (W/m^2)
         swi1df   , & ! sw down, near IR1, diffuse (W/m^2)
         swi2dr   , & ! sw down, near IR2, direct  (W/m^2)
         swi2df   , & ! sw down, near IR2, diffuse (W/m^2)
         swi3dr   , & ! sw down, near IR3, direct  (W/m^2)
         swi3df       ! sw down, near IR3, diffuse (W/m^2)

      !LiuJP's team
      real (kind=dbl_kind), dimension(nx_block,ny_block), &
         intent(in) :: &
         iage,  & ! ice age
         coszen,& ! cosine of zenith
         cldf,  & ! cloud fraction
         apondn,& ! pond fraction
         hpondn,& ! pond depth
         fsn      ! snow fraction
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block,nslyr), &
         intent(out) :: &
         Sswabs       ! SW absorbed in particular snow layer (W m-2)
...
!
      ...
      !   alidfns    ! near-ir, diffuse, snow  (fraction)
         alvdrni,  & ! visible, direct, ice     (fraction)
         ali1drni, & ! near-ir1, direct, ice    (fraction)
         ali2drni, & ! near-ir2, direct, ice    (fraction)
         ali3drni, & ! near-ir3, direct, ice    (fraction)
         alvdfni,  & ! visible, diffuse, ice    (fraction)
         ali1dfni, & ! near-ir1, diffuse, ice   (fraction)
         ali2dfni, & ! near-ir2, diffuse, ice   (fraction)
         ali3dfni, & ! near-ir3, diffuse, ice   (fraction)
         alvdrns,  & ! visible, direct, snow    (fraction)
         ali1drns, & ! near-ir1, direct, snow   (fraction)
         ali2drns, & ! near-ir2, direct, snow   (fraction)
         ali3drns, & ! near-ir3, direct, snow   (fraction)
         alvdfns,  & ! visible, diffuse, snow   (fraction)
         ali1dfns, & ! near-ir1, diffuse, snow  (fraction)
         ali2dfns, & ! near-ir2, diffuse, snow  (fraction)
         ali3dfns, & ! near-ir3, diffuse, snow  (fraction)
         alvdrnp,  & ! visible, direct, pond    (fraction)
         ali1drnp, & ! near-ir1, direct, pond   (fraction)
         ali2drnp, & ! near-ir2, direct, pond   (fraction)
         ali3drnp, & ! near-ir3, direct, pond   (fraction)
         alvdfnp,  &  ! visible, diffuse, pond  (fraction)
         ali1dfnp, & ! near-ir1, diffuse, pond  (fraction)
         ali2dfnp, & ! near-ir2, diffuse, pond  (fraction)
         ali3dfnp    ! near-ir3, diffuse, piond (fraction)
      ...
      if (trim(albedo_type) == 'constant') then
         call constant_albedos (nx_block,   ny_block,                      &
                                ...
                                !alvdrns,    alidrns,                      &
                                !alvdfns,    alidfns,                      &
                                alvdrni,    ali1drni,  ali2drni, ali3drni, &
                                alvdfni,    ali1dfni,  ali2dfni, ali3dfni, &
                                alvdrns,    ali1drns,  ali2drns, ali3drns, &
                                alvdfns,    ali1dfns,  ali2dfns, ali3dfns, &
                                alvdrn,     alidrn,                        &
                                alvdfn,     alidfn,                        &
                                albin,      albsn)
      else ! default
         call compute_albedos (nx_block,   ny_block,                       &
                               ...
                               alvdrns,    ali1drns,  ali2drns, ali3drns,  &
                               alvdfns,    ali1dfns,  ali2dfns, ali3dfns,  &
                               alvdrnp,    ali1drnp,  ali2drnp, ali3drnp,  &
                               alvdfnp,    ali1dfnp,  ali2dfnp, ali3dfnp,  &
                               alvdrn,     alidrn,                         &
                               alvdfn,     alidfn,                         &
                               albin,      albsn,                          &
                               iage,                                       &
      ...
      call absorbed_solar  (nx_block,   ny_block,                      &
                            ...
                            !fswsfc,     fswint,                       &
                            !fswthru,    Iswabs)
                            swi1dr,     swi2dr,   swi3dr,              &
                            swi1df,     swi2df,   swi3df,              &
                            alvdrni,    ali1drni,  ali2drni, ali3drni, &
                            alvdfni,    ali1dfni,  ali2dfni, ali3dfni, &
                            alvdrns,    ali1drns,  ali2drns, ali3drns, &
                            alvdfns,    ali1dfns,  ali2dfns, ali3dfns, &
...
! !USES:
      ...
      use ice_forcing, &
      only: frci1dr_clr, frci1df_clr,                           &
            frci2dr_clr, frci2df_clr, frci3dr_clr, frci3df_clr, &
            frci1dr_cld, frci1df_cld,                           &
            frci2dr_cld, frci2df_cld, frci3dr_cld, frci3df_cld
      use ice_calendar, only: days_per_year
      use ice_therm_vertical, only: hs_min
!
! !INPUT/OUTPUT PARAMETERS:
!
      integer (kind=int_kind), intent(in) :: &
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block), &
         ...
         ali1dfnp, & ! near-ir1, diffuse, pond  (fraction)
         ali2dfnp, & ! near-ir2, diffuse, pond  (fraction)
         ali3dfnp, & ! near-ir3, diffuse, pond  (fraction)
         alvdrn  , & ! visible, direct, avg     (fraction)
         alidrn  , & ! near-ir, direct, avg     (fraction)
         alvdfn  , & ! visible, diffuse, avg    (fraction)
         alidfn  , & ! near-ir, diffuse, avg    (fraction)
...
!
      ...
      real (kind=dbl_kind) :: &
         ...
         !LiuJP's team
         hs1 ,    & ! snow thickness span (m) 
         hs2 ,    & ! snow thickness span (m)  
         alidrni, & ! average directive near-ir(1,2,3) albedos of ice, temporary variables  
         alidrns, & ! average directive near-ir(1,2,3) albedos of snow, temporary variables
         alidfni, & ! average diffuse near-ir(1,2,3) albedos of ice, temporary variables  
         alidfns    ! average diffuse near-ir(1,2,3) albedos of snow, temporary variables 
...
!ocl novrec      !Fujitsu
      ...
      !LiuJP's team
      secs_per_year = real(days_per_year,kind=dbl_kind) * secday
      ...
      do ij = 1, icells
         ...
         ! bare ice,  ice age dependence
         ! following Schramm et al., 1997
         if ( iage(i,j) >= secs_per_year .OR. .NOT.tr_iage) then 

           ! bare MY ice diffuse albedos
           alvdfni(i,j)  = 0.778_dbl_kind
           ali1dfni(i,j) = 0.443_dbl_kind
           ali2dfni(i,j) = 0.055_dbl_kind
           ali3dfni(i,j) = 0.036_dbl_kind

           ! bare MY ice direct albedos
           alvdrni(i,j)  = alvdfni(i,j)
           ali1drni(i,j) = ali1dfni(i,j)
           ali2drni(i,j) = ali2dfni(i,j)
           ali3drni(i,j) = ali3dfni(i,j)

           if(apondn(i,j) > c0)then
             ! meltponds above MY diffuse albedos
             if(cldf(i,j) > p1)then ! cloudy
               alvdfnp(i,j)  = 0.342_dbl_kind + exp(-20.512_dbl_kind*hpondn(i,j) - 0.83_dbl_kind)
             else  ! clear 
               alvdfnp(i,j)  = 0.342_dbl_kind + exp(-9.0_dbl_kind*hpondn(i,j) - 0.999_dbl_kind)
             endif ! cld
             ali1dfnp(i,j) = 0.020_dbl_kind + exp(-14.187_dbl_kind*hpondn(i,j) - 0.86_dbl_kind)
             ali2dfnp(i,j) = 0.033_dbl_kind + exp(-2.58_dbl_kind*hpondn(i,j) - 3.82_dbl_kind)
             ali3dfnp(i,j) = 0.030_dbl_kind

             ! meltponds above MY direct albedos
             alvdrnp(i,j) =alvdfnp(i,j)      
             ali1drnp(i,j)=ali1dfnp(i,j)
             ali2drnp(i,j)=ali2dfnp(i,j)
             ali3drnp(i,j)=ali3dfnp(i,j)
           endif ! apond

         else ! First-year ice

           ! bare FY ice, thickness dependence
           ! following Schramm et al., 1997
           if( hi >= c1 ) then
             ! bare FY ice diffuse albedos
             alvdfni(i,j)  = 0.760_dbl_kind 
             ali1dfni(i,j) = 0.247_dbl_kind
             ali2dfni(i,j) = 0.055_dbl_kind 
             ali3dfni(i,j) = 0.036_dbl_kind 

             ! bare FY ice direct albedos
             alvdrni(i,j)  = alvdfni(i,j)
             ali1drni(i,j) = ali1dfni(i,j)
             ali2drni(i,j) = ali2dfni(i,j)
             ali3drni(i,j) = ali3dfni(i,j)
             ...
             ! bare FY ice diffuse albedos
             alvdfni(i,j)  = 0.760_dbl_kind + 0.140_dbl_kind * log(hi)
             ali1dfni(i,j) = 0.247_dbl_kind + 0.029_dbl_kind * log(hi)
             ali2dfni(i,j) = 0.055_dbl_kind 
             ali3dfni(i,j) = 0.036_dbl_kind 
           ...
           if(apondn(i,j) > c0) then
             ...
             ! meltponds above FY diffuse albedos
             if(hi >= c1)then
               if(cldf(i,j) > p1)then ! cloudy
                 alvdfnp(i,j)  = 0.251_dbl_kind + exp(-17.134_dbl_kind*hpondn(i,j) - 0.675_dbl_kind)
               else ! clear
                 alvdfnp(i,j)  = 0.251_dbl_kind + exp(-11.0_dbl_kind*hpondn(i,j) - 1.174_dbl_kind)
               endif ! cldf
               ali1dfnp(i,j)   = 0.01_dbl_kind + exp(-17.31_dbl_kind*hpondn(i,j) - 1.439_dbl_kind)
             else ! hi<c1
               if(cldf(i,j) > p1)then ! cloudy
                 alvdfnp(i,j)  = 0.25_dbl_kind + exp(-17.134_dbl_kind*hpondn(i,j))*(alvdfni(i,j) - 0.251_dbl_kind)
               else ! clear
                 alvdfnp(i,j)  = 0.251_dbl_kind + exp(-11.0_dbl_kind*hpondn(i,j) - 1.174_dbl_kind)
               endif ! cldf
               ali1dfnp(i,j)   = 0.01_dbl_kind + exp(-17.31_dbl_kind*hpondn(i,j))*(ali1dfni(i,j) - 0.01_dbl_kind)
             endif ! hi
             ali2dfnp(i,j) = 0.033_dbl_kind + exp(-2.28_dbl_kind*hpondn(i,j) - 3.82_dbl_kind)
             ali3dfnp(i,j) = 0.03_dbl_kind

             ! meltponds above FY direct albedos
             alvdrnp(i,j)  = alvdfnp(i,j)
             ali1drnp(i,j) = ali1dfnp(i,j)
             ali2drnp(i,j) = ali2dfnp(i,j)
             ali3drnp(i,j) = ali3dfnp(i,j)
         ...
         if (hs > puny) then
           ...
           if( c0 - Tsfcn(i,j) < c1 ) then   ! melting snow

             ! melting snow diffuse albedos
             alvdfns(i,j)  = 0.871_dbl_kind 
             ali1dfns(i,j) = 0.702_dbl_kind
             ali2dfns(i,j) = 0.079_dbl_kind
             ali3dfns(i,j) = 0.010_dbl_kind

             if(hs < p1) then
               alvdfns(i,j)  = alvdfns(i,j) - (alvdfns(i,j) - alvdfni(i,j)) * hs1 / hs2   
               ali1dfns(i,j) = ali1dfns(i,j) - (ali1dfns(i,j) - ali1dfni(i,j)) * hs1 / hs2 
               ali2dfns(i,j) = ali2dfns(i,j) - (ali2dfns(i,j) - ali2dfni(i,j)) * hs1 / hs2 
               ali3dfns(i,j) = ali2dfns(i,j) - (ali3dfns(i,j) - ali3dfni(i,j)) * hs1 / hs2 
             endif    

             ! melting snow direct albedos
             alvdrns(i,j)  = alvdfns(i,j)
             ali1drns(i,j) = ali1dfns(i,j)
             ali2drns(i,j) = ali2dfns(i,j)
             ali3drns(i,j) = ali3dfns(i,j)

           else                              ! dry snow

             ! dry snow diffuse albedos
             alvdfns(i,j)  = 0.975_dbl_kind 
             ali1dfns(i,j) = 0.832_dbl_kind
             ali2dfns(i,j) = 0.250_dbl_kind
             ali3dfns(i,j) = 0.025_dbl_kind      

             ! dry snow direct albedos
             alvdrns(i,j)  = 0.980_dbl_kind - 0.008_dbl_kind * coszen(i,j)
             ali1drns(i,j) = 0.902_dbl_kind - 0.116_dbl_kind * coszen(i,j)
             ali2drns(i,j) = 0.384_dbl_kind - 0.222_dbl_kind * coszen(i,j)
             ali3drns(i,j) = 0.053_dbl_kind - 0.047_dbl_kind * coszen(i,j)
             ...
             if(hs < p1) then
               ...
               ! !dry snow direct albedos
               alvdrns(i,j)  = alvdrns(i,j)  - (alvdrns(i,j)  - alvdrni(i,j))  * hs1 / hs2   
               ali1drns(i,j) = ali1drns(i,j) - (ali1drns(i,j) - ali1drni(i,j)) * hs1 / hs2 
               ali2drns(i,j) = ali2drns(i,j) - (ali2drns(i,j) - ali2drni(i,j)) * hs1 / hs2 
               ali3drns(i,j) = ali2drns(i,j) - (ali3drns(i,j) - ali3drni(i,j)) * hs1 / hs2 
             endif   ! hs < p1
         ...
         ! combine ice and snow albedos (for coupler)
         alvdfn(i,j) = alvdfni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + alvdfnp(i,j)*apondn(i,j)&
                       + alvdfns(i,j)*fsn(i,j)
         alvdrn(i,j) = alvdrni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + alvdrnp(i,j)*apondn(i,j)&
                       + alvdrns(i,j)*fsn(i,j)

         if(cldf(i,j) > p1)then ! cloudy

           alidfn(i,j) = &
           (frci1df_cld*(ali1dfni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali1dfnp(i,j)*apondn(i,j) + ali1dfns(i,j)*fsn(i,j)) + &
            frci2df_cld*(ali2dfni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali2dfnp(i,j)*apondn(i,j) + ali2dfns(i,j)*fsn(i,j)) + &
            frci3df_cld*(ali3dfni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali3dfnp(i,j)*apondn(i,j) + ali3dfns(i,j)*fsn(i,j))) / & 
            (frci1df_cld + frci2df_cld + frci3df_cld)                 
           alidrn(i,j) = &
           (frci1dr_cld*(ali1drni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali1drnp(i,j)*apondn(i,j) + ali1drns(i,j)*fsn(i,j)) + &
            frci2dr_cld*(ali2drni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali2drnp(i,j)*apondn(i,j) + ali2drns(i,j)*fsn(i,j)) + &
            frci3dr_cld*(ali3drni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali3drnp(i,j)*apondn(i,j) + ali3drns(i,j)*fsn(i,j))) / & 
            (frci1dr_cld + frci2dr_cld + frci3dr_cld)

         else

           alidfn(i,j) = &
           (frci1df_clr*(ali1dfni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali1dfnp(i,j)*apondn(i,j) + ali1dfns(i,j)*fsn(i,j)) + &
            frci2df_clr*(ali2dfni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali2dfnp(i,j)*apondn(i,j) + ali2dfns(i,j)*fsn(i,j)) + &
            frci3df_clr*(ali3dfni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali3dfnp(i,j)*apondn(i,j) + ali3dfns(i,j)*fsn(i,j))) / &
            (frci1df_clr + frci2df_clr + frci3df_clr)                 
           alidrn(i,j) = &
           (frci1dr_clr*(ali1drni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali1drnp(i,j)*apondn(i,j) + ali1drns(i,j)*fsn(i,j)) + &
            frci2dr_clr*(ali2drni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali2drnp(i,j)*apondn(i,j) + ali2drns(i,j)*fsn(i,j)) + &
            frci3dr_clr*(ali3drni(i,j)*(c1-fsn(i,j)-apondn(i,j)) + ali3drnp(i,j)*apondn(i,j) + ali3drns(i,j)*fsn(i,j))) / &
            (frci1dr_clr + frci2dr_clr + frci3dr_clr)
         ...
         if(cldf(i,j) > p1) then ! cloudy
             ...
             alidrni = (frci1dr_cld*ali1drni(i,j) + frci2dr_cld*ali2drni(i,j) + &
                       frci3dr_cld*ali3drni(i,j)) / (frci1dr_cld + frci2dr_cld + frci3dr_cld)
             alidrns = (frci1dr_cld*ali1drns(i,j) + frci2dr_cld*ali2drns(i,j) + &
                       frci3dr_cld*ali3drns(i,j)) / (frci1dr_cld + frci2dr_cld + frci3dr_cld)
             alidfni = (frci1df_cld*ali1dfni(i,j) + frci2df_cld*ali2dfni(i,j) + &
                       frci3df_cld*ali3dfni(i,j)) / (frci1df_cld + frci2df_cld + frci3df_cld)
             alidfns = (frci1df_cld*ali1dfns(i,j) + frci2df_cld*ali2dfns(i,j) + &
                       frci3df_cld*ali3dfns(i,j)) / (frci1df_cld + frci2df_cld + frci3df_cld)

         else

             alidrni = (frci1dr_clr*ali1drni(i,j) + frci2dr_clr*ali2drni(i,j) + &
                       frci3dr_clr*ali3drni(i,j)) / (frci1dr_clr + frci2dr_clr + frci3dr_clr)
             alidrns = (frci1dr_clr*ali1drns(i,j) + frci2dr_clr*ali2drns(i,j) + &
                       frci3dr_clr*ali3drns(i,j)) / (frci1dr_clr + frci2dr_clr + frci3dr_clr)
             alidfni = (frci1df_clr*ali1dfni(i,j) + frci2df_clr*ali2dfni(i,j) + &
                       frci3df_clr*ali3dfni(i,j)) / (frci1df_clr + frci2df_clr + frci3df_clr)
             alidfns = (frci1df_clr*ali1dfns(i,j) + frci2df_clr*ali2dfns(i,j) + &
                       frci3df_clr*ali3dfns(i,j)) / (frci1df_clr + frci2df_clr + frci3df_clr)
         endif ! cldf			  

         ! save ice and snow albedos (for history)
         albin(i,j) = awtvdr*alvdrni(i,j) + awtidr*alidrni &
                    + awtvdf*alvdfni(i,j) + awtidf*alidfni 
         albsn(i,j) = awtvdr*alvdrns(i,j) + awtidr*alidrns &
                    + awtvdf*alvdfns(i,j) + awtidf*alidfns 
...
!ocl novrec      !Fujitsu
      do ij = 1, icells
         ...
         ! direct albedos (same as diffuse for now)
         alvdrni(i,j)  = alvdrn(i,j)
         ali1drni(i,j) = alidrn(i,j)
         ali2drni(i,j) = alidrn(i,j)
         ali3drni(i,j) = alidrn(i,j)
         alvdrns(i,j)  = alvdrn(i,j)
         ali1drns(i,j) = alidrn(i,j)
         ali2drns(i,j) = alidrn(i,j)
         ali3drns(i,j) = alidrn(i,j)
         alvdfni(i,j)  = alvdfn(i,j)
         ali1dfni(i,j) = alidfn(i,j)
         ali2dfni(i,j) = alidfn(i,j)
         ali3dfni(i,j) = alidfn(i,j)
         alvdfns(i,j)  = alvdfn(i,j)
         ali1dfns(i,j) = alidfn(i,j)
         ali2dfns(i,j) = alidfn(i,j)
         ali3dfns(i,j) = alidfn(i,j)
...
!
        ...
        use ice_therm_vertical, only: heat_capacity, hs_min
!
! !INPUT/OUTPUT PARAMETERS:
!
      integer (kind=int_kind), intent(in) :: &
...
!
      real (kind=dbl_kind), parameter :: &
         i0vis = 0.70_dbl_kind,  & ! fraction of penetrating solar rad into ice (visible)
         !LiuJP's team
         i0nir = 0.70_dbl_kind,  & ! fraction of penetrating solar rad into ice (near infrared)
         s0vis = 0.50_dbl_kind,  & ! fraction of penetrating solar rad into snow (visible)
         s0nir = 0.50_dbl_kind,  & ! fraction of penetrating solar rad into snow (near infrared)
         p0vis = 0.80_dbl_kind,  & ! fraction of penetrating solar rad into pond (visible)
         p0nir = 0.80_dbl_kind     ! fraction of penetrating solar rad into pond (near infrared)
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block) :: &
         ...
         fswpenv     , & ! visible, SW penetrating beneath ice surface  (W/m^2)
         fswpeni     , & ! near IR, SW penetrating beneath ice surface  (W/m^2)
         fswpeni1    , & ! near IR1, SW penetrating beneath ice surface (W/m^2)
         fswpeni2    , & ! near IR2, SW penetrating beneath ice surface (W/m^2)
         fswpeni3    , & ! near IR3, SW penetrating beneath ice surface (W/m^2)
         fswpenvni   , & ! visible, SW penetrating beneath bare ice surface       (W/m^2)
         fswpenini   , & ! near IR, SW penetrating beneath bare ice surface       (W/m^2)
         fswpeni1ni  , & ! near IR1, SW penetrating beneath bare ice surface      (W/m^2)
         fswpeni2ni  , & ! near IR2, SW penetrating beneath bare ice surface      (W/m^2)
         fswpeni3ni  , & ! near IR3, SW penetrating beneath bare ice surface      (W/m^2)
         fswpenvns   , & ! visible, SW penetrating beneath snow surface      (W/m^2)
         fswpenins   , & ! near IR, SW penetrating beneath snow surface      (W/m^2)
         fswpeni1ns  , & ! near IR1, SW penetrating beneath snow surface     (W/m^2)
         fswpeni2ns  , & ! near IR2, SW penetrating beneath snow surface     (W/m^2)
         fswpeni3ns  , & ! near IR3, SW penetrating beneath snow surface     (W/m^2)
         trantopv    , & ! visible, transmitted frac of penetrating SW at top layer 
         trantopi1   , & ! near IR1, transmitted frac of penetrating SW at top layer 
         trantopi2   , & ! near IR2, transmitted frac of penetrating SW at top layer
         trantopi3   , & ! near IR3, transmitted frac of penetrating SW at top layer
         tranbotv    , & ! visible, transmitted frac of penetrating SW at bot layer 
         tranboti1   , & ! near IR1, transmitted frac of penetrating SW at bot layer 
         tranboti2   , & ! near IR2, transmitted frac of penetrating SW at bot layer 
         tranboti3       ! near IR3, transmitted frac of penetrating SW at bot layer 
      ...
      real (kind=dbl_kind) :: &
         ...
         fswpenvdrni , & ! penetrating SW, vis, direct, bare ice
         fswpeni1drni, & ! penetrating SW, near IR1 direct, bare ice
         fswpeni2drni, & ! penetrating SW, near IR2 direct, bare ice
         fswpeni3drni, & ! penetrating SW, near IR3 direct, bare ice
         fswpenvdfni , & ! penetrating SW, vis, diffuse, bare ice
         fswpeni1dfni, & ! penetrating SW, near IR1 diffuse, bare ice
         fswpeni2dfni, & ! penetrating SW, near IR2 diffuse, bare ice
         fswpeni3dfni, & ! penetrating SW, near IR3 diffuse, bare ice
         fswpenvdrns , & ! penetrating SW, vis, direct, snow
         fswpeni1drns, & ! penetrating SW, near IR1 direct, snow
         fswpeni2drns, & ! penetrating SW, near IR2 direct, snow
         fswpeni3drns, & ! penetrating SW, near IR3 direct, snow
         fswpenvdfns , & ! penetrating SW, vis, diffuse, snow
         fswpeni1dfns, & ! penetrating SW, near IR1 diffuse, snow
         fswpeni2dfns, & ! penetrating SW, near IR2 diffuse, snow
         fswpeni3dfns, & ! penetrating SW, near IR3 diffuse, snow
         hi          , & ! ice thickness (m)
         hs          , & ! snow thickness (m)
...
!ocl novrec      !Fujitsu
      ...
      !-----------------------------------------------------------------
         ...
         swabsv  = swvdr(i,j) * ( (c1-alvdrni(i,j))*(c1-fsn(i,j)-apondn(i,j))   &
                                + (c1-alvdrns(i,j))*fsn(i,j)                    &
                                + (c1-alvdrnp(i,j))*apondn(i,j) )               &
                 + swvdf(i,j) * ( (c1-alvdfni(i,j))*(c1-fsn(i,j)-apondn(i,j))   &
                                + (c1-alvdfns(i,j))*fsn(i,j)                    &
                                + (c1-alvdfnp(i,j))*apondn(i,j) )

         swabsi  = swi1dr(i,j) * ( (c1-ali1drni(i,j))*(c1-fsn(i,j)-apondn(i,j)) &
                                 + (c1-ali1drns(i,j))*fsn(i,j)                  &
                                 + (c1-ali1drnp(i,j))*apondn(i,j) )             &
                 + swi2dr(i,j) * ( (c1-ali2drni(i,j))*(c1-fsn(i,j)-apondn(i,j)) &
                                 + (c1-ali2drns(i,j))*fsn(i,j)                  &
                                 + (c1-ali2drnp(i,j))*apondn(i,j) )             &
                 + swi3dr(i,j) * ( (c1-ali3drni(i,j))*(c1-fsn(i,j)-apondn(i,j)) &
                                 + (c1-ali3drns(i,j))*fsn(i,j)                  &
                                 + (c1-ali3drnp(i,j))*apondn(i,j) )             &
                 + swi1df(i,j) * ( (c1-ali1dfni(i,j))*(c1-fsn(i,j)-apondn(i,j)) &
                                 + (c1-ali1dfns(i,j))*fsn(i,j)                  &
                                 + (c1-ali1dfnp(i,j))*apondn(i,j) )             &
                 + swi2df(i,j) * ( (c1-ali2dfni(i,j))*(c1-fsn(i,j)-apondn(i,j)) &
                                 + (c1-ali2dfns(i,j))*fsn(i,j)                  &
                                 + (c1-ali2dfnp(i,j))*apondn(i,j) )             &
                 + swi3df(i,j) * ( (c1-ali3dfni(i,j))*(c1-fsn(i,j)-apondn(i,j)) &
                                 + (c1-ali3dfns(i,j))*fsn(i,j)                  &
                                 + (c1-ali3dfnp(i,j))*apondn(i,j) )
      ...
      !-----------------------------------------------------------------  
         ...
         fswpenvdrni = swvdr(i,j) * (c1-alvdrni(i,j)) * (1-fsn(i,j)) * i0vis
         fswpenvdfni = swvdf(i,j) * (c1-alvdfni(i,j)) * (1-fsn(i,j)) * i0vis
         fswpenvni(i,j) = fswpenvdrni + fswpenvdfni

         fswpeni1drni = swi1dr(i,j) * (c1-ali1drni(i,j)) * (1-fsn(i,j)) * i0vis
         fswpeni1dfni = swi1df(i,j) * (c1-ali1dfni(i,j)) * (1-fsn(i,j)) * i0vis
         fswpeni1ni(i,j) = fswpeni1drni + fswpeni1dfni

         fswpeni2drni = swi2dr(i,j) * (c1-ali2drni(i,j)) * (1-fsn(i,j)) * i0vis
         fswpeni2dfni = swi2df(i,j) * (c1-ali2dfni(i,j)) * (1-fsn(i,j)) * i0vis
         fswpeni2ni(i,j) = fswpeni2drni + fswpeni2dfni

         fswpeni3drni = swi3dr(i,j) * (c1-ali3drni(i,j)) * (1-fsn(i,j)) * i0vis
         fswpeni3dfni = swi3df(i,j) * (c1-ali3dfni(i,j)) * (1-fsn(i,j)) * i0vis
         fswpeni3ni(i,j) = fswpeni3drni + fswpeni3dfni

         fswpenini(i,j) = fswpeni1ni(i,j) + fswpeni2ni(i,j) + fswpeni3ni(i,j)

      !-----------------------------------------------------------------
      ! Only penetrating ice
      !----------------------------------------------------------------- 

         fswpenv(i,j)  = fswpenvni(i,j)
         fswpeni1(i,j) = fswpeni1ni(i,j)
         fswpeni2(i,j) = fswpeni2ni(i,j)
         fswpeni3(i,j) = fswpeni3ni(i,j)
 
         fswpen(i,j) = fswpenv(i,j) + fswpeni1(i,j) + fswpeni2(i,j) + fswpeni3(i,j)
      ...
      !-----------------------------------------------------------------          
         ...
         do k=1,nilyr
            tranbotv(i,j)  = exp(-kappavni  * hilyr * real(k,kind=dbl_kind))
            tranboti1(i,j) = exp(-kappai1ni * hilyr * real(k,kind=dbl_kind))
            tranboti2(i,j) = exp(-kappai2ni * hilyr * real(k,kind=dbl_kind))
            tranboti3(i,j) = exp(-kappai3ni * hilyr * real(k,kind=dbl_kind))

            Iswabs(i,j,k) = fswpenv(i,j)  * (trantopv(i,j)-tranbotv(i,j))    &
                          + fswpeni1(i,j) * (trantopi1(i,j)-tranboti1(i,j))  &
                          + fswpeni2(i,j) * (trantopi2(i,j)-tranboti2(i,j))  &
                          + fswpeni3(i,j) * (trantopi3(i,j)-tranboti3(i,j)) 
           
            ! bottom of layer k = top of layer k+1
            trantopv(i,j)  = tranbotv(i,j)
            trantopi1(i,j) = tranboti1(i,j)
            trantopi2(i,j) = tranboti2(i,j)
            trantopi3(i,j) = tranboti3(i,j)

         enddo       ! nilyr

         fswthru(i,j) = fswpenv(i,j)  * tranbotv(i,j)     &
                      + fswpeni1(i,j) * tranboti1(i,j)    &
                      + fswpeni2(i,j) * tranboti2(i,j)    &
                      + fswpeni3(i,j) * tranboti3(i,j)
