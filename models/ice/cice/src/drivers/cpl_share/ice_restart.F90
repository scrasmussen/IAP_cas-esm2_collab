...
!
      ...
      !LiuJP's team
      call ice_write(nu_dump,0,cldf,'ruf8',diag)
      call ice_write(nu_dump,0,swi1dr,'ruf8',diag)
      call ice_write(nu_dump,0,swi1df,'ruf8',diag)
      call ice_write(nu_dump,0,swi2dr,'ruf8',diag)
      call ice_write(nu_dump,0,swi2df,'ruf8',diag)
      call ice_write(nu_dump,0,swi3dr,'ruf8',diag)
      call ice_write(nu_dump,0,swi3df,'ruf8',diag)
...
!
      ...
      !LiuJP's team
      call define_rest_field(File,'cldf',dims)
      call define_rest_field(File,'swi1dr',dims)
      call define_rest_field(File,'swi1df',dims)
      call define_rest_field(File,'swi2dr',dims)
      call define_rest_field(File,'swi2df',dims)
      call define_rest_field(File,'swi3dr',dims)
      call define_rest_field(File,'swi3df',dims)
      ...
      !LiuJP's team
      status = pio_inq_varid(File,'cldf',varid)
      call pio_write_darray(File, varid, iodesc2d, cldf, status, fillval=c0)

      status = pio_inq_varid(File,'swi1dr',varid)
      call pio_write_darray(File, varid, iodesc2d, swi1dr, status, fillval=c0)

      status = pio_inq_varid(File,'swi1df',varid)
      call pio_write_darray(File, varid, iodesc2d, swi1df, status, fillval=c0)

      status = pio_inq_varid(File,'swi2dr',varid)
      call pio_write_darray(File, varid, iodesc2d, swi2dr, status, fillval=c0)

      status = pio_inq_varid(File,'swi2df',varid)
      call pio_write_darray(File, varid, iodesc2d, swi2df, status, fillval=c0)

      status = pio_inq_varid(File,'swi3dr',varid)
      call pio_write_darray(File, varid, iodesc2d, swi3dr, status, fillval=c0)

      status = pio_inq_varid(File,'swi3df',varid)
      call pio_write_darray(File, varid, iodesc2d, swi3df, status, fillval=c0)
...
!
      ...
      if (trim(resttype) == 'new') then
         ...
         call ice_read(nu_restart,0,swidf,'ruf8',diag, &
                       field_loc_center, field_type_scalar)
         !LiuJP's team
         call ice_read(nu_restart,0,cldf,'ruf8',diag, &
                       field_loc_center, field_type_scalar)
         call ice_read(nu_restart,0,swi1dr,'ruf8',diag, &
                       field_loc_center, field_type_scalar)
         call ice_read(nu_restart,0,swi1df,'ruf8',diag, &
                       field_loc_center, field_type_scalar)
         call ice_read(nu_restart,0,swi2dr,'ruf8',diag, &
                       field_loc_center, field_type_scalar)
         call ice_read(nu_restart,0,swi2df,'ruf8',diag, &
                       field_loc_center, field_type_scalar)
         call ice_read(nu_restart,0,swi3dr,'ruf8',diag, &
                       field_loc_center, field_type_scalar)
         call ice_read(nu_restart,0,swi3df,'ruf8',diag, &
                       field_loc_center, field_type_scalar)
      end if
...
!
      ...
      !LiuJP's team
      status = pio_inq_varid(File,'cldf',varid)
      call pio_read_darray(File, varid, iodesc2d, cldf, status)
      call ice_HaloUpdate (cldf,               halo_info,     &
                           field_loc_center,    field_type_scalar)

      status = pio_inq_varid(File,'swi1dr',varid)
      call pio_read_darray(File, varid, iodesc2d, swi1dr, status)
      call ice_HaloUpdate (swi1dr,               halo_info,     &
                           field_loc_center,    field_type_scalar)

      status = pio_inq_varid(File,'swi1df',varid)
      call pio_read_darray(File, varid, iodesc2d, swi1df, status)
      call ice_HaloUpdate (swi1df,               halo_info,     &
                           field_loc_center,    field_type_scalar)

      status = pio_inq_varid(File,'swi2dr',varid)
      call pio_read_darray(File, varid, iodesc2d, swi2dr, status)
      call ice_HaloUpdate (swi2dr,               halo_info,     &
                           field_loc_center,    field_type_scalar)

      status = pio_inq_varid(File,'swi2df',varid)
      call pio_read_darray(File, varid, iodesc2d, swi2df, status)
      call ice_HaloUpdate (swi2df,               halo_info,     &
                           field_loc_center,    field_type_scalar)

      status = pio_inq_varid(File,'swi3dr',varid)
      call pio_read_darray(File, varid, iodesc2d, swi3dr, status)
      call ice_HaloUpdate (swi3dr,               halo_info,     &
                           field_loc_center,    field_type_scalar)

      status = pio_inq_varid(File,'swi3df',varid)
      call pio_read_darray(File, varid, iodesc2d, swi3df, status)
