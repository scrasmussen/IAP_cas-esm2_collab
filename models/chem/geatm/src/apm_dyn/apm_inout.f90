...
subroutine wr_apm_var( myid,lapm,nest,nzz,sx,ex,sy,ey,ne  &
 ...
 print*,'write apm data :',myid,sx(ne),ex(ne),sy(ne),ey(ne)
 write(345+ne) myid,sx(ne),ex(ne),sy(ne),ey(ne)
 ...
 loop_sulf : do is=1,NSO4
   do k=1,nzz
     ...
     do i=sx(ne),ex(ne)
        ...
        write(345+ne) apm_sulf(iapm+ixy)
        !kk=kk+1
        !print*,k,j,i
        !print*,kk
     enddo
 ...
 loop_salt : do is=1,NSEA
   do k=1,nzz
     ...
     do i=sx(ne),ex(ne)
        ...
        write(345+ne) apm_salt(iapm+ixy)
     enddo
     enddo
   enddo
 enddo loop_salt
 ...
 loop_dust : do is=1,NDSTB
   do k=1,nzz
     ...
     do i=sx(ne),ex(ne)
        ...
        write(345+ne) apm_dust(iapm+ixy)
     enddo
     enddo
   enddo
 enddo loop_dust
 ...
 loop_bcoc : do is=1,NBCOCT
   do k=1,nzz
     ...
     do i=sx(ne),ex(ne)
        ...
        write(345+ne) apm_bcoc(iapm+ixy)
     enddo
     enddo
   enddo
 enddo loop_bcoc
...
! enddo
 ...
 do k=1,nzz
   ...
   do i=sx(ne),ex(ne)
      ixy=(ex(ne)-sx(ne)+3)*(j -sy(ne)+1)+i-sx(ne)+1
      iapm=ip3mem(k,ne)
      write(345+ne) msltsulf(iapm+ixy),mdstsulf(iapm+ixy) &
 ...
 loop_type : do is=1,5
   do k=1,nzz
     ...
     do i=sx(ne),ex(ne)
        ...
        write(345+ne) number_ccn1(iapm+ixy),number_ccn2(iapm+ixy) &
