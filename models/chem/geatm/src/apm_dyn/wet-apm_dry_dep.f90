   ...
   do k = 1,nzz-1
     ...
     do i = sx(ne),ex(ne)
        ...
        do is=1,NSO4
          ...
          if(rw_sulf(iapm+ixy).le.0) then
            print*,is,k,j,i
            print*,rw_sulf(iapm+ixy)
            stop 'sulf radius erro'
           endif
        enddo

        do is=1,NSEA
          iapm=ip_salt(k,is,ne)
          if(rw_salt(iapm+ixy).le.0) then
            print*,is,k,j,i
            print*,rw_salt(iapm+ixy)
            stop 'salt radius erro'
          endif
        enddo

        do is=1,NDSTB
          iapm=ip_dust(k,is,ne)
          if(rw_dust(iapm+ixy).le.0) then
            print*,is,k,j,i
            print*,rw_dust(iapm+ixy)
            stop 'dust radius erro'
          endif
        enddo

        do imode=1,2
          iapm=ipmode_bcoc(k,imode,ne)
          if(refw_bc(iapm+ixy).le.0.or.refw_bc(iapm+ixy).le.0) then
            print*,is,k,j,i
            print*,refw_bc(iapm+ixy),refw_bc(iapm+ixy)
            stop 'bcoc radius erro'
          endif
        enddo
...
if(lcoated_dyn) then
   ...
   do i = sx(ne),ex(ne)
      ixy = (ex(ne)-sx(ne)+3)*(j -sy(ne)+1)+i-sx(ne)+1
      do is=1,NBCOCT
        iapm=ipmode_bcoc(k,ridx(is),ne)
        if(flagbcoc(is).eq.'bc') then
          radius(is)=refw_bc(iapm+ixy)
        elseif(flagbcoc(is).eq.'oc') then
          radius(is)=refw_oc(iapm+ixy) 
        endif
