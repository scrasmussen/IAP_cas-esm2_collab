...
print*,'kkbcemit',m1p*sum(bcoc_unit_em2(:,ridx(is)))*ug2kg
enddo
stop
...
! apm_bcoc : ug/m3
         ...
         if(is.eq.1) then     ! FF(mode1) QW BC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & mass_frac*bc_emit_ff(i03+ixy)*fbc_qw*m1p*sum(bcoc_unit_em2(:,1))*ug2kg*dt/dz1d
         elseif(is.eq.2) then ! BB(mode2) QW BC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & mass_frac*bc_emit_bb(i03+ixy)*fbc_qw*m1p*sum(bcoc_unit_em2(:,2))*ug2kg*dt/dz1d
         elseif(is.eq.3) then ! FF(mode1) QW OC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & mass_frac*oc_emit_ff(i03+ixy)*foc_qw*m1p*sum(bcoc_unit_em2(:,1))*ug2kg*dt/dz1d
         elseif(is.eq.4) then ! BB(mode2) QW OC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & mass_frac*oc_emit_bb(i03+ixy)*foc_qw*m1p*sum(bcoc_unit_em2(:,2))*ug2kg*dt/dz1d
         elseif(is.eq.5) then ! FF(mode1) SW BC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & mass_frac*bc_emit_ff(i03+ixy)*fbc_sw*m1p*sum(bcoc_unit_em2(:,1))*ug2kg*dt/dz1d
         elseif(is.eq.6) then ! BB(mode2) SW BC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & mass_frac*bc_emit_bb(i03+ixy)*fbc_sw*m1p*sum(bcoc_unit_em2(:,2))*ug2kg*dt/dz1d
         elseif(is.eq.7) then ! FF(mode1) SW OC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & mass_frac*oc_emit_ff(i03+ixy)*foc_sw*m1p*sum(bcoc_unit_em2(:,1))*ug2kg*dt/dz1d
         elseif(is.eq.8) then ! BB(mode2) SW OC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & mass_frac*oc_emit_bb(i03+ixy)*foc_sw*m1p*sum(bcoc_unit_em2(:,2))*ug2kg*dt/dz1d
         endif
