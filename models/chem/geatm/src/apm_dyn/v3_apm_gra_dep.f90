...
!          con(k)=10.0
        ...
        do k=1,nzz
          ratiocore(k)=amin1(amax1(concore2(k)/concore1(k),0.0),10.0)
        enddo
...
!           msltsulf(i03+ixy)=msltsulf(i03+ixy)*ratiocore(k)
        enddo
        endif

        if(i.eq.33.and.j.eq.33.and..true.) then
!          print*,'sea-coating'
          do k=1,20
!          print*,ratiocore(k),sum(consp(:,k))
          enddo
!          stop
