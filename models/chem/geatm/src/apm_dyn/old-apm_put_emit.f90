...
real,parameter :: frac95=0.95
real,parameter :: frac05=0.05

real :: emission

!=========================
!-------------------------
if(ls2sulf.and.lem_sulf) then ! add primary sulfate to apm sulfate particle
!print*,'put sulf emit'
   ...
   do k=1,nzz-1
      ...
      do i = sx(ne),ex(ne)
         ...
         apm_sulf(iapm+ixy) = apm_sulf(iapm+ixy)+ &
                            & sulf_emit(i03+ixy)*sulf_unit_em2(is,2)*dt/dz1d
      enddo
      enddo
   enddo
 enddo loop_so4
   ...
   do k=1,nzz-1
      ...
      do i = sx(ne),ex(ne)
         ...
         apm_salt(iapm+ixy) = apm_salt(iapm+ixy)+ &
                            & dt*salt_emit(iapm+ixy)/dz1d*const
      enddo
      enddo
   enddo
 enddo loop_salt
   ...
   do k=1,nzz-1
      ...
      do i = sx(ne),ex(ne)
         ...
         apm_dust(iapm+ixy) = apm_dust(iapm+ixy)+ &
                            & dust_emit(i03+ixy)*fdstbin(is)*dt/dz1d*const
      enddo
      enddo
   enddo
 enddo loop_dust
...
! apm_bcoc : ug/m3
         ...
         if(is.eq.1) then     ! FF(mode1) QW BC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & bc_emit(i03+ixy)*fbc_qw*m1p*sum(bcoc_unit_em2(:,1))*ug2kg*dt/dz1d*const
         elseif(is.eq.2) then ! BB(mode2) QW BC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & bc_emit(i03+ixy)*fbc_qw*m1p*sum(bcoc_unit_em2(:,2))*ug2kg*dt/dz1d*const
         elseif(is.eq.3) then ! FF(mode1) QW OC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & oc_emit(i03+ixy)*foc_qw*m1p*sum(bcoc_unit_em2(:,1))*ug2kg*dt/dz1d*const
         elseif(is.eq.4) then ! BB(mode2) QW OC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & oc_emit(i03+ixy)*foc_qw*m1p*sum(bcoc_unit_em2(:,2))*ug2kg*dt/dz1d*const
         elseif(is.eq.5) then ! FF(mode1) SW BC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & bc_emit(i03+ixy)*fbc_sw*m1p*sum(bcoc_unit_em2(:,1))*ug2kg*dt/dz1d*const
         elseif(is.eq.6) then ! BB(mode2) SW BC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & bc_emit(i03+ixy)*fbc_sw*m1p*sum(bcoc_unit_em2(:,2))*ug2kg*dt/dz1d*const
         elseif(is.eq.7) then ! FF(mode1) SW OC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & oc_emit(i03+ixy)*foc_sw*m1p*sum(bcoc_unit_em2(:,1))*ug2kg*dt/dz1d*const
         elseif(is.eq.8) then ! BB(mode2) SW OC
           apm_bcoc(iapm+ixy) = apm_bcoc(iapm+ixy)+ &
            & oc_emit(i03+ixy)*foc_sw*m1p*sum(bcoc_unit_em2(:,2))*ug2kg*dt/dz1d*const
         endif
...
!===========================
!---------------------------
if(ls2bcoc.and..false.) then ! add primary sulfate to bcoc coated sulfate mass
   do k=1,nzz-1
      i03=ip3mem(k,ne)
      iapm=ip_dust(k,is,ne)
      do j = sy(ne),ey(ne)
      do i = sx(ne),ex(ne)
         ixy = (ex(ne)-sx(ne)+3)*(j -sy(ne)+1)+i-sx(ne)+1
         dz1d=dz(i03+ixy)
         mbcsulf(i03+ixy)=mbcsulf(i03+ixy)+sulf_emit(i03+ixy)*fs2bc*dt/dz1d
         mocsulf(i03+ixy)=mocsulf(i03+ixy)+sulf_emit(i03+ixy)*fs2oc*dt/dz1d
      enddo
      enddo
   enddo
endif
