...
!apm_1.007.2007032022
 ...
 call system("mkdir -p out/apm_tmp."//trim(pflag) )
 call system("mkdir -p out/apm_tmp."//trim(pflag)//'/'//date(1:8))

 write (cmyid,'(i3.3)') myid

! write (fname(1:3),'(i3.3)') myid

 fname='apm_'//cdnum//'.'//cmyid//'.'//date//cminute

! print*,'output_fname : ',trim(fname)

 call get_funit(funit)
 open(funit,file='out/apm_tmp.'//trim(pflag)//'/'//date(1:8)//'/'//trim(fname) &
