...
real :: nh4_aqchem_1d,no3_aqchem_1d
...
!     All conc units must be mol/mol (mixing ratio)
  if( cliq.ge.cwmin .and. tcell.ge.tamin ) then ! enough liquid cloud water ?
    ...
    dust_tot = 0.0
    if(luse_apmaeraq) then
     if(lapm) then
      do is=1,NDSTB
        iapm = ip_dust(k,is,ne)
        dust_tot = dust_tot + apm_dust(iapm+ixy)
      enddo
     else
      stop 'flag erro luse_apmaeraq-lapm_11 '
     endif
    else
      do is=1,isize
    ...
    if(luse_apmaeraq) then
     if(lapm) then
      do is=1,NSEA
        ...
        salt_tot = salt_tot + apm_salt(iapm+ixy)
      enddo
     else
      stop 'flag erro luse_apmaeraq-lapm_22 '
     endif
    else
      do is=1,isize
...
!!!!
    ...
    if(lnaqpms_pso4) then
      ...
      ASO4(i03+ixy)=ASO4(i03+ixy)+so4_aqchem(i03+ixy) ! -> SO4--(aq)
      ANO3(i03+ixy)=ANO3(i03+ixy)+no3_aqchem_1d       ! -> NO3- (aq)
      ANH4(i03+ixy)=ANH4(i03+ixy)+nh4_aqchem_1d       ! -> NH4+ (aq)
    endif
    !===================================================================
