...
!------------------------------------------------------------------------------
     ...
     &           IACT10,IACT20,IACT30,NTEMPOUT1,
...
!===================================================
     ...
     &          ,bc_ccn2_1d,oc_ccn2_1d
      ...
      real*8  :: XVBINBC(nbincb),XVBINOC(nbincb)
      real*8  :: ztn_binbc,ztn_binoc
      real*8  :: DTCOAG4,DTCOAG5
      real*8  :: DENBINBCWET,DENBINOCWET
      real*8  :: RBINBCWETCM(nbincb),RBINOCWETCM(nbincb)
      real*8  :: RBINBCWET(nbincb),RBINOCWET(nbincb)
      real*8  :: RBINBCDRY(nbincb),RBINOCDRY(nbincb)
      integer :: itmp     


      real*8  :: CSBINBC,AREABINBC
      real*8  :: CSBINOC,AREABINOC
      real*8  :: YAREABINBC(nbincb)
      real*8  :: YAREABINOC(nbincb)
      real*8  :: YCS_BINBC,YCS_BINOC

      real*8  :: vfrc_core,vfrcdry_core,vfrcdry_msp,vfrc_msp,vfrc_water

      real*8  :: diam1bc,diam2bc
      real*8  :: diam1oc,diam2oc

      real*8  :: frc_qwbc,frc_qwoc

      integer :: i03nm_bc,i10nm_bc,iact1_bc,iact2_bc,iact3_bc
      integer :: i03nm_oc,i10nm_oc,iact1_oc,iact2_oc,iact3_oc
...
!      INTEGER :: NCOAG(2), NCOAGMAX, I3nm, IERF, I10nm
      ...
      INTEGER   :: NCOAG1,NCOAG2,NCOAG3,NCOAG4,NCOAG5
...
! BCOC
      ...
      IF(MCORE(4).gt.1.0e-20) THEN
        frc_qwbc=(MBCOC8(1)+MBCOC8(2))/MCORE(4)
      ELSE
        frc_qwbc=0.0
      ENDIF

      IF(MCORE(5).gt.1.0e-20) THEN
        frc_qwoc=(MBCOC8(3)+MBCOC8(4))/MCORE(5)
      ELSE
        frc_qwoc=0.0
      ENDIF
      ...
      if(lbincb) then
        MCORE(4) = sum(XMBINBC(:))
        do n=1,nbincb
          XVBINBC(n)=XMBINBC(n)*1.d-3/DENAER(4) ! XV in cm3/cm3
        enddo
        MCORE(5) = sum(XMBINOC(:))
        do n=1,nbincb
          XVBINOC(n)=XMBINOC(n)*1.d-3/DENAER(5) ! XV in cm3/cm3
        enddo
      endif
...
! the size of BCOC that takes into account other species associated.
      ...
      IF(lbincb) THEN
         vfrc_core=1.0/(RRATIOWET**3.0)
         vfrcdry_core=1.0/(RRATIO**3.0)
         vfrcdry_msp=1.0-vfrcdry_core
         vfrc_msp=vfrc_core*vfrcdry_msp/vfrcdry_core
         vfrc_water=1.0-vfrc_core-vfrc_msp
         DENBINBCWET = DENAER(4)*vfrc_core+DENAER(1)*vfrc_msp
     &                +DENWATER*vfrc_water ! g/cm3
         do n=1,nbincb
           RBINBCDRY(n)=radcbm(n)*RRATIO
           RBINBCWET(n)=radcbm(n)*RRATIOWET
           RBINBCWETCM(n)=RBINBCWET(n)*100.0   ! m -> cm 
         enddo
      ENDIF
...
!============================
     ...
     &           + MBCOC8(2)*1.E-6*TOTNUMBC(2)*FBCOC2   ! BCPI from BB
      ...
      IF(lbincb) THEN
        ...
        i10nm_bc=1
        DO n=2,nbincb
          diam1bc=RBINBCDRY(n-1)*2.0
          diam2bc=RBINBCDRY(n)*2.0
          IF(3.d-9.GE.diam1bc.and.3.d-9.LT.diam2bc) i03nm_bc=n
          IF(10.d-9.GE.diam1bc.and.10.d-9.LT.diam2bc) i10nm_bc=n
          IF(ZDACT(4,1).GE.diam1bc.and.ZDACT(4,1).LT.diam2bc) iact1_bc=n
          IF(ZDACT(4,2).GE.diam1bc.and.ZDACT(4,2).LT.diam2bc) iact2_bc=n
          IF(ZDACT(4,3).GE.diam1bc.and.ZDACT(4,3).LT.diam2bc) iact3_bc=n
        ENDDO
        ZCCN(4,1)=sum(XNBINBC(iact1_bc:nbincb))*frc_qwbc
        ZCCN(4,2)=sum(XNBINBC(iact2_bc:nbincb))*frc_qwbc
        ZCCN(4,3)=sum(XNBINBC(iact3_bc:nbincb))*frc_qwbc
      ENDIF
      ...
      IF(lbincb) THEN
         ...
         vfrc_msp=vfrc_core*vfrcdry_msp/vfrcdry_core
         vfrc_water=1.0-vfrc_core-vfrc_msp
         DENBINOCWET = DENAER(5)*vfrc_core+DENAER(1)*vfrc_msp
     &                +DENWATER*vfrc_water ! g/cm3
         do n=1,nbincb
           RBINOCDRY(n)=radcbm(n)*RRATIO
           RBINOCWET(n)=radcbm(n)*RRATIOWET   ! m
           RBINOCWETCM(n)=RBINOCWET(n)*100.0   ! m -> cm 
         enddo
      ENDIF
...
!===============================
     ...
     &           + MBCOC8(4)*1.E-6*TOTNUMOC(2)*FBCOC2   ! OCPI from BB
      ...
      IF(lbincb) THEN
        ...
        i10nm_oc=1
        DO n=2,nbincb
          diam1oc=RBINOCDRY(n-1)*2
          diam2oc=RBINOCDRY(n)*2
          IF(3.d-9.GE.diam1oc.and.3.d-9.LT.diam2oc) i03nm_oc=n
          IF(10.d-9.GE.diam1oc.and.10.d-9.LT.diam2oc) i10nm_oc=n
          IF(ZDACT(5,1).GE.diam1oc.and.ZDACT(5,1).LT.diam2oc) iact1_oc=n
          IF(ZDACT(5,2).GE.diam1oc.and.ZDACT(5,2).LT.diam2oc) iact2_oc=n
          IF(ZDACT(5,3).GE.diam1oc.and.ZDACT(5,3).LT.diam2oc) iact3_oc=n
        ENDDO
        ZCCN(5,1)=sum(XNBINOC(iact1_oc:nbincb))*frc_qwoc
        ZCCN(5,2)=sum(XNBINOC(iact2_oc:nbincb))*frc_qwoc
        ZCCN(5,3)=sum(XNBINOC(iact3_oc:nbincb))*frc_qwoc
      ENDIF
...
!
      ...
      IF(lbincb) THEN
           ...
           AREABINBC = 1.d-21
           DO NI = 1, nbincb
              XRCM = RBINBCWET(NI)*100.   ! cm
              AREA = 4.*ONEPI*XRCM*XRCM*XNBINBC(NI)  !cm2/cm3
              AREABINBC = AREABINBC + AREA
              YAREABINBC(NI) = AREA
              YKN = FREEP/XRCM
              FCORR = YKN/(0.75+YKN)
              CSBINBC = CSBINBC + 0.25*VGAS*AREA*FCORR  ! s-1
           ENDDO
           YCS_BINBC=CSBINBC
           ...
           AREABINOC = 1.d-21
           DO NI = 1, nbincb
              XRCM = RBINOCWET(NI)*100.   ! cm
              AREA = 4.*ONEPI*XRCM*XRCM*XNBINOC(NI)  !cm2/cm3
              AREABINOC = AREABINOC + AREA
              YAREABINOC(NI) = AREA
              YKN = FREEP/XRCM
              FCORR = YKN/(0.75+YKN)
              CSBINOC = CSBINOC + 0.25*VGAS*AREA*FCORR  ! s-1
           ENDDO
           YCS_BINOC=CSBINOC
...
!          print*,'shun if coasacv:',TCS-CSSULF
     ...
     &     DENBINOCWET,RBINOCWETCM,XNBINOC  )
           endif ! shun@debug_nucl
          ENDIF
...
!==================================================
! chenxueshun@beijing_20141220 
! size resolved carbonaceous particles coagulation

!----------------------------------------
! binbc

      ztn_binbc=sum(XNBINBC(:))

      IF(ztn_binbc.GT.1.) THEN  ! only consider coag if substatial # of particle exits 
        NCOAG(4) = MAX(1,INT(EPC/(0.5d-8*ztn_binbc*DT)))
        IF((NCOAG4+1).GE.NCOAG(4)
     &               .or.(NCOAG4+1).GE.NCOAGMAX) THEN
         DTCOAG4 = float(NCOAG4+1)*DT
         ITYPE = 104 ! carbonaceous particles
!          print*,'shun binbc coag',II,JJ,LL
!          print*,'NCOAG4=',NCOAG4
!          print*,'NCOAG(4)=',NCOAG(4)
!          stop
          CALL APM_COAG(ITYPE,nbincb,TK,PMB,DTCOAG4,
     &            RBINBCWETCM,DENBINBCWET,XNBINBC,XVBINBC)
          NCOAG4 = 0  ! reset counter to 0 after coag is called

          DO N = 1, nbincb
            XMBINBC(N)=XVBINBC(N)*DENAER(4)*1.d3  !XVA in cm3/cm3  XMA in kg/m3
          ENDDO
...
!------------------------------------------
! binoc
      ztn_binoc=sum(XNBINOC(:))

      IF(ztn_binoc.GT.1.) THEN  ! only consider coag if substatial # of particle exits 
        NCOAG(5) = MAX(1,INT(EPC/(0.5d-8*ztn_binoc*DT)))
        IF(    (NCOAG5+1).GE.NCOAG(5)
     &     .or.(NCOAG5+1).GE.NCOAGMAX ) THEN
         DTCOAG5 = float(NCOAG5+1)*DT
         ITYPE = 105 ! carbonaceous particles
!          print*,'shun binbc coag',II,JJ,LL
!          print*,'NCOAG5=',NCOAG5
!          print*,'NCOAG(5)=',NCOAG(5)
!          stop
          CALL APM_COAG(ITYPE,nbincb,TK,PMB,DTCOAG5,
     &            RBINOCWETCM,DENBINOCWET,XNBINOC,XVBINOC)
          NCOAG5 = 0  ! reset counter to 0 after coag is called

          DO N = 1, nbincb
            XMBINOC(N)=XVBINOC(N)*DENAER(5)*1.d3  !XVA in cm3/cm3  XMA in kg/m3
          ENDDO

        ELSE
          NCOAG5=NCOAG5 + 1   ! count step that coag is not call in the grid
        ENDIF
      ENDIF
