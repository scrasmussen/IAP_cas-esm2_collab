...
c       limitations, terms, and provisions of use as noted above.
      subroutine clddiagf_mark(nx,ny,nz,lwet,delfx,delfy,uf,vf,htf,
     &                    presf,temprf,rainf,rh0,sigmaf,pstarf,psfcf,
     &                    lati,dt,conc,iwb,ieb,jsb,jeb,
     &                    ismMax,smconv)
c
c-----ATDM v1.00
c
...
c      parameter (mxfz=10)
      ...
      real :: dt 
      integer :: ismMax
      real  ::  smconv(ismMax,iwb:ieb,jsb:jeb,nz)
c! the fraction of every source
      real  ::  smsc(ismMax,iwb:ieb,jsb:jeb) 
c! the mean fraction in sub-cloud layers
      real  ::  TMPsmconv(ismMax,nz) 
c!tempory variable
...
c             write (*,*) 'clddiagf', i,j,k,ffnonpr(i,j,k)
   80     continue 
c
c-----Estimate convecivv results on the fraction of every sources  -----------------
       DO 81 is=1,ismMax
         DO 82 k=1,nz
...
!       call convmix_mark(nz,cd,kbot,ktop,kfreez,dt,delfx(i,j,1),
!     &       delfy(i,j,1),pstarf(i,j),xmdf(i,j),rainf(i,j),
!     &       sigmaf,zi,ti,pi,alfa,ismMax,TMPsmconv)
       ENDIF


       IF(fstrtf(i,j).ge.0) then
!        call strtmix_mark(nz,csav,kbot,ktop,kfreez,dt,delfx(i,j,1),
!     &        delfy(i,j,1),rainf(i,j),pstarf(i,j),
!     &        sigmaf,zi,ti,pi,qi,alfa,ismMax,TMPsmconv)        
       ENDIF 
