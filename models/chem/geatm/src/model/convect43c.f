...
C
        SUBROUTINE CONVECT
     *    (T,   Q,    QS,     U,    V,      TRA,    P,    PH,
     *     ND,  NL,   NTRA,   DELT,  FTRA, FTRAD, FTRAU,FTRAO,        
     1     FTRAE,CBMF,   IFLAG       )
C
C-----------------------------------------------------------------------------
C    *** On input:      ***
...
C
C     FTRAU: Array of forcing of tracer content from the lower layer, in tracer mixing ratio
C            per second, defined at same levels as T. Dimensioned (ND,NTRA)
C     FTRAD: same as FTRAU, but from upper layer    
C        
...
C
      ...
      REAL FTRAU(ND,NTRA),FTRAD(ND,NTRA),FTRAO(ND,NTRA),FTRAE(ND,NTRA)
