 ...
 subroutine  get_w(myid,w,w0,u,v,dx,dy,dz,topo,hh,sx,ex,sy,ey,k)
  integer myid, sx, ex, sy, ey, k
  real, dimension(sx-1:ex+1,sy-1:ey+1) :: w,u,v,w0,dx,dy,dz,topo
  real hh
...
SUBROUTINE GETZ0(MYID,LANDUSE,Z0,IMONTH,SX,EX,SY,EY,NE)
INTEGER :: MYID, SX,EX,SY,EY,NE,IMODIS
REAL,DIMENSION(SX-1:EX+1,SY-1:EY+1) :: Z0,LANDUSE
DO J = SY, EY
 DO I = SX, EX 
...
SUBROUTINE DUSTHEIGHT(MYID,HEIZ,TERRAIN,LANDUSE,DUSTK,TOTALDUST, U,V,SX,EX,SY,EY,K,NE)
 INTEGER MYID,SX,EX,SY,EY,NE,K ! juanxiong he
 REAL,DIMENSION(SX-1:EX+1,SY-1:EY+1) :: TERRAIN,LANDUSE,DUSTK,TOTALDUST
 REAL,DIMENSION(SX-1:EX+1,SY-1:EY+1) :: HEIZ,U,V
 REAL :: WIND
...
SUBROUTINE PUTDUST(MYID,C,DUSTCOMP1,DUSTCOMP2,DUSTCOMP3,DUSTCOMP4,DUSTCOMP5,DUSTCOMP6,&
...
SUBROUTINE PUTSALT (MYID, C,SEACOMP1, SEACOMP2,SEACOMP3,SEACOMP4,SEACOMP5,&
...
REAL DR,CONST
REAL,DIMENSION(5000) :: RMID ! juanxiong
REAL,DIMENSION(0:5000) :: REDGE ! juanxiong
...
! IF(I.NE.SX.AND.I.NE.EX.AND.J.NE.SY.AND.J.NE.EX) THEN ! juanxiong he
IF(I.NE.SX-1.AND.I.NE.EX+1.AND.J.NE.SY-1.AND.J.NE.EY+1) THEN ! juanxiong he
...
IF (.NOT.ISURF) THEN
 ...
 DO R = 1, NR
   ...
   DENSITY =  1.E03 * ( 3.8033-16.248*RH_TMP + 46.085 * RH_TMP **2 - 68.317 * RH_TMP ** 3 &
                + 50.932*RH_TMP**4 - 15.261 * RH_TMP**5   ) / 1.E03 ! DENSITY IN KG/M3
...
ELSE IF (ISURF) THEN ! OFFSHORE
 ...
 DO R = 1, NR
   ...
   DENSITY =  1.E03 * ( 3.8033-16.248*RH_TMP + 46.085 * RH_TMP **2 - 68.317 * RH_TMP ** 3 &
                + 50.932*RH_TMP**4 - 15.261 * RH_TMP**5   ) ! DENSITY IN KG/M3
   ...
   B = ( 0.433 - LOG10(RMID(R)) ) / 0.433 ! juanxiong he
...
!stop
 ...
 do j = sy,ey
      do i = sx,ex
        DUSTDRY(I,J) = DUSTDRY(I,J) + dryvel(i,j)*c(i,j)*dt/1.e9 ! kg/m2/hr
