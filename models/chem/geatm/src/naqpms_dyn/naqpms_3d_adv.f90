...
!if(myid.eq.0) print*,'naqpms hadv'
...
!stop
        ...
        if(laerv2) then
          ...
          do is=1,naerbin
            ...
            if(ia.gt.1.and.is.gt.1) cycle ! skip zero aersol tracers

            i04aer=ip4mem_aer(k,is,ia,ne)
          
            do i=sx(ne)-1,ex(ne)+1
              ixy=(ex(ne)-sx(ne)+3)*(j -sy(ne)+1)+i-sx(ne)+1
              Q0(i)=aerom(i04aer+ixy)
!Q0(i)=i+1
            enddo
            if(trim(flagadv).eq.'walcek') then
              call advec1d_walcek( sx(ne),ex(ne),Q0,QN,u1D,DEN0,DEN1,DXX,DD0,tstep &
            ...
            do i = sx(ne),ex(ne)
              ixy=(ex(ne)-sx(ne)+3)*(j -sy(ne)+1)+i-sx(ne)+1
              aerom(i04aer+ixy)=amax1(QN(i),1.E-20)
            enddo
!print*,'QN=',QN
!stop
          enddo
        ...
        do is=1,isize
           ...
           IF(ia==1) THEN ! sea salt
             do iduc = 1, nseacom
                ...
                do i = sx(ne),ex(ne)
                  ixy=(ex(ne)-sx(ne)+3)*(j -sy(ne)+1)+i-sx(ne)+1
                  SEACOMP(i05c+ixy)=amax1(QN(i),1.E-20)
                enddo
             enddo
           ELSE  IF(ia==2) THEN ! for dust
             do iduc = 1, ndustcom
                ...
                do i = sx(ne),ex(ne)
                  ixy=(ex(ne)-sx(ne)+3)*(j -sy(ne)+1)+i-sx(ne)+1
                  DUSTCOMP(i05c+ixy)=amax1(QN(i),1.E-20)
                enddo
             enddo ! iduc   
           ENDIF ! ia
...
!Q0(j)=j+1
            ...
            do j = sy(ne),ey(ne)
              ixy=(ex(ne)-sx(ne)+3)*(j -sy(ne)+1)+i-sx(ne)+1
              aerom(i04aer+ixy)=amax1(QN(j),1.E-20)
            enddo
!print*,'QN=',QN
!stop
          enddo
        ...
        do is=1,isize
           ...
           IF(ia==1) THEN ! sea salt
             do iduc = 1, nseacom
                ...
                do j = sy(ne),ey(ne)
                  ixy=(ex(ne)-sx(ne)+3)*(j -sy(ne)+1)+i-sx(ne)+1
                  SEACOMP(i05c+ixy)=amax1(QN(j),1.E-20)
                enddo
             enddo
           ELSE  IF(ia==2) THEN ! for dust
             do iduc = 1, ndustcom
                ...
                do j = sy(ne),ey(ne)
                  ixy=(ex(ne)-sx(ne)+3)*(j -sy(ne)+1)+i-sx(ne)+1
                  DUSTCOMP(i05c+ixy)=amax1(QN(j),1.E-20)
                enddo
             enddo ! iduc   
           ENDIF ! ia
...
!if(myid.eq.0) print*,'naqpms vadv'
...
do j = sy(ne),ey(ne)
  ...
  if(j.ge.(ny(ne)-4).or.j.le.5) cycle ! chenxsh@20181206
...
stop
       ...
       do k=1,nzz-1
         i04=ip4mem(k,ig,ne)
         gas(i04+ixy)=amax1(QN(k),1.0e-20)
       enddo  
!do k=1,nzz-1
!print*,Q0(k),entrn(k),dilut(k),depth(k)
!enddo
!print*,'zadv QN=',QN
!@print*,sum(QN(:))+1+nzz,sum(Q0(:))
!stop
     enddo
...
!Q0(k)=k
          ...
          do k=1,nzz-1
             i04aer=ip4mem_aer(k,is,ia,ne)
             aerom(i04aer+ixy)=amax1(QN(k),1.E-20)
          enddo
if(1==2) then
do k=1,nzz-1
print*,Q0(k),u1d(k),QN(k),u1d(k-1)*tstep/DXX(k),u1d(k)*tstep/DXX(k)
enddo
print*,'zadv QN=',QN
!@print*,sum(QN(:))+1+nzz,sum(Q0(:))
stop
...
stop
        ...
        if(ia==1) then ! sea salt
           do iduc = 1, nseacom
             ...
             do k=1,nzz-1
               i05c = ip5memcs (k,is,iduc,ne)
               SEACOMP(i05c+ixy)=amax1(QN(k),1.E-20)
             enddo
           enddo
         elseif(ia==2) then
           do iduc = 1, ndustcom
             ...
             do k=1,nzz-1
               i05c = ip5memc (k,is,iduc,ne)
               DUSTCOMP(i05c+ixy)=amax1(QN(k),1.E-20)
             enddo
           enddo
         endif
