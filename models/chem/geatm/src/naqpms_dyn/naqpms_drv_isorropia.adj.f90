...
use apm_varlist, only : naq_nh4,naq_no3
implicit none

logical :: lapm

! equilibrium proceed with a delay timescale of 2 hours
logical,parameter :: ladjust=.false.
real,parameter :: tau=2.0*3600 
...
real :: cno3,cnh4,chno3,cnh3,conaer,congas,dcon
...
!========================================
! 20161026
! preserve no3 and nh4 befeore isoropia
if(lapm) then
        naq_nh4(i03+ixy)=ANH4(i03+ixy)
        naq_no3(i03+ixy)=ANO3(i03+ixy)
endif
!========================================
...
! Modelling the partitioning of ammonium nitrate in
! the convective boundary layer (2012)
if(ladjust) then

! adjust nitrate
  ibin=1
  i04aer=ip4mem_aer(k,ibin,idx_no3,ne)
  conaer=aerom(i04aer+ixy)
  aerom(i04aer+ixy)=conaer+(cno3-conaer)*exp(-dt_cbmz/tau)
  dcon=aerom(i04aer+ixy)-conaer ! change of nitrate (ug/m3) 
! adjust nitric acid gas
  congas=gas(i04_2+ixy)
  gas(i04_2+ixy)=congas-dcon*wgt_hno3/wgt_no3 
...
! adjust ammonium
  ...
  i04aer=ip4mem_aer(k,ibin,idx_nh4,ne)
  conaer=aerom(i04aer+ixy)
  aerom(i04aer+ixy)=conaer+dcon*wgt_nh4/wgt_no3
! adjust ammonia
  congas=gas(i04_4+ixy)
  gas(i04_4+ixy)=congas-dcon*wgt_nh3/wgt_no3
