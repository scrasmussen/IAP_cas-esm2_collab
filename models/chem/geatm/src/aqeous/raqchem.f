...
c      IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) PRINT*, 'AQUE STEP 1'
...
c            XMSG = 'MET DATA ERROR'
         write(*,*) 
     &    'Error in RADM aqueous chemistry (RAQCHEM) , Invalid
     &     met data: T, P',TEMP,PRES_PA
c          END IF
c        END IF
      END IF
...
c      IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) PRINT*, 'AQUE STEP 1_1'

      I20C = 0
20    CONTINUE
C!***** LIJIE
c      IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) PRINT*, I20C,'AQUE STEP 1_2'
...
C! *** LIJIE 
c      IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) 
c     &             PRINT*, I7777C,'AQUE STEP 1_3'
...
30    CONTINUE
C! *** LIJIE ***
c      IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5.AND.I30C.EQ.10) 
c     &      PRINT*, I30C,'AQUE STEP 1_4'
...
c      IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5.AND.I30C.EQ.10) 
c     &      PRINT*, I30C,'AQUE STEP 1_6'

      IF ( HTST .LE. TST ) GO TO 30

c      IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) PRINT*,'AQUE STEP 1_7'   
...
c      IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) PRINT*,'AQUE STEP 1_8'
...
C...  the next timestep.
c      IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) PRINT*,
c     &                        TIMEW,TAUCLD,'AQUE STEP 1_9'
...
c         IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) PRINT*,'AQUE STEP 1_10'
...
c        IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) PRINT*,'AQUE STEP 1_13'
...
c       IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) PRINT*,'AQUE STEP 1_15'
        GO TO 20
      END IF
CC *** LIJIE ***
CC      IF(II.EQ.71.AND.JJ.EQ.72.AND.KK.EQ.1) PRINT*, I30C,'4444'
c      IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) PRINT*, 'AQUE STEP 2'
...
c      HPWDEP = WETDEP( 1 )
c
c      IF(II.EQ.65.AND.JJ.EQ.11.AND.KK.EQ.5) PRINT*, 'AQUE STEP 3'
      RETURN
