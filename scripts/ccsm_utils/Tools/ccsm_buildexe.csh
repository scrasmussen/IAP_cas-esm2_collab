...
set wrfstr = 0
if ($NTHRDS_WRF > 1 || $BUILD_THREADED == "TRUE") set wrfstr = 1
set geastr = 0
if ($NTHRDS_GEA > 1 || $BUILD_THREADED == "TRUE") set geastr = 1
set atmstr = 0
...
set smpstr = w${wrfstr}ge${geastr}a${atmstr}l${lndstr}i${icestr}o${ocnstr}g${glcstr}c${cplstr}
...
# -------------------------------------------------------------------------
   ...
   if ($DEF_CHANGE != "FALSE") then
   ./xmlchange -file env_conf.xml -id DEF_CHANGE -val FALSE
   if ($status != 0) then
     echo  ERROR: ccsm_buildexe.csh call to xmlchange failed
     exit  100
   endif
