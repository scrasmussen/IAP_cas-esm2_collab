...
subroutine lpjreset(itypwat,zlnd,albsat,albdry,nfcon_pft,nfvar_col,nfvar_pft,n_pft,fcon_pft,fvar_col,fvar_pft,&
                    ivt_r,ivt_r_old,ifpre,ifpre_old,wt_patch,wt_patch_old,prec365,wliq6mon)
...
! ----------------------------------------------------------------
  ...
  real(r8),  intent(in)    :: zlnd      !roughness length for soil [m]
  real(r8),  intent(in)    :: albsat    ! wet soil albedo for different coloured soils [-]
  real(r8),  intent(in)    :: albdry    ! dry soil albedo for different coloured soils [-]
...
!      if(vegwt<1.0E-6) return
   ...
   do p = 1, n_pft
     ...
     ! tran      => fcon_pft(21:24,p)        
      refl      => fcon_pft(17:20,p)          
      refs      => fcon_pft(21:24,p)          
      tranl     => fcon_pft(25:28,p)          
      trans     => fcon_pft(29:32,p)          
...
!     litter_bg => fvar_pft(ub,p)                 ; ub = ub+1             !59
   ...
   !  ifpre     => fvar_pft(ub,p)                                         !* 
      ...
      annfrg    => fvar_pft(ub,p)                 ; ub = ub+1             !68
      afirefrac1=> fvar_pft(ub,p)                 ; ub = ub+1             !69
      nfireg1   => fvar_pft(ub,p)                                         !70
#if(defined DyN)
      ub=ub+1
