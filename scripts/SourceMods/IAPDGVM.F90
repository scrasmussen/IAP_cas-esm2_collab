  subroutine IAPDGVM(nyr_r   ,n_pft         ,npftpar       ,pftpar        ,&
                 ...
                 sla         ,agddtw        ,firelength    ,afirefrac1, nfireg1,&
                 lm_sapl                                                  ,&
                 sm_sapl     ,hm_sapl       ,rm_sapl       ,wt_patch      ,&
                 wt_column   ,prec365 ,     wliq6mon   ,afirec_gcell  ,afiref_gcell  ,&
...
! INTENT IN VARIAVLES:
   ...
   real(r8),INTENT(in)  :: afirefrac1(n_pft)
   real(r8),INTENT(in)  :: nfireg1(n_pft)
...
! INTENT OUT VARIABLES:
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!2016.11.3
 !  real(r8),INTENT(out) :: n_fireg_gcell(n_pft)
...
! LOCAL VARIABLES:
   ...
   integer  :: nyr
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   real(r8) :: nfireg(n_pft)
 !  real(r8) :: emCO2(n_pft)
 !  real(r8) :: emCO(n_pft)
 !  real(r8) :: emCH4(n_pft)
 !  real(r8) :: emNHMC(n_pft)
 !  real(r8) :: emH2(n_pft)
 !  real(r8) :: emNOx(n_pft)
 !  real(r8) :: emN2O(n_pft)
 !  real(r8) :: emPM25(n_pft)
 !  real(r8) :: emTPM(n_pft)
 !  real(r8) :: emTC(n_pft)
 !  real(r8) :: emOC(n_pft)
 !  real(r8) :: emBC(n_pft)
...
! Avoid silly gfortran intrinsic subroutine name collision problem
  ...
  ! Transform ivt_r from real to integer
    ...
    do p = 1,n_pft
       if(fpcgrid(p) .lt. 1.0E-16) cycle                             !add by zjw 
       if (nyr == 1) then
   ...
   ! Determine grid values of npp and microbial respiration
    ...
    call BuildNatVegFilter(n_pft, ivt, ifpre, num_natvegp, filter_natvegp)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 call Fire(n_pft, afirefrac(1:), nfireg(1:),acfluxfire(1:),&
...
!=================================================================================
subroutine Fire(n_pft, afire_frac,n_fireg,acflux_fire, &
                  ivt, lm_ind, sm_ind, hm_ind, rm_ind, &
                  fpcgrid,ifpre,vegclass,litter_ag,litter_bg,nind,afire_frac1,n_fireg1, &
                  fmi,ccl,ccst,ccr,ccd)
                !  efCO2, efCO, efCH4, efNHMC, efH2, efNOx, efN2O, efPM25, efTPM,efTC, efOC, efBC,&
...
!---------------------------------------------------------------------
    ...
    real(r8), intent(out) :: n_fireg(n_pft)
!    real(r8), intent(out) :: em_CO2(n_pft)
!    real(r8), intent(out) :: em_CO(n_pft)
!    real(r8), intent(out) :: em_CH4(n_pft)
!    real(r8), intent(out) :: em_NHMC(n_pft)
!    real(r8), intent(out) :: em_H2(n_pft)
!    real(r8), intent(out) :: em_NOx(n_pft)
!    real(r8), intent(out) :: em_N2O(n_pft)
!    real(r8), intent(out) :: em_PM25(n_pft)
!    real(r8), intent(out) :: em_TPM(n_pft)
!    real(r8), intent(out) :: em_TC(n_pft)
!    real(r8), intent(out) :: em_OC(n_pft)
!    real(r8), intent(out) :: em_BC(n_pft)
! INTENT IN VARIABLES: 
    ...
    real(r8), INTENT(in) :: ifpre(n_pft)         ! whether this pft present in patch
    real(r8), INTENT(in) :: vegclass(n_pft)      ! 1.tree 2.shrub 3.grass 4.crop
    real(r8), INTENT(in) :: fmi(n_pft)
    real(r8), INTENT(in) :: ccl(n_pft)
    real(r8), INTENT(in) :: ccst(n_pft)
    real(r8), INTENT(in) :: ccr(n_pft)
    real(r8), INTENT(in) :: ccd(n_pft)
   ! real(r8), INTENT(in) :: efCO2(n_pft)
   ! real(r8), INTENT(in) :: efCO(n_pft)
   ! real(r8), INTENT(in) :: efCH4(n_pft)
   ! real(r8), INTENT(in) :: efNHMC(n_pft)
   ! real(r8), INTENT(in) :: efH2(n_pft)
   ! real(r8), INTENT(in) :: efNOx(n_pft)
   ! real(r8), INTENT(in) :: efN2O(n_pft)
   ! real(r8), INTENT(in) :: efPM25(n_pft)
   ! real(r8), INTENT(in) :: efTPM(n_pft)
   ! real(r8), INTENT(in) :: efTC(n_pft)
   ! real(r8), INTENT(in) :: efOC(n_pft)
   ! real(r8), INTENT(in) :: efBC(n_pft)
    real(r8), INTENT(in) :: afire_frac1(n_pft)
    real(r8), INTENT(in) :: n_fireg1(n_pft)
! INTENT INOUT VARIABLES:
    real(r8), INTENT(inout) :: litter_ag(n_pft)  ! above ground litter
    real(r8), INTENT(inout) :: litter_bg(n_pft)  ! above ground litter
...
! INTENT IN VARIABLES:
    integer , INTENT(in) :: n_pft                ! total patches
!   real(r8), INTENT(in) :: wt_patch(n_pft)      ! pft weight relative to grid
!   cell
...
! OTHER LOCAL VARIABLES:
    ...
    real(r8) :: fpc_shrub_total       !total fractional cover of trees in vegetated portion of gridcell
    ...
    real(r8) :: est_tmp
    real(r8) :: est_pot_tree(n_pft)
    real(r8) :: est_pot_shrub(n_pft)
    real(r8) :: est_pot_grass(n_pft)
...
!print *, 'estab', grid_tmomin20,grid_agdd20,grid_agddtw,fpcsum
    ...
    ! Determine present, survive, estab.  Note - if tmomin20 > tcmax then
    ! crops,
    ...
    ! ground. Thus they make fpc_grid_total < 1. Solve by allowing one more pft per grid cell than the number of pfts.
...
!       end if   ! if soil
    ...
    ! Calculate total woody FPC and number of woody PFTs present and able to
    ! establish
...
do p = 1, numpft
       ...
       ! Prohibit establishment under extreme temperature or water stress.
        ...
        if (ivt(p)>=1 .and. ivt(p)<=8) then
         ...
         soil_liq_3lys=grid_wliq6mon**2/(sqrt(2.0*PI)*0.4)*exp(-(grid_wliq6mon-1.0)**2/(2*0.4**2))
          ...
          ! Maximum establishment rate reduced by shading as tree FPC approaches
          ! 1
          ! Total establishment rate partitioned equally among regenerating
          ! woody PFTs
 ...
 ! Avoid unbelievable value when nind<nind_min, add by zhq @ 09/14/2010
             ...
             ! individual such that the basic allometric relationships (A-C
             ! below)
             ! are satisfied.
             ! (A) (leaf area) = latosa * (sapwood xs area)
             !        (Pipe Model, Shinozaki et al. 1964a,b; Waring et al 1982)
 ...
 !  (9) stemdiam = ( 4 * (sm_ind + hm_ind ) / wooddens / PI /
             ...
             ! Recalculate sapwood mass, transferring excess sapwood to
             ! heartwood
