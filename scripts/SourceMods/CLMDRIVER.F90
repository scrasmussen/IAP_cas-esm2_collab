...
!=======================================================================
 ...
 use colm_varMod, only : numpatch, numcolumn, numgrid, ftune, forc, &
                         ...
                         Isf_pft, Iss_pft, Ksf_pft, Kss_pft
 use timemgr, only : idate, idate_p, dtime, get_nstep, get_curr_date
 use spmd_decomp, only : ppmap, cgmap, gxmap, gymap
 use colm_ioMod, only : getig, getrhm
#ifdef VEGDATA
 use vegdata, only : interp_vegdata
#endif
 use colm_varctl, only : enable_carbon_spinup, cspinup_nstep
#ifdef AUTOMASK
 use metdata, only : forcmask
#endif
 use spmd_decomp, only : pgmap, cgmap, gxmap, gymap
 use nanMod, only : nan
...
! V. Local declaration
! -----------------------------------------------------------------
  integer, parameter :: npftpar=49       ! 32 parameter of each PFT 
  ...
  logical  :: pool_tuning
  real(r8) :: Isf(numpft+1)
  real(r8) :: Iss(numpft+1)
  real(r8) :: Ksf(numpft+1)
  real(r8) :: Kss(numpft+1)
  real(r8), pointer :: Isf_sum(:)
  real(r8), pointer :: Iss_sum(:)
  real(r8), pointer :: Ksf_sum(:)
  real(r8), pointer :: Kss_sum(:)
  ...
  real(r8) :: getig_in                        !IAPDGVM                   
  real(r8) :: getrhm_in                        !IAPDGVM
  integer i,j,lc,uc,lb,ub,jm               ! loop/array indices
  integer c,p,p1,p2,n_pft,pp
...
! -----------------------------------------------------------------
      ...
      do c = 1, numcolumn
         ...
         if(i.eq.214 .and. j.eq.64) then
            do p = p1, p2
               write(6,*) 'wxy_patch', ppmap(p), wxy_patch(p), c
            end do
         end if
      ...
      if(enable_carbon_spinup .and. MOD(get_nstep(),cspinup_nstep) == 0) then
         if(p_master) write(6,*) 'cpool tuning'
         pool_tuning = .true.
      else
         pool_tuning = .false.
      end if
      ...
      DO c = 1, numcolumn
         ...
         if(mybug) write(6,*), 'debug', p_iam, c
#endif
...
! ======================================================================
         ...
         itypwat_c =  nint(fcon_col(uc,c))       ; lc = uc + 1; uc = uc + 2        !3
         albsat    => fcon_col(lc:uc,c)          ; lc = uc + 1; uc = uc + 2        !4
         albdry    => fcon_col(lc:uc,c)          ; lc = uc + 1; uc = uc + nl_soil  !5
...
#ifdef MYBUG
         if(abs(dlat*180./3.14-29.06).lt.0.1 .and. abs(dlon*180./3.14-105.47).lt.0.1) then
            mybug = .true.
         else
            mybug = .false.
         end if
         ...
         refl      => fcon_pft(lb:ub,p1:p2)          ; lb = ub + 1; ub = ub + 4        !17-20
         refs      => fcon_pft(lb:ub,p1:p2)          ; lb = ub + 1; ub = ub + 4        !17-20
         tranl     => fcon_pft(lb:ub,p1:p2)          ; lb = ub + 1; ub = ub + 4        !17-20
         trans     => fcon_pft(lb:ub,p1:p2)          ; lb = ub + 1; ub = ub + nl_soil  !21-24
         rootfr    => fcon_pft(lb:ub,p1:p2)                                            !25-34
#if(defined DGVM)
         ub = ub + 1
       !  write(6,*) 'CLMDRIVER zjw1', ub
         pftpar    => fcon_pft(ub:ub+npftpar-1,p1:p2)    ; ub = ub+npftpar       !35-66
       !  write(6,*) 'CLMDRIVER zjw2', pftpar(28,:)
       !  write(6,*) 'CLMDRIVER zjw3', pftpar(29,:)
         ...
         prec365   => fvar_col(uc,c)                     ; uc = uc+1             !3
         wliq6mon  => fvar_col(uc,c)   !4
#endif
#if(defined DyN)
         uc = uc+1
         ...
         annfrg    => fvar_pft(ub,p1:p2)                 ; ub = ub+1 
         afirefrac1=> fvar_pft(ub,p1:p2)                 ; ub = ub+1             !69
         nfireg1   => fvar_pft(ub,p1:p2)                                         !70
#if(defined DyN)
         ub=ub+1
...
! Yiqi Luo's spinup
!
         Isf_sum => Isf_pft(p1:p2)
         Iss_sum => Iss_pft(p1:p2)
         Ksf_sum => Ksf_pft(p1:p2)
         Kss_sum => Kss_pft(p1:p2)
#endif

#if(defined VEGDATA)
         call interp_vegdata(itypwat_c,p1,p2,lai_r,sai_r,htop_r)
#endif
...
! ======================================================================
         ...
         if(mybug) then
            ...
            write(6,*) "lai", lai(1:n_pft)
            write(6,*) "ivt", ivt(1:n_pft)
            write(6,*) "fsena", fsena(1:n_pft)
            write(6,*) "lfevpa", lfevpa(1:n_pft)
            write(6,*) "olrg", olrg(1:n_pft)
            write(6,*) "tg", tg
         end if
#endif
         ...
         if(mybug) then
            ...
            write(6,*) "p1, p2", p1, p2
            write(6,*) "wt_pft", wt_pft(1:n_pft)
            write(6,*) "wxy_patch", wxy_patch(p1:p2)
         end if
...
!   do pp=1,n_pft 
!   if (wt_pft(pp) < 0._r8 .or. wt_pft(pp) > 1._r8+1.e-6_r8) then
!           write(6,*) 'wt_pft(CLMDRI) zjw=', wt_pft(pp),wt_col
!   end if
!   end do
       ...
       ! vegetation information
         ...
         nday,nyr,&
         Isf,Iss,Ksf,Kss,&
         area_xy, getig_in, getrhm_in,& !IAPDGVM
#endif   
#if (defined DyN)
       ...
       ! fluxes
         ...
         parsun(1:n_pft), parsha(1:n_pft), sabvsun, sabvsha, sabg, sabvg(1:n_pft), xerr, zerr, &
         snm, qsubl(1:n_pft), &
...
#if (defined VEGDATA)
         lai_r(1:n_pft), sai_r(1:n_pft), green_r(1:n_pft), fveg_r(1:n_pft), htop_r(1:n_pft), &
#endif
       ...
       ! additional variables required by coupling with regional model (WRF & RSM) 
         ...
         if(enable_carbon_spinup .and. itypwat_c==0) then
            Isf_sum = Isf_sum + Isf
            Iss_sum = Iss_sum + Iss
            Ksf_sum = Ksf_sum + Ksf
            Kss_sum = Kss_sum + Kss
            ...
            if(i == 1 .and. j == 1) then
               write(6,*) "I1J1 Isf", Isf
               write(6,*) "I1J1 Iss", Iss
               write(6,*) "I1J1 Ksf", Ksf
               write(6,*) "I1J1 Kss", Kss
               write(6,*) "I1J1 cpool_fast", cpool_fast
               write(6,*) "I1J1 cpool_slow", cpool_slow
            end if

            if(i == 2 .and. j == 1) then
               write(6,*) "I2J1 Isf", Isf
               write(6,*) "I2J1 Iss", Iss
               write(6,*) "I2J1 Ksf", Ksf
               write(6,*) "I2J1 Kss", Kss
               write(6,*) "I2J1 cpool_fast", cpool_fast
               write(6,*) "I2J1 cpool_slow", cpool_slow
            end if
#endif
            ...
            if(pool_tuning) then
             ...
             ! print *, 'cpool_slow', sum(cpool_slow)
               ...
               where(Ksf_sum.ne.0) cpool_fast = Isf_sum/Ksf_sum
               where(Kss_sum.ne.0) cpool_slow = Iss_sum/Kss_sum

#ifdef MYBUG
               if(i == 1 .and. j == 1) then
                  write(6,*) "I1J1 Isf_sum", Isf_sum
                  write(6,*) "I1J1 Iss_sum", Iss_sum
                  write(6,*) "I1J1 Ksf_sum", Ksf_sum
                  write(6,*) "I1J1 Kss_sum", Kss_sum
                  write(6,*) "I1J1 cpool_fast", cpool_fast
                  write(6,*) "I1J1 cpool_slow", cpool_slow
               end if

               if(i == 2 .and. j == 1) then
                  write(6,*) "I2J1 Isf_sum", Isf_sum
                  write(6,*) "I2J1 Iss_sum", Iss_sum
                  write(6,*) "I2J1 Ksf_sum", Ksf_sum
                  write(6,*) "I2J1 Kss_sum", Kss_sum
                  write(6,*) "I2J1 cpool_fast", cpool_fast
                  write(6,*) "I2J1 cpool_slow", cpool_slow
               end if
#endif
             ...
             !                  Isf_sum(p), Iss_sum(p), Ksf_sum(p), Kss_sum(p)
             !    end if
             ! end do
...
! ======================================================================
       ...
       ! forcing
         ...
         fldv_pft%rnet (p1:p2) = sabvg(1:n_pft) + frl - olrg(1:n_pft)
         ...
         fldv_pft%avsdr(p1:p2) = alb(1,:)
         fldv_pft%avsdf(p1:p2) = alb(3,:)
         fldv_pft%anidr(p1:p2) = alb(2,:)
         fldv_pft%anidf(p1:p2) = alb(4,:)
       ...
       ! snow internal temperature [tsn:K]
         ...
         fldv_pft%qsubl(p1:p2) = qsubl(1:n_pft)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ! liquid water content of up 3 layers over 6 mons IAPDGVM
         call get_curr_date(yr, mon, day, sec)

    ! write(*,*) "yr=", yr
     !write(*,*) "mon=", mon
!     write(*,*) "h2osoi_liq6=", h2osoi_liq6mon(:) 
