#include <define.h>
 ...
 !=======================================================================
  ...
  use colm_varMod, only: lon_points, lat_points, numpatch, numcolumn, numgrid,ftune,&
...
!  real(r8), pointer :: litter_repr(:)         !litter derived from allocation to reproduction for PFT on modelled
 ...
 ! -----------------------------------------------------------------
   ...
   integer, parameter :: npftpar=49 ! 49 parameter of each PFT 
 ...
 !---------------------------------------------------------------------------------------
  ...
  ! g(C)/m2/s -> kg(C)/m2/s
   ...
   !    albdry    => fcon_col(5,c)                                            !5
       albsat    => fcon_col(4:5,c)          
       albdry    => fcon_col(6:7,c) 
       uc = 82                      
     ...
     !  write(6,*) 'zjw1', pftpar(28,:)
     !  write(6,*) 'zjw2', pftpar(29,:)       
       ...
       annfrg            => fvar_pft(ub,p1:p2)                 ; ub = ub+1             !68
       ...
       IF (itypwat_c==0) THEN
        ...
        ! GPP, NPP, NEP, NBP have already been weighted.
          ...
          IF (newyear) THEN
             ...
             frg_ind  = annfrg
#ifndef DyN
             CALL IAPDGVM(nyr         ,n_pft       ,npftpar       ,pftpar        ,&
                      vegclass    ,bm_inc      ,fpcgrid       ,agdd          ,&
                      t_mo_min    ,tmomin20    ,agdd20        ,afmicr        ,&
                      ifpre       ,lm_ind      ,hm_ind        ,sm_ind        ,&
                      rm_ind      ,litterag    ,litterbg      ,turnover_ind  ,&
                      ...
                      lm_sapl       ,&
                      sm_sapl     ,hm_sapl     ,rm_sapl       ,wt_pft        ,&
                      wt_col      ,prec365     ,wliq6mon      ,afirec        ,afiref,&
             ...
             CALL lpjreset(itypwat_c,zlnd,albsat,albdry,nfcon_pft,nfvar_col,nfvar_pft,n_pft,&
                           fcon_pft(1:nfcon_pft,p1:p2),fvar_col(1:nfvar_col,c),&
                           fvar_pft(1:nfvar_pft,p1:p2),ivt,&
                           ivt_old,ifpre,ifpre_old,wt_pft,wt_pft_old,prec365,wliq6mon)
    
             nyr = nyr + 1.

           ! gC/m2/yr => molC/m2/s
           !  nep_residual(g) = (afirec-aestabc)*wxy_column(c)/(12.*dtime)
             nep_residual(g) = -aestabc*wxy_column(c)/(12.*dtime)  !zjw 20180720
